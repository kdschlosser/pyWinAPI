...
IR_SET_WAKE_PATTERN_PARAMS = _IOCTL_IR_SET_WAKE_PATTERN_PARAMS
PIR_SET_WAKE_PATTERN_PARAMS = POINTER(_IOCTL_IR_SET_WAKE_PATTERN_PARAMS)
...
# Author:
# Revision History:
__IRCLASS_IOCTL_H__ = None

if not defined(__IRCLASS_IOCTL_H__):
    __IRCLASS_IOCTL_H__ = 1
    ...
    if NTDDI_VERSION >= NTDDI_VISTA:
        ...
        # This value is defined in wdm.h, but user mode code shouldn't include
        # wdm.h,
        # so we define this here.
        MAXIMUM_FILENAME_LENGTH = None
        if not defined(MAXIMUM_FILENAME_LENGTH):
        ...
        _IR_DEV_CAPS._fields_ = [
            ('ProtocolVersion', ULONG_PTR), # out
            ('NumTransmitPorts', ULONG_PTR), # out
            ('NumReceivePorts', ULONG_PTR), # out
            ('LearningReceiverMask', ULONG_PTR), # out
            ('DevCapsFlags', ULONG_PTR), # out
        ]
        IR_DEV_CAPS_V1 = IR_DEV_CAPS
        PIR_DEV_CAPS_V1 = POINTER(IR_DEV_CAPS)
        ...
        _IR_DEV_CAPS_V2._fields_ = [
            ('WakeProtocols', ULONG_PTR),
        ...
        _IR_TRANSMIT_PARAMS._fields_ = [
            ('TransmitPortMask', ULONG_PTR), # in
            ('CarrierPeriod', ULONG_PTR), # in
            ('Flags', ULONG_PTR), # in
            ('PulseSize', ULONG_PTR), # in
        ]
        TRANSMIT_FLAGS_PULSE_MODE = 0x0001
        TRANSMIT_FLAGS_DC_MODE = 0x0002
        _IR_TRANSMIT_CHUNK._fields_ = [
            ('OffsetToNextChunk', ULONG_PTR), # IR_TRANSMIT_CHUNK (or zero if no more chunks in buffer)
            ('RepeatCount', ULONG_PTR), # number of times to serially repeat "ByteCount" bytes of data
            ('ByteCount', ULONG_PTR), # count of data bytes to be sent
            ('Data', LONG * 1), # Note: Each chunk is filled to integral ULONG_PTR boundary
        ]
        ...
        _IR_PRIORITY_RECEIVE_PARAMS._fields_ = [
            ('DataEnd', ULONG_PTR), # out
            ('ByteCount', ULONG_PTR), # in
            ('CarrierFrequency', ULONG_PTR), # out
            ('Data', LONG * 1), # in
        ]
        ...
        _IOCTL_IR_ENTER_PRIORITY_RECEIVE_PARAMS._fields_ = [
            ('Receiver', ULONG_PTR), # in
            ('TimeOut', ULONG_PTR), # in
        ]
        ...
        _IOCTL_IR_SET_WAKE_PATTERN_PARAMS._fields_ = [
            ('Protocol', ULONG), # in
            ('Payload', ULONG), # in
            ('Address', ULONG), # in
        ]
