...
PKINTERRUPT_POLARITY = POINTER(_KINTERRUPT_POLARITY)
    ...
    if not defined(_PCI_X_):
        ...
        if defined(__cplusplus):
            class PCI_COMMON_HEADER(_PCI_COMMON_CONFIG :):
                pass
        ...
        PCI_COMMON_HDR_LENGTH = (
            UFIELD_OFFSET (PCI_COMMON_CONFIG,
            DeviceSpecific),
        )
        ...
        # if it contains the value of - 1.    # END IF   _PCI_X_
    ...
    PCDRIVER_RUNTIME_INIT_FLAGS = POINTER(_PCDRIVER_RUNTIME_INIT_FLAGS)
    ...
    if POOL_NX_OPTIN and not POOL_NX_OPTOUT:
        ...
        VersionInfo.dwOSVersionInfoSize = ctypes.sizeof(VersionInfo)

        Status = RtlGetVersion(ctypes.byref(VersionInfo))

        if not NT_VERIFY(NT_SUCCESS(Status))):
            MajorVersion = 5
            MinorVersion = 0
        else:
        ...
        if (RuntimeFlags & _DRIVER_RUNTIME_INIT_FLAGS.DrvRtPoolNxOptIn) != 0:
            # Discover whether NX pool support is available on this
            # platform, and, if so, initialize the default non-paged pool type.
