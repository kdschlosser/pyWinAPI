import ctypes
from pyWinAPI import *
from pyWinAPI.shared.wtypes_h import *


HIMAGELIST = None
HPROPSHEETPAGE = None


_INC_SETUPAPI = None
_SETUPAPI_ = None
_SETUPAPI_VER = None
_SPAPI_ERRORS = None
USE_SP_BACKUP_QUEUE_PARAMS_V1 = False
USE_SP_BACKUP_QUEUE_PARAMS_V3 = False
USE_SP_INF_SIGNER_INFO_V1 = False

USE_SP_ALTPLATFORM_INFO_V1 = False
USE_SP_ALTPLATFORM_INFO_V3 = False

USE_SP_DRVINFO_DATA_V1 = False

# + + Copyright (c) Microsoft Corporation. All rights reserved. Module Name:
# setupapi.h Abstract: Public header file for Windows NT Setup and Device
# Installer services Dll. - -
if not defined(_INC_SETUPAPI):
    _INC_SETUPAPI = 1

    from pyWinAPI.shared.winapifamily_h import *
    from pyWinAPI.shared.sdkddkver_h import *
    from pyWinAPI.shared.guiddef_h import *

    if _MSC_VER > 1000:
        pass
    # END IF

    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP):
        if defined(_MSC_VER) and (_MSC_VER >= 1200):
            pass
        # END IF

        # Define API decoration for direct importing of DLL references.
        if not defined(_SETUPAPI_):
            # WINSETUPAPI = DECLSPEC_IMPORT
            WINSETUPAPI = 1
        else:
            WINSETUPAPI = 1
        # END IF

        # determine version of setupapi based on _WIN32_WINDOWS and
        # _WIN32_WINNT

        # NT4 version of setupapi (_WIN32_WINNT_NT4) is earliest, and
        # installed onto Win95 by IE.
        # Win2k version of setupapi (_WIN32_WINNT_WIN2K) also shipped in WinME
        # we'll use "0x0410" to indicate version of setupapi shipped with Win98

        if not defined(_SETUPAPI_VER):
            if (
                defined(_WIN32_WINNT) and
                (
                    not defined(_WIN32_WINDOWS) or
                    (_WIN32_WINNT < _WIN32_WINDOWS)
                )
            ):
                _SETUPAPI_VER = _WIN32_WINNT

            elif defined(_WIN32_WINDOWS):
                if _WIN32_WINDOWS >= 0x0490:
                    _SETUPAPI_VER = _WIN32_WINNT_WIN2K
                elif _WIN32_WINDOWS >= 0x0410:
                    _SETUPAPI_VER = 0x0410
                else:
                    _SETUPAPI_VER = _WIN32_WINNT_NT4
                # END IF   _WIN32_WINDOWS

            else: #  _WIN32_WINNT/_WIN32_WINDOWS
                _SETUPAPI_VER = _WIN32_WINNT_WINXP
            # END IF   _WIN32_WINNT/_WIN32_WINDOWS

        # END IF   not _SETUPAPI_VER

        if not defined(__LPGUID_DEFINED__):
            __LPGUID_DEFINED__ = 1
            LPGUID = POINTER(GUID)
        # END IF

        # Include spapidef.h for basic definitions and flags
        from pyWinAPI.um.spapidef_h import * # NOQA
        # Include commctrl.h for our use of HIMAGELIST and wizard support.
        from pyWinAPI.um.commctrl_h import *  # NOQA
        # Include devpropdef.h for our use of DEVPROPERTYKEY and DEVPROPTYPE.
        from pyWinAPI.shared.devpropdef_h import *  # NOQA

        if defined(_WIN64):
            # include <pshpack8.h>   // Assume 8-byte (64-bit) packing throughout
            pass
        else:
            # include <pshpack1.h>   // Assume byte packing throughout (32-bit processor)
            pass
        # END IF
        if defined(__cplusplus):
            pass
        # END IF

        LINE_LEN = 256
        MAX_INF_STRING_LENGTH = 4096
        MAX_INF_SECTION_NAME_LENGTH = 255
        MAX_TITLE_LEN = 60
        MAX_INSTRUCTION_LEN = 256
        MAX_LABEL_LEN = 30
        MAX_SERVICE_NAME_LEN = 256
        MAX_SUBTITLE_LEN = 256
        SP_MAX_MACHINENAME_LENGTH = MAX_PATH + 3

        # Inf context structure. Applications must not interpret or
        HINF = PVOID


        # overwrite values in these structures.
        class _INFCONTEXT(ctypes.Structure):
            pass


        _INFCONTEXT._fields_ = [
            ('Inf', PVOID),
            ('CurrentInf', PVOID),
            ('Section', UINT),
            ('Line', UINT),
        ]
        INFCONTEXT = _INFCONTEXT
        PINFCONTEXT = POINTER(_INFCONTEXT)

        # Inf file information structure.
        class _SP_INF_INFORMATION(ctypes.Structure):
            pass


        _SP_INF_INFORMATION._fields_ = [
            ('InfStyle', DWORD),
            ('InfCount', DWORD),
            ('VersionData', BYTE * ANYSIZE_ARRAY),
        ]
        SP_INF_INFORMATION = _SP_INF_INFORMATION
        PSP_INF_INFORMATION = POINTER(_SP_INF_INFORMATION)

        # Define structure for passing alternate platform info into

        # SetupSetFileQueueAlternatePlatform and
        # SetupQueryInfOriginalFileInformation.
        class _SP_ALTPLATFORM_INFO_V3(ctypes.Structure):
            pass


        class DUMMYUNIONNAME(ctypes.Union):
            pass


        DUMMYUNIONNAME._fields_ = [
            ('Reserved', WORD), # for compatibility with V1 structure
            ('Flags', WORD), # indicates validity of non V1 fields
        ]
        _SP_ALTPLATFORM_INFO_V3.DUMMYUNIONNAME = DUMMYUNIONNAME
        _SP_ALTPLATFORM_INFO_V3._fields_ = [
            ('cbSize', DWORD),

            # platform to use
            # (VER_PLATFORM_WIN32_WINDOWS or VER_PLATFORM_WIN32_NT)
            ('Platform', DWORD),
            ('MajorVersion', DWORD), # major and minor version numbers to use
            ('MinorVersion', DWORD),
            ('ProcessorArchitecture', WORD), # or PROCESSOR_ARCHITECTURE_ARM).
            ('DUMMYUNIONNAME', _SP_ALTPLATFORM_INFO_V3.DUMMYUNIONNAME),
            ('FirstValidatedMajorVersion', DWORD), # MinorVersion fields.
            ('FirstValidatedMinorVersion', DWORD),
            ('ProductType', BYTE), # VER_SUITE_SMALLBUSINESS.
            ('SuiteMask', WORD),
            ('BuildNumber', DWORD), # later.
        ]
        SP_ALTPLATFORM_INFO_V3 = _SP_ALTPLATFORM_INFO_V3
        PSP_ALTPLATFORM_INFO_V3 = POINTER(_SP_ALTPLATFORM_INFO_V3)
        DUMMYUNIONNAME._fields_ = [
            ('Reserved', WORD), # for compatibility with V1 structure
            ('Flags', WORD), # indicates validity of non V1 fields
        ]

        # specify SP_ALTPLATFORM_FLAGS_VERSION_RANGE in Flags
        # to use FirstValidatedMajorVersion and FirstValidatedMinorVersion
        # Major and minor versions of the oldest previous OS for which this
        # package's digital signature may be considered valid. For example, say
        # the alternate platform is VER_PLATFORM_WIN32_NT, version 5.1.
        # However,
        # it is wished that driver packages INT with a 5.0 osattr also be
        # considered valid. In this case, you'd have a
        # MajorVersion/MinorVersion
        # of 5.1, and a FirstValidatedMajorVersion/FirstValidatedMinorVersion
        # of
        # 5.0. To validate packages INT for any previous OS release, specify
        # 0 for these fields. To only validate against the target alternate
        # platform, specify the same values as those in the MajorVersion and
        # MinorVersion fields.
        # specify non - zero value (e.g. VER_NT_WORKSTATION) in ProductType to
        # use
        # field, and/or specify SP_ALTPLATFORM_FLAGS_SUITE_MASK in Flags to use
        # SuiteMask field, which may be zero.
        # Product type and suite mask of alternate platform. Used to select
        # matching decorated install sections within driver packages that
        # target
        # specific product variants of the OS. For example, for only Server
        # products with the Enterprise or Small Business suite classification,
        # use ProductType VER_NT_SERVER with SuiteMask VER_SUITE_ENTERPRISE and

        # VER_SUITE_SMALLBUSINESS.
        # Build number of alternate platform. Used to select matching
        # decorated install sections within driver packages that target a
        # minimal build number with the specified OS
        # MajorVersion/MinorVersion. If no specific minimal build number
        # targeting is required, a value of zero should be specified. Note that
        # this capability is only supported on certain builds of 10.0 and
        # later.
        class _SP_ALTPLATFORM_INFO_V2(ctypes.Structure):
            pass


        DUMMYUNIONNAME._fields_ = [
            ('Reserved', WORD),
            ('Flags', WORD),
        ]
        _SP_ALTPLATFORM_INFO_V2.DUMMYUNIONNAME = DUMMYUNIONNAME
        _SP_ALTPLATFORM_INFO_V2._fields_ = [
            ('cbSize', DWORD),
            ('Platform', DWORD),
            ('MajorVersion', DWORD),
            ('MinorVersion', DWORD),
            ('ProcessorArchitecture', WORD),
            ('DUMMYUNIONNAME', _SP_ALTPLATFORM_INFO_V2.DUMMYUNIONNAME),
            ('FirstValidatedMajorVersion', DWORD),
            ('FirstValidatedMinorVersion', DWORD),
        ]
        SP_ALTPLATFORM_INFO_V2 = _SP_ALTPLATFORM_INFO_V2
        PSP_ALTPLATFORM_INFO_V2 = POINTER(_SP_ALTPLATFORM_INFO_V2)
        DUMMYUNIONNAME._fields_ = [
            ('Reserved', WORD),
            ('Flags', WORD),
        ]


        class _SP_ALTPLATFORM_INFO_V1(ctypes.Structure):
            pass


        _SP_ALTPLATFORM_INFO_V1._fields_ = [
            ('cbSize', DWORD),
            ('Platform', DWORD),
            ('MajorVersion', DWORD),
            ('MinorVersion', DWORD),
            ('ProcessorArchitecture', WORD),
            ('Reserved', WORD), # must be zero.
        ]
        SP_ALTPLATFORM_INFO_V1 = _SP_ALTPLATFORM_INFO_V1
        PSP_ALTPLATFORM_INFO_V1 = POINTER(_SP_ALTPLATFORM_INFO_V1)

        if USE_SP_ALTPLATFORM_INFO_V1 or (_SETUPAPI_VER < _WIN32_WINNT_WINXP):
            # use version 1 altplatform info data structure
            SP_ALTPLATFORM_INFO = SP_ALTPLATFORM_INFO_V1
            PSP_ALTPLATFORM_INFO = PSP_ALTPLATFORM_INFO_V1
        elif USE_SP_ALTPLATFORM_INFO_V3 and (NTDDI_VERSION >= NTDDI_WIN10_RS1):
            # use version 3 altplatform info data structure
            SP_ALTPLATFORM_INFO = SP_ALTPLATFORM_INFO_V3
            PSP_ALTPLATFORM_INFO = PSP_ALTPLATFORM_INFO_V3
        else:
            # use version 2 altplatform info data structure
            SP_ALTPLATFORM_INFO = SP_ALTPLATFORM_INFO_V2
            PSP_ALTPLATFORM_INFO = PSP_ALTPLATFORM_INFO_V2
        # END IF   use default version of altplatform info data structure

        # SP_ALTPLATFORM_INFO.Flags values
        if _WIN32_WINNT >= _WIN32_WINNT_WINXP:
            SP_ALTPLATFORM_FLAGS_VERSION_RANGE = 0x0001
        # END IF

        if NTDDI_VERSION >= NTDDI_WIN10_RS1:
            SP_ALTPLATFORM_FLAGS_SUITE_MASK = 0x0002
        # END IF

        # Define structure that is filled in by
        # SetupQueryInfOriginalFileInformation
        # to indicate the INF's original name and the original name of the
        # (potentially

        # platform - specific) catalog file specified by that INF.
        class _SP_ORIGINAL_FILE_INFO_A(ctypes.Structure):
            pass


        _SP_ORIGINAL_FILE_INFO_A._fields_ = [
            ('cbSize', DWORD),
            ('OriginalInfName', CHAR * MAX_PATH),
            ('OriginalCatalogName', CHAR * MAX_PATH),
        ]
        SP_ORIGINAL_FILE_INFO_A = _SP_ORIGINAL_FILE_INFO_A
        PSP_ORIGINAL_FILE_INFO_A = POINTER(_SP_ORIGINAL_FILE_INFO_A)


        class _SP_ORIGINAL_FILE_INFO_W(ctypes.Structure):
            pass


        _SP_ORIGINAL_FILE_INFO_W._fields_ = [
            ('cbSize', DWORD),
            ('OriginalInfName', WCHAR * MAX_PATH),
            ('OriginalCatalogName', WCHAR * MAX_PATH),
        ]
        SP_ORIGINAL_FILE_INFO_W = _SP_ORIGINAL_FILE_INFO_W
        PSP_ORIGINAL_FILE_INFO_W = POINTER(_SP_ORIGINAL_FILE_INFO_W)

        if defined(UNICODE):
            SP_ORIGINAL_FILE_INFO = SP_ORIGINAL_FILE_INFO_W
            PSP_ORIGINAL_FILE_INFO = PSP_ORIGINAL_FILE_INFO_W
        else:
            SP_ORIGINAL_FILE_INFO = SP_ORIGINAL_FILE_INFO_A
            PSP_ORIGINAL_FILE_INFO = PSP_ORIGINAL_FILE_INFO_A
        # END IF

        # SP_INF_INFORMATION.InfStyle values
        INF_STYLE_NONE = 0x00000000
        INF_STYLE_OLDNT = 0x00000001
        INF_STYLE_WIN4 = 0x00000002

        # Additional InfStyle flags that may be specified when calling
        # SetupOpenInfFile.
        INF_STYLE_CACHE_ENABLE = 0x00000010
        INF_STYLE_CACHE_DISABLE = 0x00000020

        if _SETUPAPI_VER >= _WIN32_WINNT_WS03:
            INF_STYLE_CACHE_IGNORE = 0x00000040
        # END IF

        # Target directory specs.
        DIRID_ABSOLUTE = - 1
        DIRID_ABSOLUTE_16BIT = 0xFFFF
        DIRID_NULL = 0
        DIRID_SRCPATH = 1
        DIRID_WINDOWS = 10
        DIRID_SYSTEM = 11
        DIRID_DRIVERS = 12
        DIRID_IOSUBSYS = DIRID_DRIVERS
        DIRID_DRIVER_STORE = 13
        DIRID_INF = 17
        DIRID_HELP = 18
        DIRID_FONTS = 20
        DIRID_VIEWERS = 21
        DIRID_COLOR = 23
        DIRID_APPS = 24
        DIRID_SHARED = 25
        DIRID_BOOT = 30
        DIRID_SYSTEM16 = 50
        DIRID_SPOOL = 51
        DIRID_SPOOLDRIVERS = 52
        DIRID_USERPROFILE = 53
        DIRID_LOADER = 54
        DIRID_PRINTPROCESSOR = 55
        DIRID_DEFAULT = DIRID_SYSTEM

        # The following DIRIDs are for commonly - used shell
        # "special folders". The
        # complete list of such folders is contained in shlobj.h. In that
        # headerfile,
        # each folder is asINT a CSIDL_* value. The DIRID values below are
        # created
        # by taking the CSIDL value in shlobj.h and OR'ing it with 0x4000.
        # Thus, if
        # an INF needs to reference other special folders not defined below,
        # it may
        # generate one using the above mechanism, and setupapi will
        # automatically deal
        # with it and use the corresponding shell's path where appropriate.
        # (Remember
        # that DIRIDs must be specified in decimal, not hex, in an INF when
        # used for string substitution.)
        DIRID_COMMON_STARTMENU = 16406
        DIRID_COMMON_PROGRAMS = 16407
        DIRID_COMMON_STARTUP = 16408
        DIRID_COMMON_DESKTOPDIRECTORY = 16409
        DIRID_COMMON_FAVORITES = 16415
        DIRID_COMMON_APPDATA = 16419
        DIRID_PROGRAM_FILES = 16422
        DIRID_SYSTEM_X86 = 16425
        DIRID_PROGRAM_FILES_X86 = 16426
        DIRID_PROGRAM_FILES_COMMON = 16427
        DIRID_PROGRAM_FILES_COMMONX86 = 16428
        DIRID_COMMON_TEMPLATES = 16429
        DIRID_COMMON_DOCUMENTS = 16430

        # First user - definable dirid. See SetupSetDirectoryId().
        DIRID_USER = 0x8000

        # Setup callback notification routine type

        # typedef UINT (CALLBACK* PSP_FILE_CALLBACK_A)(
        # IN PVOID Context,
        # IN UINT Notification,
        # IN UINT_PTR Param1,
        # IN UINT_PTR Param2
        # );

        PSP_FILE_CALLBACK_A = POINTER(
            CALLBACK(UINT, PVOID, UINT, UINT_PTR, UINT_PTR)
        )

        # typedef UINT (CALLBACK* PSP_FILE_CALLBACK_W)(
        # IN PVOID Context,
        # IN UINT Notification,
        # IN UINT_PTR Param1,
        # IN UINT_PTR Param2
        # );

        PSP_FILE_CALLBACK_W = POINTER(
            CALLBACK(UINT, PVOID, UINT, UINT_PTR, UINT_PTR)
        )

        if defined(UNICODE):
            PSP_FILE_CALLBACK = PSP_FILE_CALLBACK_W
        else:
            PSP_FILE_CALLBACK = PSP_FILE_CALLBACK_A
        # END IF

        # Operation/queue start/end notification. These are ordinal values.
        SPFILENOTIFY_STARTQUEUE = 0x00000001
        SPFILENOTIFY_ENDQUEUE = 0x00000002
        SPFILENOTIFY_STARTSUBQUEUE = 0x00000003
        SPFILENOTIFY_ENDSUBQUEUE = 0x00000004
        SPFILENOTIFY_STARTDELETE = 0x00000005
        SPFILENOTIFY_ENDDELETE = 0x00000006
        SPFILENOTIFY_DELETEERROR = 0x00000007
        SPFILENOTIFY_STARTRENAME = 0x00000008
        SPFILENOTIFY_ENDRENAME = 0x00000009
        SPFILENOTIFY_RENAMEERROR = 0x0000000A
        SPFILENOTIFY_STARTCOPY = 0x0000000B
        SPFILENOTIFY_ENDCOPY = 0x0000000C
        SPFILENOTIFY_COPYERROR = 0x0000000D
        SPFILENOTIFY_NEEDMEDIA = 0x0000000E
        SPFILENOTIFY_QUEUESCAN = 0x0000000F

        # These are used with SetupIterateCabinet().
        SPFILENOTIFY_CABINETINFO = 0x00000010
        SPFILENOTIFY_FILEINCABINET = 0x00000011
        SPFILENOTIFY_NEEDNEWCABINET = 0x00000012
        SPFILENOTIFY_FILEEXTRACTED = 0x00000013
        SPFILENOTIFY_FILEOPDELAYED = 0x00000014

        # These are used for backup operations
        SPFILENOTIFY_STARTBACKUP = 0x00000015
        SPFILENOTIFY_BACKUPERROR = 0x00000016
        SPFILENOTIFY_ENDBACKUP = 0x00000017

        # Extended notification for
        # SetupScanFileQueue(Flags=SPQ_SCAN_USE_CALLBACKEX)
        SPFILENOTIFY_QUEUESCAN_EX = 0x00000018
        SPFILENOTIFY_STARTREGISTRATION = 0x00000019
        SPFILENOTIFY_ENDREGISTRATION = 0x00000020

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # Extended notification for
            # SetupScanFileQueue(Flags=SPQ_SCAN_USE_CALLBACK_SIGNERINFO)
            SPFILENOTIFY_QUEUESCAN_SIGNERINFO = 0x00000040
        # END IF

        # Copy notification. These are bit flags that may be combined.
        SPFILENOTIFY_LANGMISMATCH = 0x00010000
        SPFILENOTIFY_TARGETEXISTS = 0x00020000
        SPFILENOTIFY_TARGETNEWER = 0x00040000

        # File operation codes and callback outcomes.
        FILEOP_COPY = 0
        FILEOP_RENAME = 1
        FILEOP_DELETE = 2
        FILEOP_BACKUP = 3
        FILEOP_ABORT = 0
        FILEOP_DOIT = 1
        FILEOP_SKIP = 2
        FILEOP_RETRY = FILEOP_DOIT
        FILEOP_NEWPATH = 4

        # Flags in inf copy sections
        COPYFLG_WARN_IF_SKIP = 0x00000001
        COPYFLG_NOSKIP = 0x00000002
        COPYFLG_NOVERSIONCHECK = 0x00000004
        COPYFLG_FORCE_FILE_IN_USE = 0x00000008
        COPYFLG_NO_OVERWRITE = 0x00000010
        COPYFLG_NO_VERSION_DIALOG = 0x00000020
        COPYFLG_OVERWRITE_OLDER_ONLY = 0x00000040
        COPYFLG_PROTECTED_WINDOWS_DRIVER_FILE = 0x00000100

        # protected as other Windows system files
        COPYFLG_REPLACEONLY = 0x00000400
        COPYFLG_NODECOMP = 0x00000800
        COPYFLG_REPLACE_BOOT_FILE = 0x00001000

        # needed by the loader); this flag implies a reboot
        COPYFLG_NOPRUNE = 0x00002000
        COPYFLG_IN_USE_TRY_RENAME = 0x00004000

        # Flags in inf delete sections

        # New flags go in high word
        DELFLG_IN_USE = 0x00000001
        DELFLG_IN_USE1 = 0x00010000

        # Source and file paths. Used when notifying queue callback

        # of SPFILENOTIFY_STARTxxx, SPFILENOTIFY_ENDxxx, and
        # SPFILENOTIFY_xxxERROR.
        class _FILEPATHS_A(ctypes.Structure):
            pass


        _FILEPATHS_A._fields_ = [
            ('Target', PCSTR),
            ('Source', PCSTR), # not used for delete operations
            ('Win32Error', UINT),
            ('Flags', DWORD), # such as SP_COPY_NOSKIP for copy errors
        ]
        FILEPATHS_A = _FILEPATHS_A
        PFILEPATHS_A = POINTER(_FILEPATHS_A)


        class _FILEPATHS_W(ctypes.Structure):
            pass


        _FILEPATHS_W._fields_ = [
            ('Target', PCWSTR),
            ('Source', PCWSTR), # not used for delete operations
            ('Win32Error', UINT),
            ('Flags', DWORD), # such as SP_COPY_NOSKIP for copy errors
        ]
        FILEPATHS_W = _FILEPATHS_W
        PFILEPATHS_W = POINTER(_FILEPATHS_W)

        if defined(UNICODE):
            FILEPATHS = FILEPATHS_W
            PFILEPATHS = PFILEPATHS_W
        else:
            FILEPATHS = FILEPATHS_A
            PFILEPATHS = PFILEPATHS_A
        # END IF
        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            class _FILEPATHS_SIGNERINFO_A(ctypes.Structure):
                pass


            _FILEPATHS_SIGNERINFO_A._fields_ = [
                ('Target', PCSTR),
                ('Source', PCSTR), # not used for delete operations
                ('Win32Error', UINT),
                ('Flags', DWORD), # such as SP_COPY_NOSKIP for copy errors
                ('DigitalSigner', PCSTR),
                ('Version', PCSTR),
                ('CatalogFile', PCSTR),
            ]
            FILEPATHS_SIGNERINFO_A = _FILEPATHS_SIGNERINFO_A
            PFILEPATHS_SIGNERINFO_A = POINTER(_FILEPATHS_SIGNERINFO_A)


            class _FILEPATHS_SIGNERINFO_W(ctypes.Structure):
                pass


            _FILEPATHS_SIGNERINFO_W._fields_ = [
                ('Target', PCWSTR),
                ('Source', PCWSTR), # not used for delete operations
                ('Win32Error', UINT),
                ('Flags', DWORD), # such as SP_COPY_NOSKIP for copy errors
                ('DigitalSigner', PCWSTR),
                ('Version', PCWSTR),
                ('CatalogFile', PCWSTR),
            ]
            FILEPATHS_SIGNERINFO_W = _FILEPATHS_SIGNERINFO_W
            PFILEPATHS_SIGNERINFO_W = POINTER(_FILEPATHS_SIGNERINFO_W)

            if defined(UNICODE):
                FILEPATHS_SIGNERINFO = FILEPATHS_SIGNERINFO_W
                PFILEPATHS_SIGNERINFO = PFILEPATHS_SIGNERINFO_W
            else:
                FILEPATHS_SIGNERINFO = FILEPATHS_SIGNERINFO_A
                PFILEPATHS_SIGNERINFO = PFILEPATHS_SIGNERINFO_A
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Structure used with SPFILENOTIFY_NEEDMEDIA
        class _SOURCE_MEDIA_A(ctypes.Structure):
            pass


        _SOURCE_MEDIA_A._fields_ = [
            ('Reserved', PCSTR),
            ('Tagfile', PCSTR), # may be NULL
            ('Description', PCSTR),
            ('SourcePath', PCSTR), # that caused us to need the media.
            ('SourceFile', PCSTR),
            ('Flags', DWORD), # subset of SP_COPY_xxx
        ]
        SOURCE_MEDIA_A = _SOURCE_MEDIA_A
        PSOURCE_MEDIA_A = POINTER(_SOURCE_MEDIA_A)


        class _SOURCE_MEDIA_W(ctypes.Structure):
            pass


        _SOURCE_MEDIA_W._fields_ = [
            ('Reserved', PCWSTR),
            ('Tagfile', PCWSTR), # may be NULL
            ('Description', PCWSTR),
            ('SourcePath', PCWSTR), # that caused us to need the media.
            ('SourceFile', PCWSTR),
            ('Flags', DWORD), # subset of SP_COPY_xxx
        ]
        SOURCE_MEDIA_W = _SOURCE_MEDIA_W
        PSOURCE_MEDIA_W = POINTER(_SOURCE_MEDIA_W)
        if defined(UNICODE):
            SOURCE_MEDIA = SOURCE_MEDIA_W
            PSOURCE_MEDIA = PSOURCE_MEDIA_W
        else:
            SOURCE_MEDIA = SOURCE_MEDIA_A
            PSOURCE_MEDIA = PSOURCE_MEDIA_A
        # END IF

        # Structure used with SPFILENOTIFY_CABINETINFO and

        # SPFILENOTIFY_NEEDNEWCABINET
        class _CABINET_INFO_A(ctypes.Structure):
            pass


        _CABINET_INFO_A._fields_ = [
            ('CabinetPath', PCSTR),
            ('CabinetFile', PCSTR),
            ('DiskName', PCSTR),
            ('SetId', USHORT),
            ('CabinetNumber', USHORT),
        ]
        CABINET_INFO_A = _CABINET_INFO_A
        PCABINET_INFO_A = POINTER(_CABINET_INFO_A)


        class _CABINET_INFO_W(ctypes.Structure):
            pass


        _CABINET_INFO_W._fields_ = [
            ('CabinetPath', PCWSTR),
            ('CabinetFile', PCWSTR),
            ('DiskName', PCWSTR),
            ('SetId', USHORT),
            ('CabinetNumber', USHORT),
        ]
        CABINET_INFO_W = _CABINET_INFO_W
        PCABINET_INFO_W = POINTER(_CABINET_INFO_W)

        if defined(UNICODE):
            CABINET_INFO = CABINET_INFO_W
            PCABINET_INFO = PCABINET_INFO_W
        else:
            CABINET_INFO = CABINET_INFO_A
            PCABINET_INFO = PCABINET_INFO_A
        # END IF

        # Structure used with SPFILENOTIFY_FILEINCABINET
        class _FILE_IN_CABINET_INFO_A(ctypes.Structure):
            pass


        _FILE_IN_CABINET_INFO_A._fields_ = [
            ('NameInCabinet', PCSTR),
            ('FileSize', DWORD),
            ('Win32Error', DWORD),
            ('DosDate', WORD),
            ('DosTime', WORD),
            ('DosAttribs', WORD),
            ('FullTargetName', CHAR * MAX_PATH),
        ]
        FILE_IN_CABINET_INFO_A = _FILE_IN_CABINET_INFO_A
        PFILE_IN_CABINET_INFO_A = POINTER(_FILE_IN_CABINET_INFO_A)


        class _FILE_IN_CABINET_INFO_W(ctypes.Structure):
            pass


        _FILE_IN_CABINET_INFO_W._fields_ = [
            ('NameInCabinet', PCWSTR),
            ('FileSize', DWORD),
            ('Win32Error', DWORD),
            ('DosDate', WORD),
            ('DosTime', WORD),
            ('DosAttribs', WORD),
            ('FullTargetName', WCHAR * MAX_PATH),
        ]
        FILE_IN_CABINET_INFO_W = _FILE_IN_CABINET_INFO_W
        PFILE_IN_CABINET_INFO_W = POINTER(_FILE_IN_CABINET_INFO_W)

        if defined(UNICODE):
            FILE_IN_CABINET_INFO = FILE_IN_CABINET_INFO_W
            PFILE_IN_CABINET_INFO = PFILE_IN_CABINET_INFO_W
        else:
            FILE_IN_CABINET_INFO = FILE_IN_CABINET_INFO_A
            PFILE_IN_CABINET_INFO = PFILE_IN_CABINET_INFO_A
        # END IF

        # Structure used for SPFILENOTIFY_***REGISTRATION

        # callback
        class _SP_REGISTER_CONTROL_STATUSA(ctypes.Structure):
            pass


        _SP_REGISTER_CONTROL_STATUSA._fields_ = [
            ('cbSize', DWORD),
            ('FileName', PCSTR),
            ('Win32Error', DWORD),
            ('FailureCode', DWORD),
        ]
        SP_REGISTER_CONTROL_STATUSA = _SP_REGISTER_CONTROL_STATUSA
        PSP_REGISTER_CONTROL_STATUSA = POINTER(_SP_REGISTER_CONTROL_STATUSA)


        class _SP_REGISTER_CONTROL_STATUSW(ctypes.Structure):
            pass


        _SP_REGISTER_CONTROL_STATUSW._fields_ = [
            ('cbSize', DWORD),
            ('FileName', PCWSTR),
            ('Win32Error', DWORD),
            ('FailureCode', DWORD),
        ]
        SP_REGISTER_CONTROL_STATUSW = _SP_REGISTER_CONTROL_STATUSW
        PSP_REGISTER_CONTROL_STATUSW = POINTER(_SP_REGISTER_CONTROL_STATUSW)

        if defined(UNICODE):
            SP_REGISTER_CONTROL_STATUS = SP_REGISTER_CONTROL_STATUSW
            PSP_REGISTER_CONTROL_STATUS = PSP_REGISTER_CONTROL_STATUSW
        else:
            SP_REGISTER_CONTROL_STATUS = SP_REGISTER_CONTROL_STATUSA
            PSP_REGISTER_CONTROL_STATUS = PSP_REGISTER_CONTROL_STATUSA
        # END IF

        # valid values for SP_REGISTER_CONTROL_STATUS.FailureCode field
        SPREG_SUCCESS = 0x00000000
        SPREG_LOADLIBRARY = 0x00000001
        SPREG_GETPROCADDR = 0x00000002
        SPREG_REGSVR = 0x00000003
        SPREG_DLLINSTALL = 0x00000004
        SPREG_TIMEOUT = 0x00000005
        SPREG_UNKNOWN = 0xFFFFFFFF

        # Define type for setup file queue
        HSPFILEQ = PVOID

        # Structure used with SetupQueueCopyIndirect
        class _SP_FILE_COPY_PARAMS_A(ctypes.Structure):
            pass


        _SP_FILE_COPY_PARAMS_A._fields_ = [
            ('cbSize', DWORD),
            ('QueueHandle', HSPFILEQ),
            ('SourceRootPath', PCSTR),
            ('SourcePath', PCSTR),
            ('SourceFilename', PCSTR),
            ('SourceDescription', PCSTR),
            ('SourceTagfile', PCSTR),
            ('TargetDirectory', PCSTR),
            ('TargetFilename', PCSTR),
            ('CopyStyle', DWORD),
            ('LayoutInf', HINF),
            ('SecurityDescriptor', PCSTR),
        ]
        SP_FILE_COPY_PARAMS_A = _SP_FILE_COPY_PARAMS_A
        PSP_FILE_COPY_PARAMS_A = POINTER(_SP_FILE_COPY_PARAMS_A)


        class _SP_FILE_COPY_PARAMS_W(ctypes.Structure):
            pass


        _SP_FILE_COPY_PARAMS_W._fields_ = [
            ('cbSize', DWORD),
            ('QueueHandle', HSPFILEQ),
            ('SourceRootPath', PCWSTR),
            ('SourcePath', PCWSTR),
            ('SourceFilename', PCWSTR),
            ('SourceDescription', PCWSTR),
            ('SourceTagfile', PCWSTR),
            ('TargetDirectory', PCWSTR),
            ('TargetFilename', PCWSTR),
            ('CopyStyle', DWORD),
            ('LayoutInf', HINF),
            ('SecurityDescriptor', PCWSTR),
        ]
        SP_FILE_COPY_PARAMS_W = _SP_FILE_COPY_PARAMS_W
        PSP_FILE_COPY_PARAMS_W = POINTER(_SP_FILE_COPY_PARAMS_W)

        if defined(UNICODE):
            SP_FILE_COPY_PARAMS = SP_FILE_COPY_PARAMS_W
            PSP_FILE_COPY_PARAMS = PSP_FILE_COPY_PARAMS_W
        else:
            SP_FILE_COPY_PARAMS = SP_FILE_COPY_PARAMS_A
            PSP_FILE_COPY_PARAMS = PSP_FILE_COPY_PARAMS_A
        # END IF

        # Define type for setup disk space list
        HDSKSPC = PVOID

        # Define type for reference to device information set
        HDEVINFO = PVOID

        # Device information structure (references a device instance
        # that is a member of a device information set)
        class _SP_DEVINFO_DATA(ctypes.Structure):
            pass


        _SP_DEVINFO_DATA._fields_ = [
            ('cbSize', DWORD),
            ('ClassGuid', GUID),
            ('DevInst', DWORD), # DEVINST handle
            ('Reserved', ULONG_PTR),
        ]
        SP_DEVINFO_DATA = _SP_DEVINFO_DATA
        PSP_DEVINFO_DATA = POINTER(_SP_DEVINFO_DATA)

        # Device interface information structure (references a device
        # interface that is associated with the device information
        # element that owns it).
        class _SP_DEVICE_INTERFACE_DATA(ctypes.Structure):
            pass


        _SP_DEVICE_INTERFACE_DATA._fields_ = [
            ('cbSize', DWORD),
            ('InterfaceClassGuid', GUID),
            ('Flags', DWORD),
            ('Reserved', ULONG_PTR),
        ]
        SP_DEVICE_INTERFACE_DATA = _SP_DEVICE_INTERFACE_DATA
        PSP_DEVICE_INTERFACE_DATA = POINTER(_SP_DEVICE_INTERFACE_DATA)

        # Flags for SP_DEVICE_INTERFACE_DATA.Flags field.
        SPINT_ACTIVE = 0x00000001
        SPINT_DEFAULT = 0x00000002
        SPINT_REMOVED = 0x00000004

        # Backward compatibility - - do not use.
        SP_INTERFACE_DEVICE_DATA = SP_DEVICE_INTERFACE_DATA
        PSP_INTERFACE_DEVICE_DATA = PSP_DEVICE_INTERFACE_DATA
        SPID_ACTIVE = SPINT_ACTIVE
        SPID_DEFAULT = SPINT_DEFAULT
        SPID_REMOVED = SPINT_REMOVED


        class _SP_DEVICE_INTERFACE_DETAIL_DATA_A(ctypes.Structure):
            pass


        _SP_DEVICE_INTERFACE_DETAIL_DATA_A._fields_ = [
            ('cbSize', DWORD),
            ('DevicePath', CHAR * ANYSIZE_ARRAY),
        ]
        SP_DEVICE_INTERFACE_DETAIL_DATA_A = _SP_DEVICE_INTERFACE_DETAIL_DATA_A
        PSP_DEVICE_INTERFACE_DETAIL_DATA_A = POINTER(_SP_DEVICE_INTERFACE_DETAIL_DATA_A)


        class _SP_DEVICE_INTERFACE_DETAIL_DATA_W(ctypes.Structure):
            pass


        _SP_DEVICE_INTERFACE_DETAIL_DATA_W._fields_ = [
            ('cbSize', DWORD),
            ('DevicePath', WCHAR * ANYSIZE_ARRAY),
        ]
        SP_DEVICE_INTERFACE_DETAIL_DATA_W = _SP_DEVICE_INTERFACE_DETAIL_DATA_W
        PSP_DEVICE_INTERFACE_DETAIL_DATA_W = POINTER(_SP_DEVICE_INTERFACE_DETAIL_DATA_W)

        if defined(UNICODE):
            SP_DEVICE_INTERFACE_DETAIL_DATA = SP_DEVICE_INTERFACE_DETAIL_DATA_W
            PSP_DEVICE_INTERFACE_DETAIL_DATA = PSP_DEVICE_INTERFACE_DETAIL_DATA_W
        else:
            SP_DEVICE_INTERFACE_DETAIL_DATA = SP_DEVICE_INTERFACE_DETAIL_DATA_A
            PSP_DEVICE_INTERFACE_DETAIL_DATA = PSP_DEVICE_INTERFACE_DETAIL_DATA_A
        # END IF

        # Backward compatibility - - do not use.
        SP_INTERFACE_DEVICE_DETAIL_DATA_W = SP_DEVICE_INTERFACE_DETAIL_DATA_W
        PSP_INTERFACE_DEVICE_DETAIL_DATA_W = PSP_DEVICE_INTERFACE_DETAIL_DATA_W
        SP_INTERFACE_DEVICE_DETAIL_DATA_A = SP_DEVICE_INTERFACE_DETAIL_DATA_A
        PSP_INTERFACE_DEVICE_DETAIL_DATA_A = PSP_DEVICE_INTERFACE_DETAIL_DATA_A

        if defined(UNICODE):
            SP_INTERFACE_DEVICE_DETAIL_DATA = SP_INTERFACE_DEVICE_DETAIL_DATA_W
            PSP_INTERFACE_DEVICE_DETAIL_DATA = PSP_INTERFACE_DEVICE_DETAIL_DATA_W
        else:
            SP_INTERFACE_DEVICE_DETAIL_DATA = SP_INTERFACE_DEVICE_DETAIL_DATA_A
            PSP_INTERFACE_DEVICE_DETAIL_DATA = PSP_INTERFACE_DEVICE_DETAIL_DATA_A
        # END IF

        # Structure for detailed information on a device information set
        # (used for
        # SetupDiGetDeviceInfoListDetail which supercedes the functionality of
        # SetupDiGetDeviceInfoListClass).
        class _SP_DEVINFO_LIST_DETAIL_DATA_A(ctypes.Structure):
            pass


        _SP_DEVINFO_LIST_DETAIL_DATA_A._fields_ = [
            ('cbSize', DWORD),
            ('ClassGuid', GUID),
            ('RemoteMachineHandle', HANDLE),
            ('RemoteMachineName', CHAR * SP_MAX_MACHINENAME_LENGTH),
        ]
        SP_DEVINFO_LIST_DETAIL_DATA_A = _SP_DEVINFO_LIST_DETAIL_DATA_A
        PSP_DEVINFO_LIST_DETAIL_DATA_A = POINTER(_SP_DEVINFO_LIST_DETAIL_DATA_A)


        class _SP_DEVINFO_LIST_DETAIL_DATA_W(ctypes.Structure):
            pass


        _SP_DEVINFO_LIST_DETAIL_DATA_W._fields_ = [
            ('cbSize', DWORD),
            ('ClassGuid', GUID),
            ('RemoteMachineHandle', HANDLE),
            ('RemoteMachineName', WCHAR * SP_MAX_MACHINENAME_LENGTH),
        ]
        SP_DEVINFO_LIST_DETAIL_DATA_W = _SP_DEVINFO_LIST_DETAIL_DATA_W
        PSP_DEVINFO_LIST_DETAIL_DATA_W = POINTER(_SP_DEVINFO_LIST_DETAIL_DATA_W)

        if defined(UNICODE):
            SP_DEVINFO_LIST_DETAIL_DATA = SP_DEVINFO_LIST_DETAIL_DATA_W
            PSP_DEVINFO_LIST_DETAIL_DATA = PSP_DEVINFO_LIST_DETAIL_DATA_W
        else:
            SP_DEVINFO_LIST_DETAIL_DATA = SP_DEVINFO_LIST_DETAIL_DATA_A
            PSP_DEVINFO_LIST_DETAIL_DATA = PSP_DEVINFO_LIST_DETAIL_DATA_A
        # END IF

        # Class installer function codes
        DIF_SELECTDEVICE = 0x00000001
        DIF_INSTALLDEVICE = 0x00000002
        DIF_ASSIGNRESOURCES = 0x00000003
        DIF_PROPERTIES = 0x00000004
        DIF_REMOVE = 0x00000005
        DIF_FIRSTTIMESETUP = 0x00000006
        DIF_FOUNDDEVICE = 0x00000007
        DIF_SELECTCLASSDRIVERS = 0x00000008
        DIF_VALIDATECLASSDRIVERS = 0x00000009
        DIF_INSTALLCLASSDRIVERS = 0x0000000A
        DIF_CALCDISKSPACE = 0x0000000B
        DIF_DESTROYPRIVATEDATA = 0x0000000C
        DIF_VALIDATEDRIVER = 0x0000000D
        DIF_DETECT = 0x0000000F
        DIF_INSTALLWIZARD = 0x00000010
        DIF_DESTROYWIZARDDATA = 0x00000011
        DIF_PROPERTYCHANGE = 0x00000012
        DIF_ENABLECLASS = 0x00000013
        DIF_DETECTVERIFY = 0x00000014
        DIF_INSTALLDEVICEFILES = 0x00000015
        DIF_UNREMOVE = 0x00000016
        DIF_SELECTBESTCOMPATDRV = 0x00000017
        DIF_ALLOW_INSTALL = 0x00000018
        DIF_REGISTERDEVICE = 0x00000019
        DIF_NEWDEVICEWIZARD_PRESELECT = 0x0000001A
        DIF_NEWDEVICEWIZARD_SELECT = 0x0000001B
        DIF_NEWDEVICEWIZARD_PREANALYZE = 0x0000001C
        DIF_NEWDEVICEWIZARD_POSTANALYZE = 0x0000001D
        DIF_NEWDEVICEWIZARD_FINISHINSTALL = 0x0000001E
        DIF_UNUSED1 = 0x0000001F
        DIF_INSTALLINTERFACES = 0x00000020
        DIF_DETECTCANCEL = 0x00000021
        DIF_REGISTER_COINSTALLERS = 0x00000022
        DIF_ADDPROPERTYPAGE_ADVANCED = 0x00000023
        DIF_ADDPROPERTYPAGE_BASIC = 0x00000024
        DIF_RESERVED1 = 0x00000025
        DIF_TROUBLESHOOTER = 0x00000026
        DIF_POWERMESSAGEWAKE = 0x00000027
        DIF_ADDREMOTEPROPERTYPAGE_ADVANCED = 0x00000028
        DIF_UPDATEDRIVER_UI = 0x00000029
        DIF_FINISHINSTALL_ACTION = 0x0000002A
        DIF_RESERVED2 = 0x00000030

        # Obsoleted DIF codes (do not use)
        DIF_MOVEDEVICE = 0x0000000E
        DI_FUNCTION = UINT

        # Device installation parameters structure (associated with a
        # particular device information element, or globally with a device
        # information set)
        class _SP_DEVINSTALL_PARAMS_A(ctypes.Structure):
            pass


        _SP_DEVINSTALL_PARAMS_A._fields_ = [
            ('cbSize', DWORD),
            ('Flags', DWORD),
            ('FlagsEx', DWORD),
            ('hwndParent', HWND),
            ('InstallMsgHandler', PSP_FILE_CALLBACK),
            ('InstallMsgHandlerContext', PVOID),
            ('FileQueue', HSPFILEQ),
            ('ClassInstallReserved', ULONG_PTR),
            ('Reserved', DWORD),
            ('DriverPath', CHAR * MAX_PATH),
        ]
        SP_DEVINSTALL_PARAMS_A = _SP_DEVINSTALL_PARAMS_A
        PSP_DEVINSTALL_PARAMS_A = POINTER(_SP_DEVINSTALL_PARAMS_A)


        class _SP_DEVINSTALL_PARAMS_W(ctypes.Structure):
            pass


        _SP_DEVINSTALL_PARAMS_W._fields_ = [
            ('cbSize', DWORD),
            ('Flags', DWORD),
            ('FlagsEx', DWORD),
            ('hwndParent', HWND),
            ('InstallMsgHandler', PSP_FILE_CALLBACK),
            ('InstallMsgHandlerContext', PVOID),
            ('FileQueue', HSPFILEQ),
            ('ClassInstallReserved', ULONG_PTR),
            ('Reserved', DWORD),
            ('DriverPath', WCHAR * MAX_PATH),
        ]
        SP_DEVINSTALL_PARAMS_W = _SP_DEVINSTALL_PARAMS_W
        PSP_DEVINSTALL_PARAMS_W = POINTER(_SP_DEVINSTALL_PARAMS_W)

        if defined(UNICODE):
            SP_DEVINSTALL_PARAMS = SP_DEVINSTALL_PARAMS_W
            PSP_DEVINSTALL_PARAMS = PSP_DEVINSTALL_PARAMS_W
        else:
            SP_DEVINSTALL_PARAMS = SP_DEVINSTALL_PARAMS_A
            PSP_DEVINSTALL_PARAMS = PSP_DEVINSTALL_PARAMS_A
        # END IF

        # SP_DEVINSTALL_PARAMS.Flags values

        # Flags for choosing a device
        DI_SHOWOEM = 0x00000001
        DI_SHOWCOMPAT = 0x00000002
        DI_SHOWCLASS = 0x00000004
        DI_SHOWALL = 0x00000007
        DI_NOVCP = 0x00000008

        # caller - supplied FileQueue
        DI_DIDCOMPAT = 0x00000010
        DI_DIDCLASS = 0x00000020
        DI_AUTOASSIGNRES = 0x00000040

        # flags returned by DiInstallDevice to indicate need to reboot/restart
        DI_NEEDRESTART = 0x00000080
        DI_NEEDREBOOT = 0x00000100

        # flags for device installation
        DI_NOBROWSE = 0x00000200

        # Flags set by DiBuildDriverInfoList
        DI_MULTMFGS = 0x00000400

        # class driver list

        # Flag indicates that device is disabled
        DI_DISABLED = 0x00000800

        # Flags for Device/Class Properties
        DI_GENERALPAGE_ADDED = 0x00001000
        DI_RESOURCEPAGE_ADDED = 0x00002000

        # Flag to indicate the setting properties for this Device (or class)
        # caused a change

        # so the Dev Mgr UI probably needs to be updatd.
        DI_PROPERTIES_CHANGE = 0x00004000

        # Flag to indicate that the sorting from the INF file should be used.
        DI_INF_IS_SORTED = 0x00008000

        # Flag to indicate that only the the INF specified by
        # SP_DEVINSTALL_PARAMS.DriverPath

        # should be searched.
        DI_ENUMSINGLEINF = 0x00010000

        # Flag that prevents ConfigMgr from removing/re - enumerating devices
        # during device

        # registration, installation, and deletion.
        DI_DONOTCALLCONFIGMG = 0x00020000

        # The following flag can be used to install a device disabled
        DI_INSTALLDISABLED = 0x00040000

        # Flag that causes SetupDiBuildDriverInfoList to build a device's
        # compatible driver

        # list from its existing class driver list, instead of the normal INF
        # search.
        DI_COMPAT_FROM_CLASS = 0x00080000

        # This flag is set if the Class Install params should be used.
        DI_CLASSINSTALLPARAMS = 0x00100000

        # This flag is set if the caller of DiCallClassInstaller does NOT
        # want the internal default action performed if the Class installer
        # returns ERROR_DI_DO_DEFAULT.
        DI_NODI_DEFAULTACTION = 0x00200000

        # The setupx flag, DI_NOSYNCPROCESSING (0x00400000L) is not support in
        # the Setup APIs.

        # flags for device installation
        DI_QUIETINSTALL = 0x00800000

        # questions or excess info
        DI_NOFILECOPY = 0x01000000
        DI_FORCECOPY = 0x02000000
        DI_DRIVERPAGE_ADDED = 0x04000000
        DI_USECI_SELECTSTRINGS = 0x08000000
        DI_OVERRIDE_INFFLAGS = 0x10000000
        DI_PROPS_NOCHANGEUSAGE = 0x20000000
        DI_NOSELECTICONS = 0x40000000
        DI_NOWRITE_IDS = 0x80000000

        # SP_DEVINSTALL_PARAMS.FlagsEx values
        DI_FLAGSEX_RESERVED2 = 0x00000001
        DI_FLAGSEX_RESERVED3 = 0x00000002
        DI_FLAGSEX_CI_FAILED = 0x00000004

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            DI_FLAGSEX_FINISHINSTALL_ACTION = 0x00000008
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        DI_FLAGSEX_DIDINFOLIST = 0x00000010
        DI_FLAGSEX_DIDCOMPATINFO = 0x00000020
        DI_FLAGSEX_FILTERCLASSES = 0x00000040
        DI_FLAGSEX_SETFAILEDINSTALL = 0x00000080
        DI_FLAGSEX_DEVICECHANGE = 0x00000100
        DI_FLAGSEX_ALWAYSWRITEIDS = 0x00000200
        DI_FLAGSEX_PROPCHANGE_PENDING = 0x00000400

        # to them, and need to have a DIF_PROPERTYCHANGE occur.
        DI_FLAGSEX_ALLOWEXCLUDEDDRVS = 0x00000800
        DI_FLAGSEX_NOUIONQUERYREMOVE = 0x00001000
        DI_FLAGSEX_USECLASSFORCOMPAT = 0x00002000

        # (Ignored if DI_COMPAT_FROM_CLASS flag is specified.)
        DI_FLAGSEX_RESERVED4 = 0x00004000
        DI_FLAGSEX_NO_DRVREG_MODIFY = 0x00008000
        DI_FLAGSEX_IN_SYSTEM_SETUP = 0x00010000
        DI_FLAGSEX_INET_DRIVER = 0x00020000
        DI_FLAGSEX_APPENDDRIVERLIST = 0x00040000

        # a new driver list to an existing list.
        DI_FLAGSEX_PREINSTALLBACKUP = 0x00080000
        DI_FLAGSEX_BACKUPONREPLACE = 0x00100000
        DI_FLAGSEX_DRIVERLIST_FROM_URL = 0x00200000

        # in SP_DEVINSTALL_PARAMS.DriverPath (empty string means

        # Windows Update website)
        DI_FLAGSEX_RESERVED1 = 0x00400000
        DI_FLAGSEX_EXCLUDE_OLD_INET_DRIVERS = 0x00800000

        # a driver list.

        # Ignored on Windows Vista and later.
        DI_FLAGSEX_POWERPAGE_ADDED = 0x01000000

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            DI_FLAGSEX_FILTERSIMILARDRIVERS = 0x02000000
            DI_FLAGSEX_INSTALLEDDRIVER = 0x04000000

            # driver list. Used in calls to SetupDiBuildDriverInfoList
            DI_FLAGSEX_NO_CLASSLIST_NODE_MERGE = 0x08000000
            DI_FLAGSEX_ALTPLATFORM_DRVSEARCH = 0x10000000

            # specified in associated file queue
            DI_FLAGSEX_RESTART_DEVICE_ONLY = 0x20000000

            # opposed to restarting all devices using those drivers.
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            DI_FLAGSEX_RECURSIVESEARCH = 0x40000000
            DI_FLAGSEX_SEARCH_PUBLISHED_INFS = 0x80000000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        # Class installation parameters header. This must be the first field
        # of any
        # class install parameter structure. The InstallFunction field must be
        # set to
        # the function code corresponding to the structure, and the cbSize
        # field must
        # be set to the size of the header structure. E.g.,
        # SP_ENABLECLASS_PARAMS EnableClassParams;
        # EnableClassParams.ClassInstallHeader.cbSize =
        # ctypes.sizeof(SP_CLASSINSTALL_HEADER);
        # EnableClassParams.ClassInstallHeader.InstallFunction =
        # DIF_ENABLECLASS;
        class _SP_CLASSINSTALL_HEADER(ctypes.Structure):
            pass


        _SP_CLASSINSTALL_HEADER._fields_ = [
            ('cbSize', DWORD),
            ('InstallFunction', DI_FUNCTION),
        ]
        SP_CLASSINSTALL_HEADER = _SP_CLASSINSTALL_HEADER
        PSP_CLASSINSTALL_HEADER = POINTER(_SP_CLASSINSTALL_HEADER)

        # Structure corresponding to a DIF_ENABLECLASS install function.
        class _SP_ENABLECLASS_PARAMS(ctypes.Structure):
            pass


        _SP_ENABLECLASS_PARAMS._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('ClassGuid', GUID),
            ('EnableMessage', DWORD),
        ]
        SP_ENABLECLASS_PARAMS = _SP_ENABLECLASS_PARAMS
        PSP_ENABLECLASS_PARAMS = POINTER(_SP_ENABLECLASS_PARAMS)

        ENABLECLASS_QUERY = 0
        ENABLECLASS_SUCCESS = 1
        ENABLECLASS_FAILURE = 2

        # Values indicating a change in a device's state
        DICS_ENABLE = 0x00000001
        DICS_DISABLE = 0x00000002
        DICS_PROPCHANGE = 0x00000003
        DICS_START = 0x00000004
        DICS_STOP = 0x00000005

        # Values specifying the scope of a device property change
        DICS_FLAG_GLOBAL = 0x00000001
        DICS_FLAG_CONFIGSPECIFIC = 0x00000002
        DICS_FLAG_CONFIGGENERAL = 0x00000004

        # changes to follow.

        # Structure corresponding to a DIF_PROPERTYCHANGE install function.
        class _SP_PROPCHANGE_PARAMS(ctypes.Structure):
            pass


        _SP_PROPCHANGE_PARAMS._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('StateChange', DWORD),
            ('Scope', DWORD),
            ('HwProfile', DWORD),
        ]
        SP_PROPCHANGE_PARAMS = _SP_PROPCHANGE_PARAMS
        PSP_PROPCHANGE_PARAMS = POINTER(_SP_PROPCHANGE_PARAMS)

        # Structure corresponding to a DIF_REMOVE install function.
        class _SP_REMOVEDEVICE_PARAMS(ctypes.Structure):
            pass


        _SP_REMOVEDEVICE_PARAMS._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Scope', DWORD),
            ('HwProfile', DWORD),
        ]
        SP_REMOVEDEVICE_PARAMS = _SP_REMOVEDEVICE_PARAMS
        PSP_REMOVEDEVICE_PARAMS = POINTER(_SP_REMOVEDEVICE_PARAMS)

        DI_REMOVEDEVICE_GLOBAL = 0x00000001
        DI_REMOVEDEVICE_CONFIGSPECIFIC = 0x00000002

        # Structure corresponding to a DIF_UNREMOVE install function.
        class _SP_UNREMOVEDEVICE_PARAMS(ctypes.Structure):
            pass


        _SP_UNREMOVEDEVICE_PARAMS._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Scope', DWORD),
            ('HwProfile', DWORD),
        ]
        SP_UNREMOVEDEVICE_PARAMS = _SP_UNREMOVEDEVICE_PARAMS
        PSP_UNREMOVEDEVICE_PARAMS = POINTER(_SP_UNREMOVEDEVICE_PARAMS)

        DI_UNREMOVEDEVICE_CONFIGSPECIFIC = 0x00000002

        # Structure corresponding to a DIF_SELECTDEVICE install function.
        class _SP_SELECTDEVICE_PARAMS_A(ctypes.Structure):
            pass


        _SP_SELECTDEVICE_PARAMS_A._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Title', CHAR * MAX_TITLE_LEN),
            ('Instructions', CHAR * MAX_INSTRUCTION_LEN),
            ('ListLabel', CHAR * MAX_LABEL_LEN),
            ('SubTitle', CHAR * MAX_SUBTITLE_LEN),
            ('Reserved', BYTE * 2), # DWORD size alignment
        ]
        SP_SELECTDEVICE_PARAMS_A = _SP_SELECTDEVICE_PARAMS_A
        PSP_SELECTDEVICE_PARAMS_A = POINTER(_SP_SELECTDEVICE_PARAMS_A)


        class _SP_SELECTDEVICE_PARAMS_W(ctypes.Structure):
            pass


        _SP_SELECTDEVICE_PARAMS_W._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Title', WCHAR * MAX_TITLE_LEN),
            ('Instructions', WCHAR * MAX_INSTRUCTION_LEN),
            ('ListLabel', WCHAR * MAX_LABEL_LEN),
            ('SubTitle', WCHAR * MAX_SUBTITLE_LEN),
        ]
        SP_SELECTDEVICE_PARAMS_W = _SP_SELECTDEVICE_PARAMS_W
        PSP_SELECTDEVICE_PARAMS_W = POINTER(_SP_SELECTDEVICE_PARAMS_W)

        if defined(UNICODE):
            SP_SELECTDEVICE_PARAMS = SP_SELECTDEVICE_PARAMS_W
            PSP_SELECTDEVICE_PARAMS = PSP_SELECTDEVICE_PARAMS_W
        else:
            SP_SELECTDEVICE_PARAMS = SP_SELECTDEVICE_PARAMS_A
            PSP_SELECTDEVICE_PARAMS = PSP_SELECTDEVICE_PARAMS_A
        # END IF

        # Callback routine for giving progress notification during detection
        # typedef BOOL (CALLBACK* PDETECT_PROGRESS_NOTIFY)(
        #              IN PVOID ProgressNotifyParam,
        #              IN DWORD DetectComplete
        #              );
        PDETECT_PROGRESS_NOTIFY = POINTER(CALLBACK(BOOL, PVOID, DWORD))

        # where:
        # ProgressNotifyParam - value supplied by caller requesting detection.
        # DetectComplete - Percent completion, to be incremented by class
        # installer, as it steps thru its detection.
        # Return Value - If TRUE, then detection is cancelled. Allows caller
        # requesting detection to stop detection asap.

        # Structure corresponding to a DIF_DETECT install function.
        class _SP_DETECTDEVICE_PARAMS(ctypes.Structure):
            pass


        _SP_DETECTDEVICE_PARAMS._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('DetectProgressNotify', PDETECT_PROGRESS_NOTIFY),
            ('ProgressNotifyParam', PVOID),
        ]
        SP_DETECTDEVICE_PARAMS = _SP_DETECTDEVICE_PARAMS
        PSP_DETECTDEVICE_PARAMS = POINTER(_SP_DETECTDEVICE_PARAMS)

        # 'Add New Device' installation wizard structure
        # (backward - compatibility

        # only - - respond to DIF_NEWDEVICEWIZARD_* requests instead).
        # Structure corresponding to a DIF_INSTALLWIZARD install function.
        # (NOTE: This structure is also applicable for DIF_DESTROYWIZARDDATA,
        # but DIF_INSTALLWIZARD is the associated function code in the class
        # installation parameter structure in both cases.)
        # Define maximum number of dynamic wizard pages that can be added to
        # hardware install wizard.
        MAX_INSTALLWIZARD_DYNAPAGES = 20


        class _SP_INSTALLWIZARD_DATA(ctypes.Structure):
            pass


        _SP_INSTALLWIZARD_DATA._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Flags', DWORD),
            ('DynamicPages', HPROPSHEETPAGE * MAX_INSTALLWIZARD_DYNAPAGES),
            ('NumDynamicPages', DWORD),
            ('DynamicPageFlags', DWORD),
            ('PrivateFlags', DWORD),
            ('PrivateData', LPARAM),
            ('hwndWizardDlg', HWND),
        ]
        SP_INSTALLWIZARD_DATA = _SP_INSTALLWIZARD_DATA
        PSP_INSTALLWIZARD_DATA = POINTER(_SP_INSTALLWIZARD_DATA)

        # SP_INSTALLWIZARD_DATA.Flags values
        NDW_INSTALLFLAG_DIDFACTDEFS = 0x00000001
        NDW_INSTALLFLAG_HARDWAREALLREADYIN = 0x00000002
        NDW_INSTALLFLAG_NEEDRESTART = DI_NEEDRESTART
        NDW_INSTALLFLAG_NEEDREBOOT = DI_NEEDREBOOT
        NDW_INSTALLFLAG_NEEDSHUTDOWN = 0x00000200
        NDW_INSTALLFLAG_EXPRESSINTRO = 0x00000400
        NDW_INSTALLFLAG_SKIPISDEVINSTALLED = 0x00000800
        NDW_INSTALLFLAG_NODETECTEDDEVS = 0x00001000
        NDW_INSTALLFLAG_INSTALLSPECIFIC = 0x00002000
        NDW_INSTALLFLAG_SKIPCLASSLIST = 0x00004000
        NDW_INSTALLFLAG_CI_PICKED_OEM = 0x00008000
        NDW_INSTALLFLAG_PCMCIAMODE = 0x00010000
        NDW_INSTALLFLAG_PCMCIADEVICE = 0x00020000
        NDW_INSTALLFLAG_USERCANCEL = 0x00040000
        NDW_INSTALLFLAG_KNOWNCLASS = 0x00080000

        # SP_INSTALLWIZARD_DATA.DynamicPageFlags values
        # This flag is set if a Class installer has added pages to the install
        # wizard.
        DYNAWIZ_FLAG_PAGESADDED = 0x00000001

        # Set this flag if you jump to the analyze page, and want it to
        # handle conflicts for you. NOTE. You will not get control back
        # in the event of a conflict if you set this flag.
        DYNAWIZ_FLAG_ANALYZE_HANDLECONFLICT = 0x00000008

        # The following flags are not used by the Windows NT hardware wizard.
        DYNAWIZ_FLAG_INSTALLDET_NEXT = 0x00000002
        DYNAWIZ_FLAG_INSTALLDET_PREV = 0x00000004

        # Reserve a range of wizard page resource IDs for internal use. Some of
        # these IDs are for use by class installers that respond to the
        # obsolete
        # DIF_INSTALLWIZARD/DIF_DESTROYWIZARDDATA messages. These IDs are
        # listed

        # below.
        MIN_IDD_DYNAWIZ_RESOURCE_ID = 10000
        MAX_IDD_DYNAWIZ_RESOURCE_ID = 11000

        # Define wizard page resource IDs to be used when adding custom pages
        # to the
        # hardware install wizard via DIF_INSTALLWIZARD. Pages marked with
        # (CLASS INSTALLER PROVIDED) _must_ be supplied by the class installer
        # if it
        # responds to the DIF_INSTALLWIZARD request.
        # Resource ID for the first page that the install wizard will go to
        # after
        # adding the class installer pages. (CLASS INSTALLER PROVIDED)
        IDD_DYNAWIZ_FIRSTPAGE = 10000

        # Resource ID for the page that the Select Device page will go back to.
        # (CLASS INSTALLER PROVIDED)
        IDD_DYNAWIZ_SELECT_PREVPAGE = 10001

        # Resource ID for the page that the Select Device page will go forward
        # to.
        # (CLASS INSTALLER PROVIDED)
        IDD_DYNAWIZ_SELECT_NEXTPAGE = 10002

        # Resource ID for the page that the Analyze dialog should go back to
        # This will only be used in the event that there is a problem, and the
        # user
        # selects Back from the analyze proc. (CLASS INSTALLER PROVIDED)
        IDD_DYNAWIZ_ANALYZE_PREVPAGE = 10003

        # Resource ID for the page that the Analyze dialog should go to if it
        # continues from the analyze proc. (CLASS INSTALLER PROVIDED)
        IDD_DYNAWIZ_ANALYZE_NEXTPAGE = 10004

        # Resource ID of the hardware install wizard's select device page.
        # This ID can be used to go directly to the hardware install wizard's
        # select
        # device page.
        # (This is the resource ID of the Select Device wizard page
        # retrieved via SetupDiGetWizardPage when SPWPT_SELECTDEVICE is the
        # requested PageType.)
        IDD_DYNAWIZ_SELECTDEV_PAGE = 10009

        # Resource ID of the hardware install wizard's device analysis page.
        # This ID can be use to go directly to the hardware install wizard's
        # analysis page.
        IDD_DYNAWIZ_ANALYZEDEV_PAGE = 10010

        # Resource ID of the hardware install wizard's install detected
        # devices page.
        # This ID can be use to go directly to the hardware install wizard's
        # install detected devices page.
        IDD_DYNAWIZ_INSTALLDETECTEDDEVS_PAGE = 10011

        # Resource ID of the hardware install wizard's select class page.
        # This ID can be use to go directly to the hardware install wizard's
        # select class page.
        IDD_DYNAWIZ_SELECTCLASS_PAGE = 10012

        # The following class installer - provided wizard page resource IDs
        # are not used by the Windows NT hardware wizard.
        IDD_DYNAWIZ_INSTALLDETECTED_PREVPAGE = 10006
        IDD_DYNAWIZ_INSTALLDETECTED_NEXTPAGE = 10007
        IDD_DYNAWIZ_INSTALLDETECTED_NODEVS = 10008

        # Structure corresponding to the following DIF_NEWDEVICEWIZARD_*
        # install
        # functions:
        # DIF_NEWDEVICEWIZARD_PRESELECT
        # DIF_NEWDEVICEWIZARD_SELECT
        # DIF_NEWDEVICEWIZARD_PREANALYZE
        # DIF_NEWDEVICEWIZARD_POSTANALYZE
        # DIF_NEWDEVICEWIZARD_FINISHINSTALL
        class _SP_NEWDEVICEWIZARD_DATA(ctypes.Structure):
            pass


        _SP_NEWDEVICEWIZARD_DATA._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('Flags', DWORD), # presently unused - - must be zero.
            ('DynamicPages', HPROPSHEETPAGE * MAX_INSTALLWIZARD_DYNAPAGES),
            ('NumDynamicPages', DWORD),
            ('hwndWizardDlg', HWND),
        ]
        SP_NEWDEVICEWIZARD_DATA = _SP_NEWDEVICEWIZARD_DATA
        PSP_NEWDEVICEWIZARD_DATA = POINTER(_SP_NEWDEVICEWIZARD_DATA)

        # The same structure is also used for retrieval of property pages via
        # the

        # following install functions:
        # DIF_ADDPROPERTYPAGE_ADVANCED
        # DIF_ADDPROPERTYPAGE_BASIC
        # DIF_ADDREMOTEPROPERTYPAGE_ADVANCED
        SP_ADDPROPERTYPAGE_DATA = SP_NEWDEVICEWIZARD_DATA
        PSP_ADDPROPERTYPAGE_DATA = PSP_NEWDEVICEWIZARD_DATA

        # Structure corresponding to the DIF_TROUBLESHOOTER install function
        class _SP_TROUBLESHOOTER_PARAMS_A(ctypes.Structure):
            pass


        _SP_TROUBLESHOOTER_PARAMS_A._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('ChmFile', CHAR * MAX_PATH),
            ('HtmlTroubleShooter', CHAR * MAX_PATH),
        ]
        SP_TROUBLESHOOTER_PARAMS_A = _SP_TROUBLESHOOTER_PARAMS_A
        PSP_TROUBLESHOOTER_PARAMS_A = POINTER(_SP_TROUBLESHOOTER_PARAMS_A)


        class _SP_TROUBLESHOOTER_PARAMS_W(ctypes.Structure):
            pass


        _SP_TROUBLESHOOTER_PARAMS_W._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('ChmFile', WCHAR * MAX_PATH),
            ('HtmlTroubleShooter', WCHAR * MAX_PATH),
        ]
        SP_TROUBLESHOOTER_PARAMS_W = _SP_TROUBLESHOOTER_PARAMS_W
        PSP_TROUBLESHOOTER_PARAMS_W = POINTER(_SP_TROUBLESHOOTER_PARAMS_W)

        if defined(UNICODE):
            SP_TROUBLESHOOTER_PARAMS = SP_TROUBLESHOOTER_PARAMS_W
            PSP_TROUBLESHOOTER_PARAMS = PSP_TROUBLESHOOTER_PARAMS_W
        else:
            SP_TROUBLESHOOTER_PARAMS = SP_TROUBLESHOOTER_PARAMS_A
            PSP_TROUBLESHOOTER_PARAMS = PSP_TROUBLESHOOTER_PARAMS_A
        # END IF

        # Structure corresponding to the DIF_POWERMESSAGEWAKE install function
        class _SP_POWERMESSAGEWAKE_PARAMS_A(ctypes.Structure):
            pass


        _SP_POWERMESSAGEWAKE_PARAMS_A._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('PowerMessageWake', CHAR * LINE_LEN*2),
        ]
        SP_POWERMESSAGEWAKE_PARAMS_A = _SP_POWERMESSAGEWAKE_PARAMS_A
        PSP_POWERMESSAGEWAKE_PARAMS_A = POINTER(_SP_POWERMESSAGEWAKE_PARAMS_A)


        class _SP_POWERMESSAGEWAKE_PARAMS_W(ctypes.Structure):
            pass


        _SP_POWERMESSAGEWAKE_PARAMS_W._fields_ = [
            ('ClassInstallHeader', SP_CLASSINSTALL_HEADER),
            ('PowerMessageWake', WCHAR * LINE_LEN*2),
        ]
        SP_POWERMESSAGEWAKE_PARAMS_W = _SP_POWERMESSAGEWAKE_PARAMS_W
        PSP_POWERMESSAGEWAKE_PARAMS_W = POINTER(_SP_POWERMESSAGEWAKE_PARAMS_W)

        if defined(UNICODE):
            SP_POWERMESSAGEWAKE_PARAMS = SP_POWERMESSAGEWAKE_PARAMS_W
            PSP_POWERMESSAGEWAKE_PARAMS = PSP_POWERMESSAGEWAKE_PARAMS_W
        else:
            SP_POWERMESSAGEWAKE_PARAMS = SP_POWERMESSAGEWAKE_PARAMS_A
            PSP_POWERMESSAGEWAKE_PARAMS = PSP_POWERMESSAGEWAKE_PARAMS_A
        # END IF

        # Driver information structure
        # (member of a driver info list that may be associated
        # with a particular device instance, or (globally) with a device
        # information set)
        class _SP_DRVINFO_DATA_V2_A(ctypes.Structure):
            pass


        _SP_DRVINFO_DATA_V2_A._fields_ = [
            ('cbSize', DWORD),
            ('DriverType', DWORD),
            ('Reserved', ULONG_PTR),
            ('Description', CHAR * LINE_LEN),
            ('MfgName', CHAR * LINE_LEN),
            ('ProviderName', CHAR * LINE_LEN),
            ('DriverDate', FILETIME),
            ('DriverVersion', DWORDLONG),
        ]
        SP_DRVINFO_DATA_V2_A = _SP_DRVINFO_DATA_V2_A
        PSP_DRVINFO_DATA_V2_A = POINTER(_SP_DRVINFO_DATA_V2_A)


        class _SP_DRVINFO_DATA_V2_W(ctypes.Structure):
            pass


        _SP_DRVINFO_DATA_V2_W._fields_ = [
            ('cbSize', DWORD),
            ('DriverType', DWORD),
            ('Reserved', ULONG_PTR),
            ('Description', WCHAR * LINE_LEN),
            ('MfgName', WCHAR * LINE_LEN),
            ('ProviderName', WCHAR * LINE_LEN),
            ('DriverDate', FILETIME),
            ('DriverVersion', DWORDLONG),
        ]
        SP_DRVINFO_DATA_V2_W = _SP_DRVINFO_DATA_V2_W
        PSP_DRVINFO_DATA_V2_W = POINTER(_SP_DRVINFO_DATA_V2_W)

        # Version 1 of the SP_DRVINFO_DATA structures, used only for
        # compatibility
        # with Windows NT 4.0/Windows 95/98 SETUPAPI.DLL
        class _SP_DRVINFO_DATA_V1_A(ctypes.Structure):
            pass


        _SP_DRVINFO_DATA_V1_A._fields_ = [
            ('cbSize', DWORD),
            ('DriverType', DWORD),
            ('Reserved', ULONG_PTR),
            ('Description', CHAR * LINE_LEN),
            ('MfgName', CHAR * LINE_LEN),
            ('ProviderName', CHAR * LINE_LEN),
        ]
        SP_DRVINFO_DATA_V1_A = _SP_DRVINFO_DATA_V1_A
        PSP_DRVINFO_DATA_V1_A = POINTER(_SP_DRVINFO_DATA_V1_A)


        class _SP_DRVINFO_DATA_V1_W(ctypes.Structure):
            pass


        _SP_DRVINFO_DATA_V1_W._fields_ = [
            ('cbSize', DWORD),
            ('DriverType', DWORD),
            ('Reserved', ULONG_PTR),
            ('Description', WCHAR * LINE_LEN),
            ('MfgName', WCHAR * LINE_LEN),
            ('ProviderName', WCHAR * LINE_LEN),
        ]
        SP_DRVINFO_DATA_V1_W = _SP_DRVINFO_DATA_V1_W
        PSP_DRVINFO_DATA_V1_W = POINTER(_SP_DRVINFO_DATA_V1_W)

        if defined(UNICODE):
            SP_DRVINFO_DATA_V1 = SP_DRVINFO_DATA_V1_W
            PSP_DRVINFO_DATA_V1 = PSP_DRVINFO_DATA_V1_W
            SP_DRVINFO_DATA_V2 = SP_DRVINFO_DATA_V2_W
            PSP_DRVINFO_DATA_V2 = PSP_DRVINFO_DATA_V2_W
        else:
            SP_DRVINFO_DATA_V1 = SP_DRVINFO_DATA_V1_A
            PSP_DRVINFO_DATA_V1 = PSP_DRVINFO_DATA_V1_A
            SP_DRVINFO_DATA_V2 = SP_DRVINFO_DATA_V2_A
            PSP_DRVINFO_DATA_V2 = PSP_DRVINFO_DATA_V2_A
        # END IF
        if USE_SP_DRVINFO_DATA_V1 or (_SETUPAPI_VER < _WIN32_WINNT_WIN2K): #  use version 1 driver info data structure
            SP_DRVINFO_DATA_A = SP_DRVINFO_DATA_V1_A
            PSP_DRVINFO_DATA_A = PSP_DRVINFO_DATA_V1_A
            SP_DRVINFO_DATA_W = SP_DRVINFO_DATA_V1_W
            PSP_DRVINFO_DATA_W = PSP_DRVINFO_DATA_V1_W
            SP_DRVINFO_DATA = SP_DRVINFO_DATA_V1
            PSP_DRVINFO_DATA = PSP_DRVINFO_DATA_V1
        else: #  use version 2 driver info data structure
            SP_DRVINFO_DATA_A = SP_DRVINFO_DATA_V2_A
            PSP_DRVINFO_DATA_A = PSP_DRVINFO_DATA_V2_A
            SP_DRVINFO_DATA_W = SP_DRVINFO_DATA_V2_W
            PSP_DRVINFO_DATA_W = PSP_DRVINFO_DATA_V2_W
            SP_DRVINFO_DATA = SP_DRVINFO_DATA_V2
            PSP_DRVINFO_DATA = PSP_DRVINFO_DATA_V2
        # END IF   use current version of driver info data structure


        # Driver information details structure
        # (provides detailed information about a
        # particular driver information structure)
        class _SP_DRVINFO_DETAIL_DATA_A(ctypes.Structure):
            pass


        _SP_DRVINFO_DETAIL_DATA_A._fields_ = [
            ('cbSize', DWORD),
            ('InfDate', FILETIME),
            ('CompatIDsOffset', DWORD),
            ('CompatIDsLength', DWORD),
            ('Reserved', ULONG_PTR),
            ('SectionName', CHAR * LINE_LEN),
            ('InfFileName', CHAR * MAX_PATH),
            ('DrvDescription', CHAR * LINE_LEN),
            ('HardwareID', CHAR * ANYSIZE_ARRAY),
        ]
        SP_DRVINFO_DETAIL_DATA_A = _SP_DRVINFO_DETAIL_DATA_A
        PSP_DRVINFO_DETAIL_DATA_A = POINTER(_SP_DRVINFO_DETAIL_DATA_A)


        class _SP_DRVINFO_DETAIL_DATA_W(ctypes.Structure):
            pass


        _SP_DRVINFO_DETAIL_DATA_W._fields_ = [
            ('cbSize', DWORD),
            ('InfDate', FILETIME),
            ('CompatIDsOffset', DWORD),
            ('CompatIDsLength', DWORD),
            ('Reserved', ULONG_PTR),
            ('SectionName', WCHAR * LINE_LEN),
            ('InfFileName', WCHAR * MAX_PATH),
            ('DrvDescription', WCHAR * LINE_LEN),
            ('HardwareID', WCHAR * ANYSIZE_ARRAY),
        ]
        SP_DRVINFO_DETAIL_DATA_W = _SP_DRVINFO_DETAIL_DATA_W
        PSP_DRVINFO_DETAIL_DATA_W = POINTER(_SP_DRVINFO_DETAIL_DATA_W)

        if defined(UNICODE):
            SP_DRVINFO_DETAIL_DATA = SP_DRVINFO_DETAIL_DATA_W
            PSP_DRVINFO_DETAIL_DATA = PSP_DRVINFO_DETAIL_DATA_W
        else:
            SP_DRVINFO_DETAIL_DATA = SP_DRVINFO_DETAIL_DATA_A
            PSP_DRVINFO_DETAIL_DATA = PSP_DRVINFO_DETAIL_DATA_A
        # END IF

        # Driver installation parameters (associated with a particular driver
        # information element)
        class _SP_DRVINSTALL_PARAMS(ctypes.Structure):
            pass


        _SP_DRVINSTALL_PARAMS._fields_ = [
            ('cbSize', DWORD),
            ('Rank', DWORD),
            ('Flags', DWORD),
            ('PrivateData', DWORD_PTR),
            ('Reserved', DWORD),
        ]
        SP_DRVINSTALL_PARAMS = _SP_DRVINSTALL_PARAMS
        PSP_DRVINSTALL_PARAMS = POINTER(_SP_DRVINSTALL_PARAMS)

        # SP_DRVINSTALL_PARAMS.Flags values
        DNF_DUPDESC = 0x00000001
        DNF_OLDDRIVER = 0x00000002
        DNF_EXCLUDEFROMLIST = 0x00000004

        # displayed in any driver select dialogs.
        DNF_NODRIVER = 0x00000008

        # (e.g no mouse drv)
        DNF_LEGACYINF = 0x00000010
        DNF_CLASS_DRIVER = 0x00000020
        DNF_COMPATIBLE_DRIVER = 0x00000040
        DNF_INET_DRIVER = 0x00000080
        DNF_UNUSED1 = 0x00000100
        DNF_UNUSED2 = 0x00000200
        DNF_OLD_INET_DRIVER = 0x00000400

        # have access to it's source files. Never attempt to

        # install a driver with this flagnot

        # Note used on Windows Vista and Later.
        DNF_BAD_DRIVER = 0x00000800
        DNF_DUPPROVIDER = 0x00001000

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            DNF_INF_IS_SIGNED = 0x00002000
            DNF_OEM_F6_INF = 0x00004000
            DNF_DUPDRIVERVER = 0x00008000
            DNF_BASIC_DRIVER = 0x00010000

            # not be chosen if other INT drivers exist.
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WS03:
            DNF_AUTHENTICODE_SIGNED = 0x00020000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WS03

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            DNF_INSTALLEDDRIVER = 0x00040000
            DNF_ALWAYSEXCLUDEFROMLIST = 0x00080000

            # alternative platform either.
            DNF_INBOX_DRIVER = 0x00100000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN7:
            DNF_REQUESTADDITIONALSOFTWARE = 0x00200000

            # by this device
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WIN7
        DNF_UNUSED_22 = 0x00400000
        DNF_UNUSED_23 = 0x00800000
        DNF_UNUSED_24 = 0x01000000
        DNF_UNUSED_25 = 0x02000000
        DNF_UNUSED_26 = 0x04000000
        DNF_UNUSED_27 = 0x08000000
        DNF_UNUSED_28 = 0x10000000
        DNF_UNUSED_29 = 0x20000000
        DNF_UNUSED_30 = 0x40000000
        DNF_UNUSED_31 = 0x80000000

        # Rank values (the lower the Rank number, the better the Rank)

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            DRIVER_HARDWAREID_RANK = 0x00000FFF

            # this value is a gold
            # HardwareID match
            DRIVER_HARDWAREID_MASK = 0x80000FFF

            # from the Rank and the result is 0
            # then the Rank is a trusted HardwareID
            # match
            DRIVER_UNTRUSTED_RANK = 0x80000000

            # "untrusted" rank, meaning that
            # the INF was UINT.
            DRIVER_W9X_SUSPECT_RANK = 0xC0000000

            # or equal to this value, and lesser
            # than or equal to 0xFFFF is suspected
            # to be a Win9x - only driver, because
            # (a) it isn't INT, and (b) there
            # is no NT - specific decoration to
            # explicitly indicate that the INF
            # supports Windows NT/2000/XP
        else:
            DRIVER_HARDWAREID_RANK = 0x00000FFF

            # this value is a trusted
            # HardwareID match
            DRIVER_COMPATID_RANK = 0x00003FFF

            # this (and greater than
            # DRIVER_HARDWAREID_RANK) is a
            # trusted CompatibleID match
            DRIVER_UNTRUSTED_RANK = 0x00008000

            # "untrusted" rank, meaning that
            # the INF was UINT.
            DRIVER_UNTRUSTED_HARDWAREID_RANK = 0x00008FFF

            # this value (and greater than
            # or equal to DRIVER_UNTRUSTED_RANK)
            # is an untrusted HardwareID match
            DRIVER_UNTRUSTED_COMPATID_RANK = 0x0000BFFF

            # this value (and greater than
            # DRIVER_UNTRUSTED_HARDWAREID_RANK)
            # is an untrusted CompatibleID match
            DRIVER_W9X_SUSPECT_RANK = 0x0000C000

            # or equal to this value, and lesser
            # than or equal to 0xFFFF is suspected
            # to be a Win9x - only driver, because
            # (a) it isn't INT, and (b) there
            # is no NT - specific decoration to
            # explicitly indicate that the INF
            # supports Windows NT/2000/XP
            DRIVER_W9X_SUSPECT_HARDWAREID_RANK = 0x0000CFFF

            # (and greater than or equal to
            # DRIVER_W9X_SUSPECT_RANK) is a
            # hardware ID match suspected of being
            # only for Windows 9x platforms.
            DRIVER_W9X_SUSPECT_COMPATID_RANK = 0x0000FFFF

            # this (and greater than
            # DRIVER_W9X_SUSPECT_HARDWAREID_RANK)
            # is a compatible ID match suspected
            # of being only for Windows 9x
            # platforms.
        # END IF   _SETUPAPI_VER < _WIN32_WINNT_LONGHORN

        # Setup callback routine for comparing detection signatures
        # Define context structure handed to co - installers
        class _COINSTALLER_CONTEXT_DATA(ctypes.Structure):
            pass


        _COINSTALLER_CONTEXT_DATA._fields_ = [
            ('PostProcessing', BOOL),
            ('InstallResult', DWORD),
            ('PrivateData', PVOID),
        ]
        COINSTALLER_CONTEXT_DATA = _COINSTALLER_CONTEXT_DATA
        PCOINSTALLER_CONTEXT_DATA = POINTER(_COINSTALLER_CONTEXT_DATA)

        # Structure containing class image list information.
        class _SP_CLASSIMAGELIST_DATA(ctypes.Structure):
            pass


        _SP_CLASSIMAGELIST_DATA._fields_ = [
            ('cbSize', DWORD),
            ('ImageList', HIMAGELIST),
            ('Reserved', ULONG_PTR),
        ]
        SP_CLASSIMAGELIST_DATA = _SP_CLASSIMAGELIST_DATA
        PSP_CLASSIMAGELIST_DATA = POINTER(_SP_CLASSIMAGELIST_DATA)

        # Structure to be passed as first parameter (LPVOID lpv) to
        # ExtensionPropSheetPageProc
        # entry point in setupapi.dll or to "EnumPropPages32" or
        # "BasicProperties32" entry
        # points provided by class/device property page providers. Used to
        # retrieve a handle
        # (or, potentially, multiple handles) to property pages for a
        # specified property page type.
        class _SP_PROPSHEETPAGE_REQUEST(ctypes.Structure):
            pass


        _SP_PROPSHEETPAGE_REQUEST._fields_ = [
            ('cbSize', DWORD),
            ('PageRequested', DWORD),
            ('DeviceInfoSet', HDEVINFO),
            ('DeviceInfoData', PSP_DEVINFO_DATA),
        ]
        SP_PROPSHEETPAGE_REQUEST = _SP_PROPSHEETPAGE_REQUEST
        PSP_PROPSHEETPAGE_REQUEST = POINTER(_SP_PROPSHEETPAGE_REQUEST)

        # Property sheet codes used in SP_PROPSHEETPAGE_REQUEST.PageRequested
        SPPSR_SELECT_DEVICE_RESOURCES = 1
        SPPSR_ENUM_BASIC_DEVICE_PROPERTIES = 2
        SPPSR_ENUM_ADV_DEVICE_PROPERTIES = 3

        # Structure used with
        # SetupGetBackupInformation/SetupSetBackupInformation
        class _SP_BACKUP_QUEUE_PARAMS_V2_A(ctypes.Structure):
            pass


        _SP_BACKUP_QUEUE_PARAMS_V2_A._fields_ = [
            ('cbSize', DWORD), # size of structure
            ('FullInfPath', CHAR * MAX_PATH), # buffer to hold ANSI pathname of INF file
            ('FilenameOffset', INT), # offset in CHAR's of filename part (after '\')
            ('ReinstallInstance', CHAR * MAX_PATH), # Instance ID (if present)
        ]
        SP_BACKUP_QUEUE_PARAMS_V2_A = _SP_BACKUP_QUEUE_PARAMS_V2_A
        PSP_BACKUP_QUEUE_PARAMS_V2_A = POINTER(_SP_BACKUP_QUEUE_PARAMS_V2_A)
        class _SP_BACKUP_QUEUE_PARAMS_V2_W(ctypes.Structure):
            pass


        _SP_BACKUP_QUEUE_PARAMS_V2_W._fields_ = [
            ('cbSize', DWORD), # size of structure
            ('FullInfPath', WCHAR * MAX_PATH), # buffer to hold UNICODE pathname of INF file
            ('FilenameOffset', INT), # offset in WCHAR's of filename part (after '\')
            ('ReinstallInstance', WCHAR * MAX_PATH), # Instance ID (if present)
        ]
        SP_BACKUP_QUEUE_PARAMS_V2_W = _SP_BACKUP_QUEUE_PARAMS_V2_W
        PSP_BACKUP_QUEUE_PARAMS_V2_W = POINTER(_SP_BACKUP_QUEUE_PARAMS_V2_W)

        # Version 1 of the SP_BACKUP_QUEUE_PARAMS structures, used only for
        # compatibility
        # with Windows 2000/Windows 95/98/ME SETUPAPI.DLL
        class _SP_BACKUP_QUEUE_PARAMS_V1_A(ctypes.Structure):
            pass


        _SP_BACKUP_QUEUE_PARAMS_V1_A._fields_ = [
            ('cbSize', DWORD), # size of structure
            ('FullInfPath', CHAR * MAX_PATH), # buffer to hold ANSI pathname of INF file
            ('FilenameOffset', INT), # offset in CHAR's of filename part (after '\')
        ]
        SP_BACKUP_QUEUE_PARAMS_V1_A = _SP_BACKUP_QUEUE_PARAMS_V1_A
        PSP_BACKUP_QUEUE_PARAMS_V1_A = POINTER(_SP_BACKUP_QUEUE_PARAMS_V1_A)


        class _SP_BACKUP_QUEUE_PARAMS_V1_W(ctypes.Structure):
            pass


        _SP_BACKUP_QUEUE_PARAMS_V1_W._fields_ = [
            ('cbSize', DWORD), # size of structure
            ('FullInfPath', WCHAR * MAX_PATH), # buffer to hold UNICODE pathname of INF file
            ('FilenameOffset', INT), # offset in WCHAR's of filename part (after '\')
        ]
        SP_BACKUP_QUEUE_PARAMS_V1_W = _SP_BACKUP_QUEUE_PARAMS_V1_W
        PSP_BACKUP_QUEUE_PARAMS_V1_W = POINTER(_SP_BACKUP_QUEUE_PARAMS_V1_W)

        if defined(UNICODE):
            SP_BACKUP_QUEUE_PARAMS_V1 = SP_BACKUP_QUEUE_PARAMS_V1_W
            PSP_BACKUP_QUEUE_PARAMS_V1 = PSP_BACKUP_QUEUE_PARAMS_V1_W
            SP_BACKUP_QUEUE_PARAMS_V2 = SP_BACKUP_QUEUE_PARAMS_V2_W
            PSP_BACKUP_QUEUE_PARAMS_V2 = PSP_BACKUP_QUEUE_PARAMS_V2_W
        else:
            SP_BACKUP_QUEUE_PARAMS_V1 = SP_BACKUP_QUEUE_PARAMS_V1_A
            PSP_BACKUP_QUEUE_PARAMS_V1 = PSP_BACKUP_QUEUE_PARAMS_V1_A
            SP_BACKUP_QUEUE_PARAMS_V2 = SP_BACKUP_QUEUE_PARAMS_V2_A
            PSP_BACKUP_QUEUE_PARAMS_V2 = PSP_BACKUP_QUEUE_PARAMS_V2_A
        # END IF
        if USE_SP_BACKUP_QUEUE_PARAMS_V1 or (_SETUPAPI_VER < _WIN32_WINNT_WINXP): # use version 1 driver info data structure
            SP_BACKUP_QUEUE_PARAMS_A = SP_BACKUP_QUEUE_PARAMS_V1_A
            PSP_BACKUP_QUEUE_PARAMS_A = PSP_BACKUP_QUEUE_PARAMS_V1_A
            SP_BACKUP_QUEUE_PARAMS_W = SP_BACKUP_QUEUE_PARAMS_V1_W
            PSP_BACKUP_QUEUE_PARAMS_W = PSP_BACKUP_QUEUE_PARAMS_V1_W
            SP_BACKUP_QUEUE_PARAMS = SP_BACKUP_QUEUE_PARAMS_V1
            PSP_BACKUP_QUEUE_PARAMS = PSP_BACKUP_QUEUE_PARAMS_V1
        else: # use version 2 driver info data structure
            SP_BACKUP_QUEUE_PARAMS_A = SP_BACKUP_QUEUE_PARAMS_V2_A
            PSP_BACKUP_QUEUE_PARAMS_A = PSP_BACKUP_QUEUE_PARAMS_V2_A
            SP_BACKUP_QUEUE_PARAMS_W = SP_BACKUP_QUEUE_PARAMS_V2_W
            PSP_BACKUP_QUEUE_PARAMS_W = PSP_BACKUP_QUEUE_PARAMS_V2_W
            SP_BACKUP_QUEUE_PARAMS = SP_BACKUP_QUEUE_PARAMS_V2
            PSP_BACKUP_QUEUE_PARAMS = PSP_BACKUP_QUEUE_PARAMS_V2
        # END IF   use current version of driver info data structure

        if not defined(_SPAPI_ERRORS):
            _SPAPI_ERRORS = 1

            # Setupapi - specific error codes

            # Inf parse outcomes
            ERROR_EXPECTED_SECTION_NAME = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0
            )
            ERROR_BAD_SECTION_NAME_LINE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                1
            )
            ERROR_SECTION_NAME_TOO_LONG = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                2
            )
            ERROR_GENERAL_SYNTAX = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                3
            )

            # Inf runtime errors
            ERROR_WRONG_INF_STYLE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x100
            )
            ERROR_SECTION_NOT_FOUND = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x101
            )
            ERROR_LINE_NOT_FOUND = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x102
            )
            ERROR_NO_BACKUP = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x103
            )

            # Device Installer/other errors
            ERROR_NO_ASSOCIATED_CLASS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x200
            )
            ERROR_CLASS_MISMATCH = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x201
            )
            ERROR_DUPLICATE_FOUND = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x202
            )
            ERROR_NO_DRIVER_SELECTED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x203
            )
            ERROR_KEY_DOES_NOT_EXIST = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x204
            )
            ERROR_INVALID_DEVINST_NAME = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x205
            )
            ERROR_INVALID_CLASS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x206
            )
            ERROR_DEVINST_ALREADY_EXISTS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x207
            )
            ERROR_DEVINFO_NOT_REGISTERED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x208
            )
            ERROR_INVALID_REG_PROPERTY = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x209
            )
            ERROR_NO_INF = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20A
            )
            ERROR_NO_SUCH_DEVINST = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20B
            )
            ERROR_CANT_LOAD_CLASS_ICON = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20C
            )
            ERROR_INVALID_CLASS_INSTALLER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20D
            )
            ERROR_DI_DO_DEFAULT = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20E
            )
            ERROR_DI_NOFILECOPY = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x20F
            )
            ERROR_INVALID_HWPROFILE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x210
            )
            ERROR_NO_DEVICE_SELECTED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x211
            )
            ERROR_DEVINFO_LIST_LOCKED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x212
            )
            ERROR_DEVINFO_DATA_LOCKED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x213
            )
            ERROR_DI_BAD_PATH = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x214
            )
            ERROR_NO_CLASSINSTALL_PARAMS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x215
            )
            ERROR_FILEQUEUE_LOCKED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x216
            )
            ERROR_BAD_SERVICE_INSTALLSECT = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x217
            )
            ERROR_NO_CLASS_DRIVER_LIST = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x218
            )
            ERROR_NO_ASSOCIATED_SERVICE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x219
            )
            ERROR_NO_DEFAULT_DEVICE_INTERFACE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21A
            )
            ERROR_DEVICE_INTERFACE_ACTIVE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21B
            )
            ERROR_DEVICE_INTERFACE_REMOVED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21C
            )
            ERROR_BAD_INTERFACE_INSTALLSECT = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21D
            )
            ERROR_NO_SUCH_INTERFACE_CLASS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21E
            )
            ERROR_INVALID_REFERENCE_STRING = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x21F
            )
            ERROR_INVALID_MACHINENAME = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x220
            )
            ERROR_REMOTE_COMM_FAILURE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x221
            )
            ERROR_MACHINE_UNAVAILABLE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x222
            )
            ERROR_NO_CONFIGMGR_SERVICES = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x223
            )
            ERROR_INVALID_PROPPAGE_PROVIDER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x224
            )
            ERROR_NO_SUCH_DEVICE_INTERFACE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x225
            )
            ERROR_DI_POSTPROCESSING_REQUIRED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x226
            )
            ERROR_INVALID_COINSTALLER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x227
            )
            ERROR_NO_COMPAT_DRIVERS = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x228
            )
            ERROR_NO_DEVICE_ICON = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x229
            )
            ERROR_INVALID_INF_LOGCONFIG = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22A
            )
            ERROR_DI_DONT_INSTALL = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22B
            )
            ERROR_INVALID_FILTER_DRIVER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22C
            )
            ERROR_NON_WINDOWS_NT_DRIVER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22D
            )
            ERROR_NON_WINDOWS_DRIVER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22E
            )
            ERROR_NO_CATALOG_FOR_OEM_INF = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x22F
            )
            ERROR_DEVINSTALL_QUEUE_NONNATIVE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x230
            )
            ERROR_NOT_DISABLEABLE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x231
            )
            ERROR_CANT_REMOVE_DEVINST = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x232
            )
            ERROR_INVALID_TARGET = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x233
            )
            ERROR_DRIVER_NONNATIVE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x234
            )
            ERROR_IN_WOW64 = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x235
            )
            ERROR_SET_SYSTEM_RESTORE_POINT = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x236
            )
            ERROR_SCE_DISABLED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x238
            )
            ERROR_UNKNOWN_EXCEPTION = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x239
            )
            ERROR_PNP_REGISTRY_ERROR = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23A
            )
            ERROR_REMOTE_REQUEST_UNSUPPORTED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23B
            )
            ERROR_NOT_AN_INSTALLED_OEM_INF = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23C
            )
            ERROR_INF_IN_USE_BY_DEVICES = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23D
            )
            ERROR_DI_FUNCTION_OBSOLETE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23E
            )
            ERROR_NO_AUTHENTICODE_CATALOG = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x23F
            )
            ERROR_AUTHENTICODE_DISALLOWED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x240
            )
            ERROR_AUTHENTICODE_TRUSTED_PUBLISHER = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x241
            )
            ERROR_AUTHENTICODE_TRUST_NOT_ESTABLISHED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x242
            )
            ERROR_AUTHENTICODE_PUBLISHER_NOT_TRUSTED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x243
            )
            ERROR_SIGNATURE_OSATTRIBUTE_MISMATCH = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x244
            )
            ERROR_ONLY_VALIDATE_VIA_AUTHENTICODE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x245
            )
            ERROR_DEVICE_INSTALLER_NOT_READY = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x246
            )
            ERROR_DRIVER_STORE_ADD_FAILED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x247
            )
            ERROR_DEVICE_INSTALL_BLOCKED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x248
            )
            ERROR_DRIVER_INSTALL_BLOCKED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x249
            )
            ERROR_WRONG_INF_TYPE = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x24A
            )
            ERROR_FILE_HASH_NOT_IN_CATALOG = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x24B
            )
            ERROR_DRIVER_STORE_DELETE_FAILED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x24C
            )

            # Setupapi exception codes
            ERROR_UNRECOVERABLE_STACK_OVERFLOW = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x300
            )
            EXCEPTION_SPAPI_UNRECOVERABLE_STACK_OVERFLOW = (
                ERROR_UNRECOVERABLE_STACK_OVERFLOW
            )

            # Backward compatibility - - do not use.
            ERROR_NO_DEFAULT_INTERFACE_DEVICE = (
                ERROR_NO_DEFAULT_DEVICE_INTERFACE
            )
            ERROR_INTERFACE_DEVICE_ACTIVE = ERROR_DEVICE_INTERFACE_ACTIVE
            ERROR_INTERFACE_DEVICE_REMOVED = ERROR_DEVICE_INTERFACE_REMOVED
            ERROR_NO_SUCH_INTERFACE_DEVICE = ERROR_NO_SUCH_DEVICE_INTERFACE

            # Win9x migration DLL error code
            ERROR_NOT_INSTALLED = (
                APPLICATION_ERROR_MASK |
                ERROR_SEVERITY_ERROR |
                0x1000
            )
        # END IF   _SPAPI_ERRORS

        setupapi = ctypes.windll.setupapi
        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetInfInformationA(
        # _In_ LPCVOID InfSpec,
        # _In_ DWORD SearchControl,
        # _Out_writes_bytes_to_opt_(ReturnBufferSize, *RequiredSize) PSP_INF_INFORMATION ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetInfInformationA = setupapi.SetupGetInfInformationA
        SetupGetInfInformationA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetInfInformationW(
        # _In_ LPCVOID InfSpec,
        # _In_ DWORD SearchControl,
        # _Out_writes_bytes_to_opt_(ReturnBufferSize, *RequiredSize) PSP_INF_INFORMATION ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetInfInformationW = setupapi.SetupGetInfInformationW
        SetupGetInfInformationW.restype = BOOL

        # SearchControl flags for SetupGetInfInformation
        INFINFO_INF_SPEC_IS_HINF = 1
        INFINFO_INF_NAME_IS_ABSOLUTE = 2
        INFINFO_DEFAULT_SEARCH = 3
        INFINFO_REVERSE_DEFAULT_SEARCH = 4
        INFINFO_INF_PATH_LIST_SEARCH = 5

        if defined(UNICODE):
            SetupGetInfInformation = SetupGetInfInformationW
        else:
            SetupGetInfInformation = SetupGetInfInformationA
        # END IF


        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfFileInformationA(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryInfFileInformationA = setupapi.SetupQueryInfFileInformationA
        SetupQueryInfFileInformationA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfFileInformationW(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryInfFileInformationW = setupapi.SetupQueryInfFileInformationW
        SetupQueryInfFileInformationW.restype = BOOL

        if defined(UNICODE):
            SetupQueryInfFileInformation = SetupQueryInfFileInformationW
        else:
            SetupQueryInfFileInformation = SetupQueryInfFileInformationA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfOriginalFileInformationA(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
        # _Out_ PSP_ORIGINAL_FILE_INFO_A OriginalFileInfo
        # );
        SetupQueryInfOriginalFileInformationA = setupapi.SetupQueryInfOriginalFileInformationA
        SetupQueryInfOriginalFileInformationA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfOriginalFileInformationW(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
        # _Out_ PSP_ORIGINAL_FILE_INFO_W OriginalFileInfo
        # );
        SetupQueryInfOriginalFileInformationW = setupapi.SetupQueryInfOriginalFileInformationW
        SetupQueryInfOriginalFileInformationW.restype = BOOL

        if defined(UNICODE):
            SetupQueryInfOriginalFileInformation = (
                SetupQueryInfOriginalFileInformationW
            )
        else:
            SetupQueryInfOriginalFileInformation = (
                SetupQueryInfOriginalFileInformationA
            )
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfVersionInformationA(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _In_opt_ PCSTR Key,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryInfVersionInformationA = setupapi.SetupQueryInfVersionInformationA
        SetupQueryInfVersionInformationA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryInfVersionInformationW(
        # _In_ PSP_INF_INFORMATION InfInformation,
        # _In_ UINT InfIndex,
        # _In_opt_ PCWSTR Key,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryInfVersionInformationW = setupapi.SetupQueryInfVersionInformationW
        SetupQueryInfVersionInformationW.restype = BOOL

        if defined(UNICODE):
            SetupQueryInfVersionInformation = SetupQueryInfVersionInformationW
        else:
            SetupQueryInfVersionInformation = SetupQueryInfVersionInformationA
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetInfDriverStoreLocationA(
            # _In_ PCSTR FileName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _In_opt_ PCSTR LocaleName,
            # _Out_writes_(ReturnBufferSize) PSTR ReturnBuffer,
            # _In_ DWORD ReturnBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupGetInfDriverStoreLocationA = setupapi.SetupGetInfDriverStoreLocationA
            SetupGetInfDriverStoreLocationA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetInfDriverStoreLocationW(
            # _In_ PCWSTR FileName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _In_opt_ PCWSTR LocaleName,
            # _Out_writes_(ReturnBufferSize) PWSTR ReturnBuffer,
            # _In_ DWORD ReturnBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupGetInfDriverStoreLocationW = setupapi.SetupGetInfDriverStoreLocationW
            SetupGetInfDriverStoreLocationW.restype = BOOL
            if defined(UNICODE):
                SetupGetInfDriverStoreLocation = (
                    SetupGetInfDriverStoreLocationW
                )
            else:
                SetupGetInfDriverStoreLocation = (
                    SetupGetInfDriverStoreLocationA
                )
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetInfPublishedNameA(
            # _In_ PCSTR DriverStoreLocation,
            # _Out_writes_(ReturnBufferSize) PSTR ReturnBuffer,
            # _In_ DWORD ReturnBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupGetInfPublishedNameA = setupapi.SetupGetInfPublishedNameA
            SetupGetInfPublishedNameA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetInfPublishedNameW(
            # _In_ PCWSTR DriverStoreLocation,
            # _Out_writes_(ReturnBufferSize) PWSTR ReturnBuffer,
            # _In_ DWORD ReturnBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupGetInfPublishedNameW = setupapi.SetupGetInfPublishedNameW
            SetupGetInfPublishedNameW.restype = BOOL

            if defined(UNICODE):
                SetupGetInfPublishedName = SetupGetInfPublishedNameW
            else:
                SetupGetInfPublishedName = SetupGetInfPublishedNameA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetInfFileListA(
        # _In_opt_ PCSTR DirectoryPath,
        # _In_ DWORD InfStyle,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetInfFileListA = setupapi.SetupGetInfFileListA
        SetupGetInfFileListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetInfFileListW(
        # _In_opt_ PCWSTR DirectoryPath,
        # _In_ DWORD InfStyle,
        # _Out_writes_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetInfFileListW = setupapi.SetupGetInfFileListW
        SetupGetInfFileListW.restype = BOOL

        if defined(UNICODE):
            SetupGetInfFileList = SetupGetInfFileListW
        else:
            SetupGetInfFileList = SetupGetInfFileListA
        # END IF
        # WINSETUPAPI
        # HINF
        # WINAPI
        # SetupOpenInfFileW(
        # _In_ PCWSTR FileName,
        # _In_opt_ PCWSTR InfClass,
        # _In_ DWORD InfStyle,
        # _Out_opt_ PUINT ErrorLine
        # );
        SetupOpenInfFileW = setupapi.SetupOpenInfFileW
        SetupOpenInfFileW.restype = HINF
        # WINSETUPAPI
        # HINF
        # WINAPI
        # SetupOpenInfFileA(
        # _In_ PCSTR FileName,
        # _In_opt_ PCSTR InfClass,
        # _In_ DWORD InfStyle,
        # _Out_opt_ PUINT ErrorLine
        # );
        SetupOpenInfFileA = setupapi.SetupOpenInfFileA
        SetupOpenInfFileA.restype = HINF

        if defined(UNICODE):
            SetupOpenInfFile = SetupOpenInfFileW
        else:
            SetupOpenInfFile = SetupOpenInfFileA
        # END IF
        # WINSETUPAPI
        # HINF
        # WINAPI
        # SetupOpenMasterInf(
        # VOID
        # );
        SetupOpenMasterInf = setupapi.SetupOpenMasterInf
        SetupOpenMasterInf.restype = HINF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupOpenAppendInfFileW(
        # _In_opt_ PCWSTR FileName,
        # _In_ HINF InfHandle,
        # _Out_opt_ PUINT ErrorLine
        # );
        SetupOpenAppendInfFileW = setupapi.SetupOpenAppendInfFileW
        SetupOpenAppendInfFileW.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupOpenAppendInfFileA(
        # _In_opt_ PCSTR FileName,
        # _In_ HINF InfHandle,
        # _Out_opt_ PUINT ErrorLine
        # );
        SetupOpenAppendInfFileA = setupapi.SetupOpenAppendInfFileA
        SetupOpenAppendInfFileA.restype = BOOL

        if defined(UNICODE):
            SetupOpenAppendInfFile = SetupOpenAppendInfFileW
        else:
            SetupOpenAppendInfFile = SetupOpenAppendInfFileA
        # END IF
        # WINSETUPAPI
        # VOID
        # WINAPI
        # SetupCloseInfFile(
        # _In_ HINF InfHandle
        # );
        SetupCloseInfFile = setupapi.SetupCloseInfFile
        SetupCloseInfFile.restype = VOID
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFindFirstLineA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR Section,
        # _In_opt_ PCSTR Key,
        # _Out_ PINFCONTEXT Context
        # );
        SetupFindFirstLineA = setupapi.SetupFindFirstLineA
        SetupFindFirstLineA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFindFirstLineW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR Section,
        # _In_opt_ PCWSTR Key,
        # _Out_ PINFCONTEXT Context
        # );
        SetupFindFirstLineW = setupapi.SetupFindFirstLineW
        SetupFindFirstLineW.restype = BOOL

        if defined(UNICODE):
            SetupFindFirstLine = SetupFindFirstLineW
        else:
            SetupFindFirstLine = SetupFindFirstLineA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFindNextLine(
        # _In_ PINFCONTEXT ContextIn,
        # _Out_ PINFCONTEXT ContextOut
        # );
        SetupFindNextLine = setupapi.SetupFindNextLine
        SetupFindNextLine.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFindNextMatchLineA(
        # _In_ PINFCONTEXT ContextIn,
        # _In_opt_ PCSTR Key,
        # _Out_ PINFCONTEXT ContextOut
        # );
        SetupFindNextMatchLineA = setupapi.SetupFindNextMatchLineA
        SetupFindNextMatchLineA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFindNextMatchLineW(
        # _In_ PINFCONTEXT ContextIn,
        # _In_opt_ PCWSTR Key,
        # _Out_ PINFCONTEXT ContextOut
        # );
        SetupFindNextMatchLineW = setupapi.SetupFindNextMatchLineW
        SetupFindNextMatchLineW.restype = BOOL

        if defined(UNICODE):
            SetupFindNextMatchLine = SetupFindNextMatchLineW
        else:
            SetupFindNextMatchLine = SetupFindNextMatchLineA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetLineByIndexA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR Section,
        # _In_ DWORD Index,
        # _Out_ PINFCONTEXT Context
        # );
        SetupGetLineByIndexA = setupapi.SetupGetLineByIndexA
        SetupGetLineByIndexA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetLineByIndexW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR Section,
        # _In_ DWORD Index,
        # _Out_ PINFCONTEXT Context
        # );
        SetupGetLineByIndexW = setupapi.SetupGetLineByIndexW
        SetupGetLineByIndexW.restype = BOOL

        if defined(UNICODE):
            SetupGetLineByIndex = SetupGetLineByIndexW
        else:
            SetupGetLineByIndex = SetupGetLineByIndexA
        # END IF
        # WINSETUPAPI
        # LONG
        # WINAPI
        # SetupGetLineCountA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR Section
        # );
        SetupGetLineCountA = setupapi.SetupGetLineCountA
        SetupGetLineCountA.restype = LONG
        # WINSETUPAPI
        # LONG
        # WINAPI
        # SetupGetLineCountW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR Section
        # );
        SetupGetLineCountW = setupapi.SetupGetLineCountW
        SetupGetLineCountW.restype = LONG

        if defined(UNICODE):
            SetupGetLineCount = SetupGetLineCountW
        else:
            SetupGetLineCount = SetupGetLineCountA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetLineTextA(
        # _In_opt_ PINFCONTEXT Context,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCSTR Section,
        # _In_opt_ PCSTR Key,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetLineTextA = setupapi.SetupGetLineTextA
        SetupGetLineTextA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetLineTextW(
        # _In_opt_ PINFCONTEXT Context,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCWSTR Section,
        # _In_opt_ PCWSTR Key,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetLineTextW = setupapi.SetupGetLineTextW
        SetupGetLineTextW.restype = BOOL

        if defined(UNICODE):
            SetupGetLineText = SetupGetLineTextW
        else:
            SetupGetLineText = SetupGetLineTextA
        # END IF
        # WINSETUPAPI
        # DWORD
        # WINAPI
        # SetupGetFieldCount(
        # _In_ PINFCONTEXT Context
        # );
        SetupGetFieldCount = setupapi.SetupGetFieldCount
        SetupGetFieldCount.restype = DWORD
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetStringFieldA(
        # _In_ PINFCONTEXT Context,
        # _In_ DWORD FieldIndex,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetStringFieldA = setupapi.SetupGetStringFieldA
        SetupGetStringFieldA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetStringFieldW(
        # _In_ PINFCONTEXT Context,
        # _In_ DWORD FieldIndex,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetStringFieldW = setupapi.SetupGetStringFieldW
        SetupGetStringFieldW.restype = BOOL

        if defined(UNICODE):
            SetupGetStringField = SetupGetStringFieldW
        else:
            SetupGetStringField = SetupGetStringFieldA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetIntField(
        # _In_ PINFCONTEXT Context,
        # _In_ DWORD FieldIndex,
        # _Out_ PINT IntegerValue
        # );
        SetupGetIntField = setupapi.SetupGetIntField
        SetupGetIntField.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetMultiSzFieldA(
        # _In_ PINFCONTEXT Context,
        # _In_ DWORD FieldIndex,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ LPDWORD RequiredSize
        # );
        SetupGetMultiSzFieldA = setupapi.SetupGetMultiSzFieldA
        SetupGetMultiSzFieldA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetMultiSzFieldW(
        # _In_ PINFCONTEXT Context,
        # _In_ DWORD FieldIndex,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ LPDWORD RequiredSize
        # );
        SetupGetMultiSzFieldW = setupapi.SetupGetMultiSzFieldW
        SetupGetMultiSzFieldW.restype = BOOL

        if defined(UNICODE):
            SetupGetMultiSzField = SetupGetMultiSzFieldW
        else:
            SetupGetMultiSzField = SetupGetMultiSzFieldA
        # END IF

        # SetupGetFileCompressionInfo is depreciated

        # use SetupGetFileCompressionInfoEx instead

        # ActualSourceFileName returned by SetupGetFileCompressionInfo

        # must be freed by the export setupapinot MyFree (NT4+ Win95 + )

        # or LocalFree (Win2k + )
        # WINSETUPAPI
        # DWORD
        # WINAPI
        # SetupGetFileCompressionInfoA(
        # _In_ PCSTR SourceFileName,
        # _Out_ PSTR *ActualSourceFileName,
        # _Out_ PDWORD SourceFileSize,
        # _Out_ PDWORD TargetFileSize,
        # _Out_ PUINT CompressionType
        # );
        SetupGetFileCompressionInfoA = setupapi.SetupGetFileCompressionInfoA
        SetupGetFileCompressionInfoA.restype = DWORD
        # WINSETUPAPI
        # DWORD
        # WINAPI
        # SetupGetFileCompressionInfoW(
        # _In_ PCWSTR SourceFileName,
        # _Out_ PWSTR *ActualSourceFileName,
        # _Out_ PDWORD SourceFileSize,
        # _Out_ PDWORD TargetFileSize,
        # _Out_ PUINT CompressionType
        # );
        SetupGetFileCompressionInfoW = setupapi.SetupGetFileCompressionInfoW
        SetupGetFileCompressionInfoW.restype = DWORD

        if defined(UNICODE):
            SetupGetFileCompressionInfo = SetupGetFileCompressionInfoW
        else:
            SetupGetFileCompressionInfo = SetupGetFileCompressionInfoA
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # SetupGetFileCompressionInfoEx is the preferred API over

            # SetupGetFileCompressionInfo. It follows the normal

            # conventions of returning BOOL and writing to user - supplied

            # buffer.
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetFileCompressionInfoExA(
            # _In_ PCSTR SourceFileName,
            # _In_reads_opt_(ActualSourceFileNameBufferLen) PSTR ActualSourceFileNameBuffer,
            # _In_ DWORD ActualSourceFileNameBufferLen,
            # _Out_opt_ PDWORD RequiredBufferLen,
            # _Out_ PDWORD SourceFileSize,
            # _Out_ PDWORD TargetFileSize,
            # _Out_ PUINT CompressionType
            # );
            SetupGetFileCompressionInfoExA = setupapi.SetupGetFileCompressionInfoExA
            SetupGetFileCompressionInfoExA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetFileCompressionInfoExW(
            # _In_ PCWSTR SourceFileName,
            # _In_reads_opt_(ActualSourceFileNameBufferLen) PWSTR ActualSourceFileNameBuffer,
            # _In_ DWORD ActualSourceFileNameBufferLen,
            # _Out_opt_ PDWORD RequiredBufferLen,
            # _Out_ PDWORD SourceFileSize,
            # _Out_ PDWORD TargetFileSize,
            # _Out_ PUINT CompressionType
            # );
            SetupGetFileCompressionInfoExW = setupapi.SetupGetFileCompressionInfoExW
            SetupGetFileCompressionInfoExW.restype = BOOL
            if defined(UNICODE):
                SetupGetFileCompressionInfoEx = SetupGetFileCompressionInfoExW
            else:
                SetupGetFileCompressionInfoEx = SetupGetFileCompressionInfoExA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Compression types
        FILE_COMPRESSION_NONE = 0
        FILE_COMPRESSION_WINLZA = 1
        FILE_COMPRESSION_MSZIP = 2
        FILE_COMPRESSION_NTCAB = 3
        # WINSETUPAPI
        # DWORD
        # WINAPI
        # SetupDecompressOrCopyFileA(
        # _In_ PCSTR SourceFileName,
        # _In_ PCSTR TargetFileName,
        # _In_opt_ PUINT CompressionType
        # );
        SetupDecompressOrCopyFileA = setupapi.SetupDecompressOrCopyFileA
        SetupDecompressOrCopyFileA.restype = DWORD
        # WINSETUPAPI
        # DWORD
        # WINAPI
        # SetupDecompressOrCopyFileW(
        # _In_ PCWSTR SourceFileName,
        # _In_ PCWSTR TargetFileName,
        # _In_opt_ PUINT CompressionType
        # );
        SetupDecompressOrCopyFileW = setupapi.SetupDecompressOrCopyFileW
        SetupDecompressOrCopyFileW.restype = DWORD

        if defined(UNICODE):
            SetupDecompressOrCopyFile = SetupDecompressOrCopyFileW
        else:
            SetupDecompressOrCopyFile = SetupDecompressOrCopyFileA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceFileLocationA(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCSTR FileName,
        # _Out_ PUINT SourceId,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetSourceFileLocationA = setupapi.SetupGetSourceFileLocationA
        SetupGetSourceFileLocationA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceFileLocationW(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCWSTR FileName,
        # _Out_ PUINT SourceId,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetSourceFileLocationW = setupapi.SetupGetSourceFileLocationW
        SetupGetSourceFileLocationW.restype = BOOL

        if defined(UNICODE):
            SetupGetSourceFileLocation = SetupGetSourceFileLocationW
        else:
            SetupGetSourceFileLocation = SetupGetSourceFileLocationA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceFileSizeA(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCSTR FileName,
        # _In_opt_ PCSTR Section,
        # _Out_ PDWORD FileSize,
        # _In_ UINT RoundingFactor
        # );
        SetupGetSourceFileSizeA = setupapi.SetupGetSourceFileSizeA
        SetupGetSourceFileSizeA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceFileSizeW(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCWSTR FileName,
        # _In_opt_ PCWSTR Section,
        # _Out_ PDWORD FileSize,
        # _In_ UINT RoundingFactor
        # );
        SetupGetSourceFileSizeW = setupapi.SetupGetSourceFileSizeW
        SetupGetSourceFileSizeW.restype = BOOL

        if defined(UNICODE):
            SetupGetSourceFileSize = SetupGetSourceFileSizeW
        else:
            SetupGetSourceFileSize = SetupGetSourceFileSizeA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetTargetPathA(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCSTR Section,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetTargetPathA = setupapi.SetupGetTargetPathA
        SetupGetTargetPathA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetTargetPathW(
        # _In_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCWSTR Section,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetTargetPathW = setupapi.SetupGetTargetPathW
        SetupGetTargetPathW.restype = BOOL

        if defined(UNICODE):
            SetupGetTargetPath = SetupGetTargetPathW
        else:
            SetupGetTargetPath = SetupGetTargetPathA
        # END IF

        # Define flags for SourceList APIs.
        SRCLIST_TEMPORARY = 0x00000001
        SRCLIST_NOBROWSE = 0x00000002
        SRCLIST_SYSTEM = 0x00000010
        SRCLIST_USER = 0x00000020
        SRCLIST_SYSIFADMIN = 0x00000040
        SRCLIST_SUBDIRS = 0x00000100
        SRCLIST_APPEND = 0x00000200
        SRCLIST_NOSTRIPPLATFORM = 0x00000400
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetSourceListA(
        # _In_ DWORD Flags,
        # _In_reads_(SourceCount) PCSTR *SourceList,
        # _In_ UINT SourceCount
        # );
        SetupSetSourceListA = setupapi.SetupSetSourceListA
        SetupSetSourceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetSourceListW(
        # _In_ DWORD Flags,
        # _In_reads_(SourceCount) PCWSTR *SourceList,
        # _In_ UINT SourceCount
        # );
        SetupSetSourceListW = setupapi.SetupSetSourceListW
        SetupSetSourceListW.restype = BOOL

        if defined(UNICODE):
            SetupSetSourceList = SetupSetSourceListW
        else:
            SetupSetSourceList = SetupSetSourceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCancelTemporarySourceList(
        # VOID
        # );
        SetupCancelTemporarySourceList = setupapi.SetupCancelTemporarySourceList
        SetupCancelTemporarySourceList.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddToSourceListA(
        # _In_ DWORD Flags,
        # _In_ PCSTR Source
        # );
        SetupAddToSourceListA = setupapi.SetupAddToSourceListA
        SetupAddToSourceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddToSourceListW(
        # _In_ DWORD Flags,
        # _In_ PCWSTR Source
        # );
        SetupAddToSourceListW = setupapi.SetupAddToSourceListW
        SetupAddToSourceListW.restype = BOOL

        if defined(UNICODE):
            SetupAddToSourceList = SetupAddToSourceListW
        else:
            SetupAddToSourceList = SetupAddToSourceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFromSourceListA(
        # _In_ DWORD Flags,
        # _In_ PCSTR Source
        # );
        SetupRemoveFromSourceListA = setupapi.SetupRemoveFromSourceListA
        SetupRemoveFromSourceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFromSourceListW(
        # _In_ DWORD Flags,
        # _In_ PCWSTR Source
        # );
        SetupRemoveFromSourceListW = setupapi.SetupRemoveFromSourceListW
        SetupRemoveFromSourceListW.restype = BOOL

        if defined(UNICODE):
            SetupRemoveFromSourceList = SetupRemoveFromSourceListW
        else:
            SetupRemoveFromSourceList = SetupRemoveFromSourceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQuerySourceListA(
        # _In_ DWORD Flags,
        # _Outptr_result_buffer_(*Count) PCSTR **List,
        # _Out_ PUINT Count
        # );
        SetupQuerySourceListA = setupapi.SetupQuerySourceListA
        SetupQuerySourceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQuerySourceListW(
        # _In_ DWORD Flags,
        # _Outptr_result_buffer_(*Count) PCWSTR **List,
        # _Out_ PUINT Count
        # );
        SetupQuerySourceListW = setupapi.SetupQuerySourceListW
        SetupQuerySourceListW.restype = BOOL

        if defined(UNICODE):
            SetupQuerySourceList = SetupQuerySourceListW
        else:
            SetupQuerySourceList = SetupQuerySourceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFreeSourceListA(
        # _Inout_ _At_(*List, _Pre_readable_size_(Count) _Post_null_) PCSTR **List,
        # _In_ UINT Count
        # );
        SetupFreeSourceListA = setupapi.SetupFreeSourceListA
        SetupFreeSourceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupFreeSourceListW(
        # _Inout_ _At_(*List, _Pre_readable_size_(Count) _Post_null_) PCWSTR **List,
        # _In_ UINT Count
        # );
        SetupFreeSourceListW = setupapi.SetupFreeSourceListW
        SetupFreeSourceListW.restype = BOOL

        if defined(UNICODE):
            SetupFreeSourceList = SetupFreeSourceListW
        else:
            SetupFreeSourceList = SetupFreeSourceListA
        # END IF
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupPromptForDiskA(
        # _In_ HWND hwndParent,
        # _In_opt_ PCSTR DialogTitle,
        # _In_opt_ PCSTR DiskName,
        # _In_opt_ PCSTR PathToSource,
        # _In_ PCSTR FileSought,
        # _In_opt_ PCSTR TagFile,
        # _In_ DWORD DiskPromptStyle,
        # _Out_writes_opt_(PathBufferSize) PSTR PathBuffer,
        # _In_ DWORD PathBufferSize,
        # _Out_opt_ PDWORD PathRequiredSize
        # );
        SetupPromptForDiskA = setupapi.SetupPromptForDiskA
        SetupPromptForDiskA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupPromptForDiskW(
        # _In_ HWND hwndParent,
        # _In_opt_ PCWSTR DialogTitle,
        # _In_opt_ PCWSTR DiskName,
        # _In_opt_ PCWSTR PathToSource,
        # _In_ PCWSTR FileSought,
        # _In_opt_ PCWSTR TagFile,
        # _In_ DWORD DiskPromptStyle,
        # _Out_writes_opt_(PathBufferSize) PWSTR PathBuffer,
        # _In_ DWORD PathBufferSize,
        # _Out_opt_ PDWORD PathRequiredSize
        # );
        SetupPromptForDiskW = setupapi.SetupPromptForDiskW
        SetupPromptForDiskW.restype = UINT

        if defined(UNICODE):
            SetupPromptForDisk = SetupPromptForDiskW
        else:
            SetupPromptForDisk = SetupPromptForDiskA
        # END IF
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupCopyErrorA(
        # _In_ HWND hwndParent,
        # _In_opt_ PCSTR DialogTitle,
        # _In_opt_ PCSTR DiskName,
        # _In_ PCSTR PathToSource,
        # _In_ PCSTR SourceFile,
        # _In_opt_ PCSTR TargetPathFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style,
        # _Out_writes_opt_(PathBufferSize) PSTR PathBuffer,
        # _In_ DWORD PathBufferSize,
        # _Out_opt_ PDWORD PathRequiredSize
        # );
        SetupCopyErrorA = setupapi.SetupCopyErrorA
        SetupCopyErrorA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupCopyErrorW(
        # _In_ HWND hwndParent,
        # _In_opt_ PCWSTR DialogTitle,
        # _In_opt_ PCWSTR DiskName,
        # _In_ PCWSTR PathToSource,
        # _In_ PCWSTR SourceFile,
        # _In_opt_ PCWSTR TargetPathFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style,
        # _Out_writes_opt_(PathBufferSize) PWSTR PathBuffer,
        # _In_ DWORD PathBufferSize,
        # _Out_opt_ PDWORD PathRequiredSize
        # );
        SetupCopyErrorW = setupapi.SetupCopyErrorW
        SetupCopyErrorW.restype = UINT

        if defined(UNICODE):
            SetupCopyError = SetupCopyErrorW
        else:
            SetupCopyError = SetupCopyErrorA
        # END IF
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupRenameErrorA(
        # _In_ HWND hwndParent,
        # _In_opt_ PCSTR DialogTitle,
        # _In_ PCSTR SourceFile,
        # _In_ PCSTR TargetFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupRenameErrorA = setupapi.SetupRenameErrorA
        SetupRenameErrorA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupRenameErrorW(
        # _In_ HWND hwndParent,
        # _In_opt_ PCWSTR DialogTitle,
        # _In_ PCWSTR SourceFile,
        # _In_ PCWSTR TargetFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupRenameErrorW = setupapi.SetupRenameErrorW
        SetupRenameErrorW.restype = UINT

        if defined(UNICODE):
            SetupRenameError = SetupRenameErrorW
        else:
            SetupRenameError = SetupRenameErrorA
        # END IF
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupDeleteErrorA(
        # _In_ HWND hwndParent,
        # _In_opt_ PCSTR DialogTitle,
        # _In_ PCSTR File,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupDeleteErrorA = setupapi.SetupDeleteErrorA
        SetupDeleteErrorA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupDeleteErrorW(
        # _In_ HWND hwndParent,
        # _In_opt_ PCWSTR DialogTitle,
        # _In_ PCWSTR File,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupDeleteErrorW = setupapi.SetupDeleteErrorW
        SetupDeleteErrorW.restype = UINT

        if defined(UNICODE):
            SetupDeleteError = SetupDeleteErrorW
        else:
            SetupDeleteError = SetupDeleteErrorA
        # END IF
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupBackupErrorA(
        # _In_ HWND hwndParent,
        # _In_opt_ PCSTR DialogTitle,
        # _In_ PCSTR SourceFile,
        # _In_opt_ PCSTR TargetFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupBackupErrorA = setupapi.SetupBackupErrorA
        SetupBackupErrorA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupBackupErrorW(
        # _In_ HWND hwndParent,
        # _In_opt_ PCWSTR DialogTitle,
        # _In_ PCWSTR SourceFile,
        # _In_opt_ PCWSTR TargetFile,
        # _In_ UINT Win32ErrorCode,
        # _In_ DWORD Style
        # );
        SetupBackupErrorW = setupapi.SetupBackupErrorW
        SetupBackupErrorW.restype = UINT

        if defined(UNICODE):
            SetupBackupError = SetupBackupErrorW
        else:
            SetupBackupError = SetupBackupErrorA
        # END IF

        # Styles for SetupPromptForDisk, SetupCopyError,

        # SetupRenameError, SetupDeleteError
        IDF_NOBROWSE = 0x00000001
        IDF_NOSKIP = 0x00000002
        IDF_NODETAILS = 0x00000004
        IDF_NOCOMPRESSED = 0x00000008
        IDF_CHECKFIRST = 0x00000100
        IDF_NOBEEP = 0x00000200
        IDF_NOFOREGROUND = 0x00000400
        IDF_WARNIFSKIP = 0x00000800

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            IDF_NOREMOVABLEMEDIAPROMPT = 0x00001000
            IDF_USEDISKNAMEASPROMPT = 0x00002000
            IDF_OEMDISK = 0x80000000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Return values for SetupPromptForDisk, SetupCopyError,

        # SetupRenameError, SetupDeleteError, SetupBackupError
        DPROMPT_SUCCESS = 0
        DPROMPT_CANCEL = 1
        DPROMPT_SKIPFILE = 2
        DPROMPT_BUFFERTOOSMALL = 3
        DPROMPT_OUTOFMEMORY = 4
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetDirectoryIdA(
        # _In_ HINF InfHandle,
        # _In_ DWORD Id,
        # _In_opt_ PCSTR Directory
        # );
        SetupSetDirectoryIdA = setupapi.SetupSetDirectoryIdA
        SetupSetDirectoryIdA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetDirectoryIdW(
        # _In_ HINF InfHandle,
        # _In_ DWORD Id,
        # _In_opt_ PCWSTR Directory
        # );
        SetupSetDirectoryIdW = setupapi.SetupSetDirectoryIdW
        SetupSetDirectoryIdW.restype = BOOL

        if defined(UNICODE):
            SetupSetDirectoryId = SetupSetDirectoryIdW
        else:
            SetupSetDirectoryId = SetupSetDirectoryIdA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetDirectoryIdExA(
        # _In_ HINF InfHandle,
        # _In_ DWORD Id,
        # _In_opt_ PCSTR Directory,
        # _In_ DWORD Flags,
        # _Reserved_ DWORD Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupSetDirectoryIdExA = setupapi.SetupSetDirectoryIdExA
        SetupSetDirectoryIdExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetDirectoryIdExW(
        # _In_ HINF InfHandle,
        # _In_ DWORD Id,
        # _In_opt_ PCWSTR Directory,
        # _In_ DWORD Flags,
        # _Reserved_ DWORD Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupSetDirectoryIdExW = setupapi.SetupSetDirectoryIdExW
        SetupSetDirectoryIdExW.restype = BOOL

        if defined(UNICODE):
            SetupSetDirectoryIdEx = SetupSetDirectoryIdExW
        else:
            SetupSetDirectoryIdEx = SetupSetDirectoryIdExA
        # END IF

        # Flags for SetupSetDirectoryIdEx
        SETDIRID_NOT_FULL_PATH = 0x00000001
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceInfoA(
        # _In_ HINF InfHandle,
        # _In_ UINT SourceId,
        # _In_ UINT InfoDesired,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetSourceInfoA = setupapi.SetupGetSourceInfoA
        SetupGetSourceInfoA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetSourceInfoW(
        # _In_ HINF InfHandle,
        # _In_ UINT SourceId,
        # _In_ UINT InfoDesired,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupGetSourceInfoW = setupapi.SetupGetSourceInfoW
        SetupGetSourceInfoW.restype = BOOL

        if defined(UNICODE):
            SetupGetSourceInfo = SetupGetSourceInfoW
        else:
            SetupGetSourceInfo = SetupGetSourceInfoA
        # END IF

        # InfoDesired values for SetupGetSourceInfo
        SRCINFO_PATH = 1
        SRCINFO_TAGFILE = 2
        SRCINFO_DESCRIPTION = 3
        SRCINFO_FLAGS = 4

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # SRC_FLAGS allow special treatment of source

            # lower 4 bits are reserved for OS use

            # the flags may determine what other parameters exist
            SRCINFO_TAGFILE2 = 5
            SRC_FLAGS_CABFILE = 0x0010
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFileA(
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCSTR SourceFile,
        # _In_opt_ PCSTR SourcePathRoot,
        # _In_opt_ PCSTR DestinationName,
        # _In_ DWORD CopyStyle,
        # _In_opt_ PSP_FILE_CALLBACK_A CopyMsgHandler,
        # _In_opt_ PVOID Context
        # );
        SetupInstallFileA = setupapi.SetupInstallFileA
        SetupInstallFileA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFileW(
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCWSTR SourceFile,
        # _In_opt_ PCWSTR SourcePathRoot,
        # _In_opt_ PCWSTR DestinationName,
        # _In_ DWORD CopyStyle,
        # _In_opt_ PSP_FILE_CALLBACK_W CopyMsgHandler,
        # _In_opt_ PVOID Context
        # );
        SetupInstallFileW = setupapi.SetupInstallFileW
        SetupInstallFileW.restype = BOOL

        if defined(UNICODE):
            SetupInstallFile = SetupInstallFileW
        else:
            SetupInstallFile = SetupInstallFileA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFileExA(
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCSTR SourceFile,
        # _In_opt_ PCSTR SourcePathRoot,
        # _In_opt_ PCSTR DestinationName,
        # _In_ DWORD CopyStyle,
        # _In_opt_ PSP_FILE_CALLBACK_A CopyMsgHandler,
        # _In_opt_ PVOID Context,
        # _Out_ PBOOL FileWasInUse
        # );
        SetupInstallFileExA = setupapi.SetupInstallFileExA
        SetupInstallFileExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFileExW(
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PINFCONTEXT InfContext,
        # _In_opt_ PCWSTR SourceFile,
        # _In_opt_ PCWSTR SourcePathRoot,
        # _In_opt_ PCWSTR DestinationName,
        # _In_ DWORD CopyStyle,
        # _In_opt_ PSP_FILE_CALLBACK_W CopyMsgHandler,
        # _In_opt_ PVOID Context,
        # _Out_ PBOOL FileWasInUse
        # );
        SetupInstallFileExW = setupapi.SetupInstallFileExW
        SetupInstallFileExW.restype = BOOL

        if defined(UNICODE):
            SetupInstallFileEx = SetupInstallFileExW
        else:
            SetupInstallFileEx = SetupInstallFileExA
        # END IF

        # CopyStyle values for copy and queue - related APIs
        SP_COPY_DELETESOURCE = 0x0000001
        SP_COPY_REPLACEONLY = 0x0000002
        SP_COPY_NEWER = 0x0000004
        SP_COPY_NEWER_OR_SAME = SP_COPY_NEWER
        SP_COPY_NOOVERWRITE = 0x0000008
        SP_COPY_NODECOMP = 0x0000010
        SP_COPY_LANGUAGEAWARE = 0x0000020
        SP_COPY_SOURCE_ABSOLUTE = 0x0000040
        SP_COPY_SOURCEPATH_ABSOLUTE = 0x0000080
        SP_COPY_IN_USE_NEEDS_REBOOT = 0x0000100
        SP_COPY_FORCE_IN_USE = 0x0000200
        SP_COPY_NOSKIP = 0x0000400
        SP_FLAG_CABINETCONTINUATION = 0x0000800
        SP_COPY_FORCE_NOOVERWRITE = 0x0001000
        SP_COPY_FORCE_NEWER = 0x0002000
        SP_COPY_WARNIFSKIP = 0x0004000
        SP_COPY_NOBROWSE = 0x0008000
        SP_COPY_NEWER_ONLY = 0x0010000
        SP_COPY_RESERVED = 0x0020000
        SP_COPY_OEMINF_CATALOG_ONLY = 0x0040000
        SP_COPY_REPLACE_BOOT_FILE = 0x0080000

        # needed by the loader); this flag implies a reboot
        SP_COPY_NOPRUNE = 0x0100000

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            SP_COPY_OEM_F6_INF = 0x0200000
        # END IF  _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            SP_COPY_ALREADYDECOMP = 0x0400000
        # END IF  _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            SP_COPY_WINDOWS_SIGNED = 0x1000000
            SP_COPY_PNPLOCKED = 0x2000000
            SP_COPY_IN_USE_TRY_RENAME = 0x4000000
            SP_COPY_INBOX_INF = 0x8000000
        # END IF  _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN7:
            SP_COPY_HARDLINK = 0x10000000
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # Flags passed to Backup notification
            SP_BACKUP_BACKUPPASS = 0x00000001
            SP_BACKUP_DEMANDPASS = 0x00000002
            SP_BACKUP_SPECIAL = 0x00000004
            SP_BACKUP_BOOTFILE = 0x00000008
        # END IF  _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        # WINSETUPAPI
        # HSPFILEQ
        # WINAPI
        # SetupOpenFileQueue(
        # VOID
        # );
        SetupOpenFileQueue = setupapi.SetupOpenFileQueue
        SetupOpenFileQueue.restype = HSPFILEQ
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCloseFileQueue(
        # _In_ HSPFILEQ QueueHandle
        # );
        SetupCloseFileQueue = setupapi.SetupCloseFileQueue
        SetupCloseFileQueue.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetFileQueueAlternatePlatformA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
        # _In_opt_ PCSTR AlternateDefaultCatalogFile
        # );
        SetupSetFileQueueAlternatePlatformA = setupapi.SetupSetFileQueueAlternatePlatformA
        SetupSetFileQueueAlternatePlatformA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetFileQueueAlternatePlatformW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
        # _In_opt_ PCWSTR AlternateDefaultCatalogFile
        # );
        SetupSetFileQueueAlternatePlatformW = setupapi.SetupSetFileQueueAlternatePlatformW
        SetupSetFileQueueAlternatePlatformW.restype = BOOL

        if defined(UNICODE):
            SetupSetFileQueueAlternatePlatform = (
                SetupSetFileQueueAlternatePlatformW
            )
        else:
            SetupSetFileQueueAlternatePlatform = (
                SetupSetFileQueueAlternatePlatformA
            )
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetPlatformPathOverrideA(
        # _In_opt_ PCSTR Override
        # );
        SetupSetPlatformPathOverrideA = setupapi.SetupSetPlatformPathOverrideA
        SetupSetPlatformPathOverrideA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupSetPlatformPathOverrideW(
        # _In_opt_ PCWSTR Override
        # );
        SetupSetPlatformPathOverrideW = setupapi.SetupSetPlatformPathOverrideW
        SetupSetPlatformPathOverrideW.restype = BOOL

        if defined(UNICODE):
            SetupSetPlatformPathOverride = SetupSetPlatformPathOverrideW
        else:
            SetupSetPlatformPathOverride = SetupSetPlatformPathOverrideA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopyA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PCSTR SourceRootPath,
        # _In_opt_ PCSTR SourcePath,
        # _In_ PCSTR SourceFilename,
        # _In_opt_ PCSTR SourceDescription,
        # _In_opt_ PCSTR SourceTagfile,
        # _In_ PCSTR TargetDirectory,
        # _In_opt_ PCSTR TargetFilename,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueCopyA = setupapi.SetupQueueCopyA
        SetupQueueCopyA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopyW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PCWSTR SourceRootPath,
        # _In_opt_ PCWSTR SourcePath,
        # _In_ PCWSTR SourceFilename,
        # _In_opt_ PCWSTR SourceDescription,
        # _In_opt_ PCWSTR SourceTagfile,
        # _In_ PCWSTR TargetDirectory,
        # _In_opt_ PCWSTR TargetFilename,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueCopyW = setupapi.SetupQueueCopyW
        SetupQueueCopyW.restype = BOOL

        if defined(UNICODE):
            SetupQueueCopy = SetupQueueCopyW
        else:
            SetupQueueCopy = SetupQueueCopyA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopyIndirectA(
        # _In_ PSP_FILE_COPY_PARAMS_A CopyParams
        # );
        SetupQueueCopyIndirectA = setupapi.SetupQueueCopyIndirectA
        SetupQueueCopyIndirectA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopyIndirectW(
        # _In_ PSP_FILE_COPY_PARAMS_W CopyParams
        # );
        SetupQueueCopyIndirectW = setupapi.SetupQueueCopyIndirectW
        SetupQueueCopyIndirectW.restype = BOOL

        if defined(UNICODE):
            SetupQueueCopyIndirect = SetupQueueCopyIndirectW
        else:
            SetupQueueCopyIndirect = SetupQueueCopyIndirectA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDefaultCopyA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ PCSTR SourceRootPath,
        # _In_ PCSTR SourceFilename,
        # _In_opt_ PCSTR TargetFilename,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueDefaultCopyA = setupapi.SetupQueueDefaultCopyA
        SetupQueueDefaultCopyA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDefaultCopyW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ PCWSTR SourceRootPath,
        # _In_ PCWSTR SourceFilename,
        # _In_opt_ PCWSTR TargetFilename,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueDefaultCopyW = setupapi.SetupQueueDefaultCopyW
        SetupQueueDefaultCopyW.restype = BOOL

        if defined(UNICODE):
            SetupQueueDefaultCopy = SetupQueueDefaultCopyW
        else:
            SetupQueueDefaultCopy = SetupQueueDefaultCopyA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopySectionA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PCSTR SourceRootPath,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCSTR Section,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueCopySectionA = setupapi.SetupQueueCopySectionA
        SetupQueueCopySectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueCopySectionW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_opt_ PCWSTR SourceRootPath,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCWSTR Section,
        # _In_ DWORD CopyStyle
        # );
        SetupQueueCopySectionW = setupapi.SetupQueueCopySectionW
        SetupQueueCopySectionW.restype = BOOL

        if defined(UNICODE):
            SetupQueueCopySection = SetupQueueCopySectionW
        else:
            SetupQueueCopySection = SetupQueueCopySectionA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDeleteA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PCSTR PathPart1,
        # _In_opt_ PCSTR PathPart2
        # );
        SetupQueueDeleteA = setupapi.SetupQueueDeleteA
        SetupQueueDeleteA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDeleteW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PCWSTR PathPart1,
        # _In_opt_ PCWSTR PathPart2
        # );
        SetupQueueDeleteW = setupapi.SetupQueueDeleteW
        SetupQueueDeleteW.restype = BOOL

        if defined(UNICODE):
            SetupQueueDelete = SetupQueueDeleteW
        else:
            SetupQueueDelete = SetupQueueDeleteA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDeleteSectionA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCSTR Section
        # );
        SetupQueueDeleteSectionA = setupapi.SetupQueueDeleteSectionA
        SetupQueueDeleteSectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueDeleteSectionW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCWSTR Section
        # );
        SetupQueueDeleteSectionW = setupapi.SetupQueueDeleteSectionW
        SetupQueueDeleteSectionW.restype = BOOL

        if defined(UNICODE):
            SetupQueueDeleteSection = SetupQueueDeleteSectionW
        else:
            SetupQueueDeleteSection = SetupQueueDeleteSectionA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueRenameA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PCSTR SourcePath,
        # _In_opt_ PCSTR SourceFilename,
        # _In_opt_ PCSTR TargetPath,
        # _In_ PCSTR TargetFilename
        # );
        SetupQueueRenameA = setupapi.SetupQueueRenameA
        SetupQueueRenameA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueRenameW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PCWSTR SourcePath,
        # _In_opt_ PCWSTR SourceFilename,
        # _In_opt_ PCWSTR TargetPath,
        # _In_ PCWSTR TargetFilename
        # );
        SetupQueueRenameW = setupapi.SetupQueueRenameW
        SetupQueueRenameW.restype = BOOL

        if defined(UNICODE):
            SetupQueueRename = SetupQueueRenameW
        else:
            SetupQueueRename = SetupQueueRenameA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueRenameSectionA(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCSTR Section
        # );
        SetupQueueRenameSectionA = setupapi.SetupQueueRenameSectionA
        SetupQueueRenameSectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueueRenameSectionW(
        # _In_ HSPFILEQ QueueHandle,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCWSTR Section
        # );
        SetupQueueRenameSectionW = setupapi.SetupQueueRenameSectionW
        SetupQueueRenameSectionW.restype = BOOL

        if defined(UNICODE):
            SetupQueueRenameSection = SetupQueueRenameSectionW
        else:
            SetupQueueRenameSection = SetupQueueRenameSectionA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCommitFileQueueA(
        # _In_opt_ HWND Owner,
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PSP_FILE_CALLBACK_A MsgHandler,
        # _In_ PVOID Context
        # );
        SetupCommitFileQueueA = setupapi.SetupCommitFileQueueA
        SetupCommitFileQueueA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCommitFileQueueW(
        # _In_opt_ HWND Owner,
        # _In_ HSPFILEQ QueueHandle,
        # _In_ PSP_FILE_CALLBACK_W MsgHandler,
        # _In_ PVOID Context
        # );
        SetupCommitFileQueueW = setupapi.SetupCommitFileQueueW
        SetupCommitFileQueueW.restype = BOOL

        if defined(UNICODE):
            SetupCommitFileQueue = SetupCommitFileQueueW
        else:
            SetupCommitFileQueue = SetupCommitFileQueueA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupScanFileQueueA(
        # _In_ HSPFILEQ FileQueue,
        # _In_ DWORD Flags,
        # _In_opt_ HWND Window,
        # _In_opt_ PSP_FILE_CALLBACK_A CallbackRoutine,
        # _In_opt_ PVOID CallbackContext,
        # _Out_ PDWORD Result
        # );
        SetupScanFileQueueA = setupapi.SetupScanFileQueueA
        SetupScanFileQueueA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupScanFileQueueW(
        # _In_ HSPFILEQ FileQueue,
        # _In_ DWORD Flags,
        # _In_opt_ HWND Window,
        # _In_opt_ PSP_FILE_CALLBACK_W CallbackRoutine,
        # _In_opt_ PVOID CallbackContext,
        # _Out_ PDWORD Result
        # );
        SetupScanFileQueueW = setupapi.SetupScanFileQueueW
        SetupScanFileQueueW.restype = BOOL

        if defined(UNICODE):
            SetupScanFileQueue = SetupScanFileQueueW
        else:
            SetupScanFileQueue = SetupScanFileQueueA
        # END IF

        # Define flags for SetupScanFileQueue.
        SPQ_SCAN_FILE_PRESENCE = 0x00000001
        SPQ_SCAN_FILE_VALIDITY = 0x00000002
        SPQ_SCAN_USE_CALLBACK = 0x00000004
        SPQ_SCAN_USE_CALLBACKEX = 0x00000008
        SPQ_SCAN_INFORM_USER = 0x00000010
        SPQ_SCAN_PRUNE_COPY_QUEUE = 0x00000020

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            SPQ_SCAN_USE_CALLBACK_SIGNERINFO = 0x00000040
            SPQ_SCAN_PRUNE_DELREN = 0x00000080
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            SPQ_SCAN_FILE_PRESENCE_WITHOUT_SOURCE = 0x00000100
            SPQ_SCAN_FILE_COMPARISON = 0x00000200
            SPQ_SCAN_ACTIVATE_DRP = 0x00000400
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        # Define flags used with Param2 for SPFILENOTIFY_QUEUESCAN
        SPQ_DELAYED_COPY = 0x00000001

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetFileQueueCount(
            # _In_ HSPFILEQ FileQueue,
            # _In_ UINT SubQueueFileOp,
            # _Out_ PUINT NumOperations
            # );
            SetupGetFileQueueCount = setupapi.SetupGetFileQueueCount
            SetupGetFileQueueCount.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetFileQueueFlags(
            # _In_ HSPFILEQ FileQueue,
            # _Out_ PDWORD Flags
            # );
            SetupGetFileQueueFlags = setupapi.SetupGetFileQueueFlags
            SetupGetFileQueueFlags.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupSetFileQueueFlags(
            # _In_ HSPFILEQ FileQueue,
            # _In_ DWORD FlagMask,
            # _In_ DWORD Flags
            # );
            SetupSetFileQueueFlags = setupapi.SetupSetFileQueueFlags
            SetupSetFileQueueFlags.restype = BOOL

            # Flags/FlagMask for use with SetupSetFileQueueFlags and returned
            # by SetupGetFileQueueFlags
            SPQ_FLAG_BACKUP_AWARE = 0x00000001

            # issue backup notifications.
            SPQ_FLAG_ABORT_IF_UNSIGNED = 0x00000002

            # fail with ERROR_SET_SYSTEM_RESTORE_POINT

            # if the user elects to proceed with an

            # UINT queue committal. This allows

            # the caller to set a system restore point,

            # then re - commit the file queue.
            SPQ_FLAG_FILES_MODIFIED = 0x00000004

            # replaced by a different version
            SPQ_FLAG_DO_SHUFFLEMOVE = 0x00000008

            # move will first try to copy the source over the

            # destination file, but if the destination file is

            # in use it will rename the destination file to a

            # temp name and queue the temp name for deletion.

            # It will then be free to copy the source to the

            # destination name. It is considered an error if

            # the destination file can't be renamed for some

            # reason.
            SPQ_FLAG_VALID = 0x0000000F
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Define OEM Source Type values for use in SetupCopyOEMInf.
        SPOST_NONE = 0
        SPOST_PATH = 1
        SPOST_URL = 2
        SPOST_MAX = 3
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCopyOEMInfA(
        # _In_ PCSTR SourceInfFileName,
        # _In_opt_ PCSTR OEMSourceMediaLocation,
        # _In_ DWORD OEMSourceMediaType,
        # _In_ DWORD CopyStyle,
        # _Out_writes_opt_(DestinationInfFileNameSize) PSTR DestinationInfFileName,
        # _In_ DWORD DestinationInfFileNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _Out_opt_ PSTR *DestinationInfFileNameComponent
        # );
        SetupCopyOEMInfA = setupapi.SetupCopyOEMInfA
        SetupCopyOEMInfA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupCopyOEMInfW(
        # _In_ PCWSTR SourceInfFileName,
        # _In_opt_ PCWSTR OEMSourceMediaLocation,
        # _In_ DWORD OEMSourceMediaType,
        # _In_ DWORD CopyStyle,
        # _Out_writes_opt_(DestinationInfFileNameSize) PWSTR DestinationInfFileName,
        # _In_ DWORD DestinationInfFileNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _Out_opt_ PWSTR  *DestinationInfFileNameComponent
        # );
        SetupCopyOEMInfW = setupapi.SetupCopyOEMInfW
        SetupCopyOEMInfW.restype = BOOL

        if defined(UNICODE):
            SetupCopyOEMInf = SetupCopyOEMInfW
        else:
            SetupCopyOEMInf = SetupCopyOEMInfA
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # Flags used by SetupUninstallOEMInf
            SUOI_FORCEDELETE = 0x00000001
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            SUOI_INTERNAL1 = 0x00000002
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupUninstallOEMInfA(
            # _In_ PCSTR InfFileName,
            # _In_ DWORD Flags,
            # _Reserved_ PVOID Reserved
            # );
            SetupUninstallOEMInfA = setupapi.SetupUninstallOEMInfA
            SetupUninstallOEMInfA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupUninstallOEMInfW(
            # _In_ PCWSTR InfFileName,
            # _In_ DWORD Flags,
            # _Reserved_ PVOID Reserved
            # );
            SetupUninstallOEMInfW = setupapi.SetupUninstallOEMInfW
            SetupUninstallOEMInfW.restype = BOOL
            if defined(UNICODE):
                SetupUninstallOEMInf = SetupUninstallOEMInfW
            else:
                SetupUninstallOEMInf = SetupUninstallOEMInfA
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupUninstallNewlyCopiedInfs(
            # _In_ HSPFILEQ FileQueue,
            # _In_ DWORD Flags,
            # _Reserved_ PVOID Reserved
            # );
            SetupUninstallNewlyCopiedInfs = setupapi.SetupUninstallNewlyCopiedInfs
            SetupUninstallNewlyCopiedInfs.restype = BOOL
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Disk space list APIs
        # WINSETUPAPI
        # HDSKSPC
        # WINAPI
        # SetupCreateDiskSpaceListA(
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ DWORD Reserved2,
        # _In_ UINT Flags
        # );
        SetupCreateDiskSpaceListA = setupapi.SetupCreateDiskSpaceListA
        SetupCreateDiskSpaceListA.restype = HDSKSPC
        # WINSETUPAPI
        # HDSKSPC
        # WINAPI
        # SetupCreateDiskSpaceListW(
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ DWORD Reserved2,
        # _In_ UINT Flags
        # );
        SetupCreateDiskSpaceListW = setupapi.SetupCreateDiskSpaceListW
        SetupCreateDiskSpaceListW.restype = HDSKSPC

        if defined(UNICODE):
            SetupCreateDiskSpaceList = SetupCreateDiskSpaceListW
        else:
            SetupCreateDiskSpaceList = SetupCreateDiskSpaceListA
        # END IF

        # Flags for SetupCreateDiskSpaceList
        SPDSL_IGNORE_DISK = 0x00000001
        SPDSL_DISALLOW_NEGATIVE_ADJUST = 0x00000002
        # WINSETUPAPI
        # HDSKSPC
        # WINAPI
        # SetupDuplicateDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ DWORD Reserved2,
        # _In_ UINT Flags
        # );
        SetupDuplicateDiskSpaceListA = setupapi.SetupDuplicateDiskSpaceListA
        SetupDuplicateDiskSpaceListA.restype = HDSKSPC
        # WINSETUPAPI
        # HDSKSPC
        # WINAPI
        # SetupDuplicateDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ DWORD Reserved2,
        # _In_ UINT Flags
        # );
        SetupDuplicateDiskSpaceListW = setupapi.SetupDuplicateDiskSpaceListW
        SetupDuplicateDiskSpaceListW.restype = HDSKSPC

        if defined(UNICODE):
            SetupDuplicateDiskSpaceList = SetupDuplicateDiskSpaceListW
        else:
            SetupDuplicateDiskSpaceList = SetupDuplicateDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDestroyDiskSpaceList(
        # _Inout_ HDSKSPC DiskSpace
        # );
        SetupDestroyDiskSpaceList = setupapi.SetupDestroyDiskSpaceList
        SetupDestroyDiskSpaceList.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryDrivesInDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _Out_writes_opt_(ReturnBufferSize) PSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryDrivesInDiskSpaceListA = setupapi.SetupQueryDrivesInDiskSpaceListA
        SetupQueryDrivesInDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryDrivesInDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR ReturnBuffer,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryDrivesInDiskSpaceListW = setupapi.SetupQueryDrivesInDiskSpaceListW
        SetupQueryDrivesInDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupQueryDrivesInDiskSpaceList = SetupQueryDrivesInDiskSpaceListW
        else:
            SetupQueryDrivesInDiskSpaceList = SetupQueryDrivesInDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQuerySpaceRequiredOnDriveA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCSTR DriveSpec,
        # _Out_ LONGLONG *SpaceRequired,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupQuerySpaceRequiredOnDriveA = setupapi.SetupQuerySpaceRequiredOnDriveA
        SetupQuerySpaceRequiredOnDriveA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQuerySpaceRequiredOnDriveW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCWSTR DriveSpec,
        # _Out_ LONGLONG *SpaceRequired,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupQuerySpaceRequiredOnDriveW = setupapi.SetupQuerySpaceRequiredOnDriveW
        SetupQuerySpaceRequiredOnDriveW.restype = BOOL

        if defined(UNICODE):
            SetupQuerySpaceRequiredOnDrive = SetupQuerySpaceRequiredOnDriveW
        else:
            SetupQuerySpaceRequiredOnDrive = SetupQuerySpaceRequiredOnDriveA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAdjustDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ LPCSTR DriveRoot,
        # _In_ LONGLONG Amount,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAdjustDiskSpaceListA = setupapi.SetupAdjustDiskSpaceListA
        SetupAdjustDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAdjustDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ LPCWSTR DriveRoot,
        # _In_ LONGLONG Amount,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAdjustDiskSpaceListW = setupapi.SetupAdjustDiskSpaceListW
        SetupAdjustDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupAdjustDiskSpaceList = SetupAdjustDiskSpaceListW
        else:
            SetupAdjustDiskSpaceList = SetupAdjustDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddToDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCSTR TargetFilespec,
        # _In_ LONGLONG FileSize,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddToDiskSpaceListA = setupapi.SetupAddToDiskSpaceListA
        SetupAddToDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddToDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCWSTR TargetFilespec,
        # _In_ LONGLONG FileSize,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddToDiskSpaceListW = setupapi.SetupAddToDiskSpaceListW
        SetupAddToDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupAddToDiskSpaceList = SetupAddToDiskSpaceListW
        else:
            SetupAddToDiskSpaceList = SetupAddToDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddSectionToDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCSTR SectionName,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddSectionToDiskSpaceListA = setupapi.SetupAddSectionToDiskSpaceListA
        SetupAddSectionToDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddSectionToDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCWSTR SectionName,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddSectionToDiskSpaceListW = setupapi.SetupAddSectionToDiskSpaceListW
        SetupAddSectionToDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupAddSectionToDiskSpaceList = SetupAddSectionToDiskSpaceListW
        else:
            SetupAddSectionToDiskSpaceList = SetupAddSectionToDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddInstallSectionToDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ PCSTR SectionName,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddInstallSectionToDiskSpaceListA = setupapi.SetupAddInstallSectionToDiskSpaceListA
        SetupAddInstallSectionToDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupAddInstallSectionToDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ PCWSTR SectionName,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupAddInstallSectionToDiskSpaceListW = setupapi.SetupAddInstallSectionToDiskSpaceListW
        SetupAddInstallSectionToDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupAddInstallSectionToDiskSpaceList = (
                SetupAddInstallSectionToDiskSpaceListW
            )
        else:
            SetupAddInstallSectionToDiskSpaceList = (
                SetupAddInstallSectionToDiskSpaceListA
            )
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFromDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCSTR TargetFilespec,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveFromDiskSpaceListA = setupapi.SetupRemoveFromDiskSpaceListA
        SetupRemoveFromDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFromDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ PCWSTR TargetFilespec,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveFromDiskSpaceListW = setupapi.SetupRemoveFromDiskSpaceListW
        SetupRemoveFromDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupRemoveFromDiskSpaceList = SetupRemoveFromDiskSpaceListW
        else:
            SetupRemoveFromDiskSpaceList = SetupRemoveFromDiskSpaceListA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveSectionFromDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCSTR SectionName,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveSectionFromDiskSpaceListA = setupapi.SetupRemoveSectionFromDiskSpaceListA
        SetupRemoveSectionFromDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveSectionFromDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF ListInfHandle,
        # _In_ PCWSTR SectionName,
        # _In_ UINT Operation,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveSectionFromDiskSpaceListW = setupapi.SetupRemoveSectionFromDiskSpaceListW
        SetupRemoveSectionFromDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupRemoveSectionFromDiskSpaceList = (
                SetupRemoveSectionFromDiskSpaceListW
            )
        else:
            SetupRemoveSectionFromDiskSpaceList = (
                SetupRemoveSectionFromDiskSpaceListA
            )
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveInstallSectionFromDiskSpaceListA(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ PCSTR SectionName,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveInstallSectionFromDiskSpaceListA = setupapi.SetupRemoveInstallSectionFromDiskSpaceListA
        SetupRemoveInstallSectionFromDiskSpaceListA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveInstallSectionFromDiskSpaceListW(
        # _In_ HDSKSPC DiskSpace,
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ PCWSTR SectionName,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ UINT Reserved2
        # );
        SetupRemoveInstallSectionFromDiskSpaceListW = setupapi.SetupRemoveInstallSectionFromDiskSpaceListW
        SetupRemoveInstallSectionFromDiskSpaceListW.restype = BOOL

        if defined(UNICODE):
            SetupRemoveInstallSectionFromDiskSpaceList = (
                SetupRemoveInstallSectionFromDiskSpaceListW
            )
        else:
            SetupRemoveInstallSectionFromDiskSpaceList = (
                SetupRemoveInstallSectionFromDiskSpaceListA
            )
        # END IF

        # Cabinet APIs
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupIterateCabinetA(
        # _In_ PCSTR CabinetFile,
        # _Reserved_ DWORD Reserved,
        # _In_ PSP_FILE_CALLBACK_A MsgHandler,
        # _In_ PVOID Context
        # );
        SetupIterateCabinetA = setupapi.SetupIterateCabinetA
        SetupIterateCabinetA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupIterateCabinetW(
        # _In_ PCWSTR CabinetFile,
        # _Reserved_ DWORD Reserved,
        # _In_ PSP_FILE_CALLBACK_W MsgHandler,
        # _In_ PVOID Context
        # );
        SetupIterateCabinetW = setupapi.SetupIterateCabinetW
        SetupIterateCabinetW.restype = BOOL

        if defined(UNICODE):
            SetupIterateCabinet = SetupIterateCabinetW
        else:
            SetupIterateCabinet = SetupIterateCabinetA
        # END IF
        # WINSETUPAPI
        # INT
        # WINAPI
        # SetupPromptReboot(
        # _In_opt_ HSPFILEQ FileQueue,
        # _In_opt_ HWND Owner,
        # _In_ BOOL ScanOnly
        # );
        SetupPromptReboot = setupapi.SetupPromptReboot
        SetupPromptReboot.restype = INT

        # Define flags that are returned by SetupPromptReboot
        SPFILEQ_FILE_IN_USE = 0x00000001
        SPFILEQ_REBOOT_RECOMMENDED = 0x00000002
        SPFILEQ_REBOOT_IN_PROGRESS = 0x00000004
        # WINSETUPAPI
        # PVOID
        # WINAPI
        # SetupInitDefaultQueueCallback(
        # _In_opt_ HWND OwnerWindow
        # );
        SetupInitDefaultQueueCallback = setupapi.SetupInitDefaultQueueCallback
        SetupInitDefaultQueueCallback.restype = PVOID
        # WINSETUPAPI
        # PVOID
        # WINAPI
        # SetupInitDefaultQueueCallbackEx(
        # _In_opt_ HWND OwnerWindow,
        # _In_opt_ HWND AlternateProgressWindow,
        # _In_ UINT ProgressMessage,
        # _Reserved_ DWORD Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupInitDefaultQueueCallbackEx = setupapi.SetupInitDefaultQueueCallbackEx
        SetupInitDefaultQueueCallbackEx.restype = PVOID
        # WINSETUPAPI
        # VOID
        # WINAPI
        # SetupTermDefaultQueueCallback(
        # _In_ PVOID Context
        # );
        SetupTermDefaultQueueCallback = setupapi.SetupTermDefaultQueueCallback
        SetupTermDefaultQueueCallback.restype = VOID
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupDefaultQueueCallbackA(
        # _In_ PVOID Context,
        # _In_ UINT Notification,
        # _In_ UINT_PTR Param1,
        # _In_ UINT_PTR Param2
        # );
        SetupDefaultQueueCallbackA = setupapi.SetupDefaultQueueCallbackA
        SetupDefaultQueueCallbackA.restype = UINT
        # WINSETUPAPI
        # UINT
        # WINAPI
        # SetupDefaultQueueCallbackW(
        # _In_ PVOID Context,
        # _In_ UINT Notification,
        # _In_ UINT_PTR Param1,
        # _In_ UINT_PTR Param2
        # );
        SetupDefaultQueueCallbackW = setupapi.SetupDefaultQueueCallbackW
        SetupDefaultQueueCallbackW.restype = UINT

        if defined(UNICODE):
            SetupDefaultQueueCallback = SetupDefaultQueueCallbackW
        else:
            SetupDefaultQueueCallback = SetupDefaultQueueCallbackA
        # END IF

        # Flags for AddReg section lines in INF. The corresponding value

        # is <ValueType> in the AddReg line format given below:

        # <RegRootString>,<SubKey>,<ValueName>,<ValueType>,<Value>...

        # The low word contains basic flags concerning the general data type

        # and AddReg action. The high word contains values that more
        # specifically

        # identify the data type of the registry value. The high word is
        # ignored

        # by the 16 - bit Windows 95 SETUPX APIs.

        # If <ValueType> has FLG_ADDREG_DELREG_BIT set, it will be ignored by
        # AddReg

        # (not supported by SetupX).

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            FLG_ADDREG_DELREG_BIT = 0x00008000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        FLG_ADDREG_BINVALUETYPE = 0x00000001
        FLG_ADDREG_NOCLOBBER = 0x00000002
        FLG_ADDREG_DELVAL = 0x00000004
        FLG_ADDREG_APPEND = 0x00000008

        # for REG_MULTI_SZ values.
        FLG_ADDREG_KEYONLY = 0x00000010
        FLG_ADDREG_OVERWRITEONLY = 0x00000020

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            FLG_ADDREG_64BITKEY = 0x00001000
            FLG_ADDREG_KEYONLY_COMMON = 0x00002000
            FLG_ADDREG_32BITKEY = 0x00004000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # The INF may supply any arbitrary data type ordinal in the highword
        # except

        # for the following: REG_NONE, REG_SZ, REG_EXPAND_SZ, REG_MULTI_SZ. If
        # this

        # technique is used, then the data is given in binary format, one byte
        # per

        # field.
        FLG_ADDREG_TYPE_MASK = 0xFFFF0000 | FLG_ADDREG_BINVALUETYPE
        FLG_ADDREG_TYPE_SZ = 0x00000000
        FLG_ADDREG_TYPE_MULTI_SZ = 0x00010000
        FLG_ADDREG_TYPE_EXPAND_SZ = 0x00020000
        FLG_ADDREG_TYPE_BINARY = 0x00000000 | FLG_ADDREG_BINVALUETYPE
        FLG_ADDREG_TYPE_DWORD = 0x00010000 | FLG_ADDREG_BINVALUETYPE
        FLG_ADDREG_TYPE_NONE = 0x00020000 | FLG_ADDREG_BINVALUETYPE

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN10:
            FLG_ADDREG_TYPE_QWORD = 0x000B0000 | FLG_ADDREG_BINVALUETYPE
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WIN10

        # Flags for DelReg section lines in INF. The corresponding value

        # is <Operation> in the extended DelReg line format given below:

        # <RegRootString>,<SubKey>,<ValueName>,<Operation>[,...]

        # In SetupX and some versions of SetupAPI, <Operation> will be ignored
        # and <ValueName> will

        # be deleted. Use with care.

        # The bits determined by mask FLG_DELREG_TYPE_MASK indicates type of
        # data expected.

        # <Operation> must have FLG_ADDREG_DELREG_BIT set, otherwise it is
        # ignored and specified

        # value will be deleted
        # (allowing an AddReg section to also be used as a DelReg section)

        # if <Operation> is not specified, <ValueName> will be deleted
        # (if specified) otherwise

        # <SubKey> will be deleted.

        # the compatability flag
        FLG_DELREG_VALUE = 0x00000000

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            FLG_DELREG_TYPE_MASK = FLG_ADDREG_TYPE_MASK
            FLG_DELREG_TYPE_SZ = FLG_ADDREG_TYPE_SZ
            FLG_DELREG_TYPE_MULTI_SZ = FLG_ADDREG_TYPE_MULTI_SZ
            FLG_DELREG_TYPE_EXPAND_SZ = FLG_ADDREG_TYPE_EXPAND_SZ
            FLG_DELREG_TYPE_BINARY = FLG_ADDREG_TYPE_BINARY
            FLG_DELREG_TYPE_DWORD = FLG_ADDREG_TYPE_DWORD
            FLG_DELREG_TYPE_NONE = FLG_ADDREG_TYPE_NONE
            FLG_DELREG_64BITKEY = FLG_ADDREG_64BITKEY
            FLG_DELREG_KEYONLY_COMMON = FLG_ADDREG_KEYONLY_COMMON
            FLG_DELREG_32BITKEY = FLG_ADDREG_32BITKEY

            # <Operation> = FLG_DELREG_MULTI_SZ_DELSTRING

            # <RegRootString>,<SubKey>,<ValueName>,0x00018002,<String>

            # removes all entries matching <String> (case ignored) from multi
            # - sz registry value
            FLG_DELREG_OPERATION_MASK = 0x000000FE
            FLG_DELREG_MULTI_SZ_DELSTRING = (
                FLG_DELREG_TYPE_MULTI_SZ |
                FLG_ADDREG_DELREG_BIT |
                0x00000002
            )
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN10:
            FLG_DELREG_TYPE_QWORD = FLG_ADDREG_TYPE_QWORD
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WIN10

        # Flags for BitReg section lines in INF.
        FLG_BITREG_CLEARBITS = 0x00000000
        FLG_BITREG_SETBITS = 0x00000001

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            FLG_BITREG_64BITKEY = 0x00001000
            FLG_BITREG_32BITKEY = 0x00004000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Flags for Ini2Reg section lines in INF.

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            FLG_INI2REG_64BITKEY = 0x00001000
            FLG_INI2REG_32BITKEY = 0x00004000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Flags for RegSvr section lines in INF
        FLG_REGSVR_DLLREGISTER = 0x00000001
        FLG_REGSVR_DLLINSTALL = 0x00000002

        # Flags for RegSvr section lines in INF
        FLG_PROFITEM_CURRENTUSER = 0x00000001
        FLG_PROFITEM_DELETE = 0x00000002
        FLG_PROFITEM_GROUP = 0x00000004
        FLG_PROFITEM_CSIDL = 0x00000008

        # Flags for AddProperty section lines in the INF
        FLG_ADDPROPERTY_NOCLOBBER = 0x00000001
        FLG_ADDPROPERTY_OVERWRITEONLY = 0x00000002
        FLG_ADDPROPERTY_APPEND = 0x00000004
        FLG_ADDPROPERTY_OR = 0x00000008
        FLG_ADDPROPERTY_AND = 0x00000010

        # Flags for DelProperty section lines in the INF
        FLG_DELPROPERTY_MULTI_SZ_DELSTRING = 0x00000001
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFromInfSectionA(
        # _In_opt_ HWND Owner,
        # _In_ HINF InfHandle,
        # _In_ PCSTR SectionName,
        # _In_ UINT Flags,
        # _In_opt_ HKEY RelativeKeyRoot,
        # _In_opt_ PCSTR SourceRootPath,
        # _In_ UINT CopyFlags,
        # _In_opt_ PSP_FILE_CALLBACK_A MsgHandler,
        # _In_opt_ PVOID Context,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupInstallFromInfSectionA = setupapi.SetupInstallFromInfSectionA
        SetupInstallFromInfSectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFromInfSectionW(
        # _In_opt_ HWND Owner,
        # _In_ HINF InfHandle,
        # _In_ PCWSTR SectionName,
        # _In_ UINT Flags,
        # _In_opt_ HKEY RelativeKeyRoot,
        # _In_opt_ PCWSTR SourceRootPath,
        # _In_ UINT CopyFlags,
        # _In_opt_ PSP_FILE_CALLBACK_W MsgHandler,
        # _In_opt_ PVOID Context,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupInstallFromInfSectionW = setupapi.SetupInstallFromInfSectionW
        SetupInstallFromInfSectionW.restype = BOOL

        if defined(UNICODE):
            SetupInstallFromInfSection = SetupInstallFromInfSectionW
        else:
            SetupInstallFromInfSection = SetupInstallFromInfSectionA
        # END IF

        # Flags for SetupInstallFromInfSection
        SPINST_LOGCONFIG = 0x00000001
        SPINST_INIFILES = 0x00000002
        SPINST_REGISTRY = 0x00000004
        SPINST_INI2REG = 0x00000008
        SPINST_FILES = 0x00000010
        SPINST_BITREG = 0x00000020
        SPINST_REGSVR = 0x00000040
        SPINST_UNREGSVR = 0x00000080
        SPINST_PROFILEITEMS = 0x00000100

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            SPINST_COPYINF = 0x00000200

            if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
                SPINST_PROPERTIES = 0x00000400
                SPINST_ALL = 0x000007FF
            else:
                SPINST_ALL = 0x000003FF
            # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN
        else:
            SPINST_ALL = 0x000001FF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        SPINST_SINGLESECTION = 0x00010000
        SPINST_LOGCONFIG_IS_FORCED = 0x00020000
        SPINST_LOGCONFIGS_ARE_OVERRIDES = 0x00040000

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            SPINST_REGISTERCALLBACKAWARE = 0x00080000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            SPINST_DEVICEINSTALL = 0x00100000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFilesFromInfSectionA(
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ HSPFILEQ FileQueue,
        # _In_ PCSTR SectionName,
        # _In_opt_ PCSTR SourceRootPath,
        # _In_ UINT CopyFlags
        # );
        SetupInstallFilesFromInfSectionA = setupapi.SetupInstallFilesFromInfSectionA
        SetupInstallFilesFromInfSectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallFilesFromInfSectionW(
        # _In_ HINF InfHandle,
        # _In_opt_ HINF LayoutInfHandle,
        # _In_ HSPFILEQ FileQueue,
        # _In_ PCWSTR SectionName,
        # _In_opt_ PCWSTR SourceRootPath,
        # _In_ UINT CopyFlags
        # );
        SetupInstallFilesFromInfSectionW = setupapi.SetupInstallFilesFromInfSectionW
        SetupInstallFilesFromInfSectionW.restype = BOOL

        if defined(UNICODE):
            SetupInstallFilesFromInfSection = SetupInstallFilesFromInfSectionW
        else:
            SetupInstallFilesFromInfSection = SetupInstallFilesFromInfSectionA
        # END IF

        # Flags for SetupInstallServicesFromInfSection(Ex). These flags are
        # also used

        # in the flags field of AddService or DelService lines in a device
        # INF. Some

        # of these flags are not permitted in the non - Ex API. These flags
        # are marked

        # as such below.

        # (AddService) move service's tag to front of its group order list
        SPSVCINST_TAGTOFRONT = 0x00000001

        # (AddService) **Ex API only** mark this service as the function
        # driver for the

        # device being installed
        SPSVCINST_ASSOCSERVICE = 0x00000002

        # (DelService) delete the associated event log entry for a service
        # specified in

        # a DelService entry
        SPSVCINST_DELETEEVENTLOGENTRY = 0x00000004

        # (AddService) don't overwrite display name if it already exists
        SPSVCINST_NOCLOBBER_DISPLAYNAME = 0x00000008

        # (AddService) don't overwrite start type value if service already
        # exists
        SPSVCINST_NOCLOBBER_STARTTYPE = 0x00000010

        # (AddService) don't overwrite error control value if service already
        # exists
        SPSVCINST_NOCLOBBER_ERRORCONTROL = 0x00000020

        # (AddService) don't overwrite load order group if it already exists
        SPSVCINST_NOCLOBBER_LOADORDERGROUP = 0x00000040

        # (AddService) don't overwrite dependencies list if it already exists
        SPSVCINST_NOCLOBBER_DEPENDENCIES = 0x00000080

        # (AddService) don't overwrite description if it already exists
        SPSVCINST_NOCLOBBER_DESCRIPTION = 0x00000100

        # (DelService) stop the associated service specified in

        # a DelService entry before deleting the service
        SPSVCINST_STOPSERVICE = 0x00000200

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # (AddService) force overwrite of security settings
            SPSVCINST_CLOBBER_SECURITY = 0x00000400
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:

            # (Start Service) start a service manually after install
            SPSVCINST_STARTSERVICE = 0x00000800
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN7:

            # (AddService) don't overwrite required privileges list if it
            # already exists
            SPSVCINST_NOCLOBBER_REQUIREDPRIVILEGES = 0x00001000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WIN7

        if _SETUPAPI_VER >= _WIN32_WINNT_WIN10:

            # (AddService) don't overwrite triggers if they already exist
            SPSVCINST_NOCLOBBER_TRIGGERS = 0x00002000
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WIN10
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallServicesFromInfSectionA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR SectionName,
        # _In_ DWORD Flags
        # );
        SetupInstallServicesFromInfSectionA = setupapi.SetupInstallServicesFromInfSectionA
        SetupInstallServicesFromInfSectionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallServicesFromInfSectionW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR SectionName,
        # _In_ DWORD Flags
        # );
        SetupInstallServicesFromInfSectionW = setupapi.SetupInstallServicesFromInfSectionW
        SetupInstallServicesFromInfSectionW.restype = BOOL

        if defined(UNICODE):
            SetupInstallServicesFromInfSection = (
                SetupInstallServicesFromInfSectionW
            )
        else:
            SetupInstallServicesFromInfSection = (
                SetupInstallServicesFromInfSectionA
            )
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallServicesFromInfSectionExA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR SectionName,
        # _In_ DWORD Flags,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupInstallServicesFromInfSectionExA = setupapi.SetupInstallServicesFromInfSectionExA
        SetupInstallServicesFromInfSectionExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupInstallServicesFromInfSectionExW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR SectionName,
        # _In_ DWORD Flags,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupInstallServicesFromInfSectionExW = setupapi.SetupInstallServicesFromInfSectionExW
        SetupInstallServicesFromInfSectionExW.restype = BOOL

        if defined(UNICODE):
            SetupInstallServicesFromInfSectionEx = (
                SetupInstallServicesFromInfSectionExW
            )
        else:
            SetupInstallServicesFromInfSectionEx = (
                SetupInstallServicesFromInfSectionExA
            )
        # END IF

        # High level routine, usually used via rundll32.dll

        # to perform right - click install action on INFs

        # May be called directly:

        # wsprintf(CmdLineBuffer,TEXT("DefaultInstall 132 %s"),InfPath);

        # InstallHinfSection(NULL,NULL,CmdLineBuffer,0);
        # VOID
        # WINAPI
        # InstallHinfSectionA(
        # _In_ HWND Window,
        # _In_ HINSTANCE ModuleHandle,
        # _In_ PCSTR CommandLine,
        # _In_ INT ShowCommand
        # );
        InstallHinfSectionA = setupapi.InstallHinfSectionA
        InstallHinfSectionA.restype = WINAPI
        # VOID
        # WINAPI
        # InstallHinfSectionW(
        # _In_ HWND Window,
        # _In_ HINSTANCE ModuleHandle,
        # _In_ PCWSTR CommandLine,
        # _In_ INT ShowCommand
        # );
        InstallHinfSectionW = setupapi.InstallHinfSectionW
        InstallHinfSectionW.restype = WINAPI

        if defined(UNICODE):
            InstallHinfSection = InstallHinfSectionW
        else:
            InstallHinfSection = InstallHinfSectionA
        # END IF

        # Define handle type for Setup file log.
        HSPFILELOG = PVOID
        # WINSETUPAPI
        # HSPFILELOG
        # WINAPI
        # SetupInitializeFileLogA(
        # _In_opt_ PCSTR LogFileName,
        # _In_ DWORD Flags
        # );
        SetupInitializeFileLogA = setupapi.SetupInitializeFileLogA
        SetupInitializeFileLogA.restype = HSPFILELOG
        # WINSETUPAPI
        # HSPFILELOG
        # WINAPI
        # SetupInitializeFileLogW(
        # _In_opt_ PCWSTR LogFileName,
        # _In_ DWORD Flags
        # );
        SetupInitializeFileLogW = setupapi.SetupInitializeFileLogW
        SetupInitializeFileLogW.restype = HSPFILELOG

        if defined(UNICODE):
            SetupInitializeFileLog = SetupInitializeFileLogW
        else:
            SetupInitializeFileLog = SetupInitializeFileLogA
        # END IF

        # Flags for SetupInitializeFileLog
        SPFILELOG_SYSTEMLOG = 0x00000001
        SPFILELOG_FORCENEW = 0x00000002
        SPFILELOG_QUERYONLY = 0x00000004
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupTerminateFileLog(
        # _In_ HSPFILELOG FileLogHandle
        # );
        SetupTerminateFileLog = setupapi.SetupTerminateFileLog
        SetupTerminateFileLog.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupLogFileA(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCSTR LogSectionName,
        # _In_ PCSTR SourceFilename,
        # _In_ PCSTR TargetFilename,
        # _In_ DWORD Checksum,
        # _In_opt_ PCSTR DiskTagfile,
        # _In_opt_ PCSTR DiskDescription,
        # _In_opt_ PCSTR OtherInfo,
        # _In_ DWORD Flags
        # );
        SetupLogFileA = setupapi.SetupLogFileA
        SetupLogFileA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupLogFileW(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCWSTR LogSectionName,
        # _In_ PCWSTR SourceFilename,
        # _In_ PCWSTR TargetFilename,
        # _In_ DWORD Checksum,
        # _In_opt_ PCWSTR DiskTagfile,
        # _In_opt_ PCWSTR DiskDescription,
        # _In_opt_ PCWSTR OtherInfo,
        # _In_ DWORD Flags
        # );
        SetupLogFileW = setupapi.SetupLogFileW
        SetupLogFileW.restype = BOOL

        if defined(UNICODE):
            SetupLogFile = SetupLogFileW
        else:
            SetupLogFile = SetupLogFileA
        # END IF

        # Flags for SetupLogFile
        SPFILELOG_OEMFILE = 0x00000001
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFileLogEntryA(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCSTR LogSectionName,
        # _In_opt_ PCSTR TargetFilename
        # );
        SetupRemoveFileLogEntryA = setupapi.SetupRemoveFileLogEntryA
        SetupRemoveFileLogEntryA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupRemoveFileLogEntryW(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCWSTR LogSectionName,
        # _In_opt_ PCWSTR TargetFilename
        # );
        SetupRemoveFileLogEntryW = setupapi.SetupRemoveFileLogEntryW
        SetupRemoveFileLogEntryW.restype = BOOL

        if defined(UNICODE):
            SetupRemoveFileLogEntry = SetupRemoveFileLogEntryW
        else:
            SetupRemoveFileLogEntry = SetupRemoveFileLogEntryA
        # END IF

        # Items retrievable from SetupQueryFileLog()


        class SetupFileLogInfo(ENUM):
            SetupFileLogSourceFilename = 1
            SetupFileLogChecksum = 2
            SetupFileLogDiskTagfile = 3
            SetupFileLogDiskDescription = 4
            SetupFileLogOtherInfo = 5
            SetupFileLogMax = 6


        SetupFileLogSourceFilename = SetupFileLogInfo.SetupFileLogSourceFilename
        SetupFileLogChecksum = SetupFileLogInfo.SetupFileLogChecksum
        SetupFileLogDiskTagfile = SetupFileLogInfo.SetupFileLogDiskTagfile
        SetupFileLogDiskDescription = SetupFileLogInfo.SetupFileLogDiskDescription
        SetupFileLogOtherInfo = SetupFileLogInfo.SetupFileLogOtherInfo
        SetupFileLogMax = SetupFileLogInfo.SetupFileLogMax
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryFileLogA(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCSTR LogSectionName,
        # _In_ PCSTR TargetFilename,
        # _In_ SetupFileLogInfo DesiredInfo,
        # _Out_writes_opt_(ReturnBufferSize) PSTR DataOut,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryFileLogA = setupapi.SetupQueryFileLogA
        SetupQueryFileLogA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupQueryFileLogW(
        # _In_ HSPFILELOG FileLogHandle,
        # _In_opt_ PCWSTR LogSectionName,
        # _In_ PCWSTR TargetFilename,
        # _In_ SetupFileLogInfo DesiredInfo,
        # _Out_writes_opt_(ReturnBufferSize) PWSTR DataOut,
        # _In_ DWORD ReturnBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupQueryFileLogW = setupapi.SetupQueryFileLogW
        SetupQueryFileLogW.restype = BOOL
        if defined(UNICODE):
            SetupQueryFileLog = SetupQueryFileLogW
        else:
            SetupQueryFileLog = SetupQueryFileLogA
        # END IF

        # Text logging APIs
        LogSeverity = DWORD
        LogSevInformation = 0x00000000
        LogSevWarning = 0x00000001
        LogSevError = 0x00000002
        LogSevFatalError = 0x00000003
        LogSevMaximum = 0x00000004

        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupLogErrorA (
        # _In_ LPCSTR MessageString,
        # _In_ LogSeverity Severity
        # );
        SetupLogErrorA = setupapi.SetupLogErrorA
        SetupLogErrorA.restype = BOOL

        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupLogErrorW (
        # _In_ LPCWSTR MessageString,
        # _In_ LogSeverity Severity
        # );
        SetupLogErrorW = setupapi.SetupLogErrorW
        SetupLogErrorW.restype = BOOL

        if defined(UNICODE):
            SetupLogError = SetupLogErrorW
        else:
            SetupLogError = SetupLogErrorA
        # END IF

        # Text log for INF debugging

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            # WINSETUPAPI
            # SP_LOG_TOKEN
            # WINAPI
            # SetupGetThreadLogToken(
            # VOID
            # );
            SetupGetThreadLogToken = setupapi.SetupGetThreadLogToken
            SetupGetThreadLogToken.restype = SP_LOG_TOKEN

            # WINSETUPAPI
            # VOID
            # WINAPI
            # SetupSetThreadLogToken(
            # _In_ SP_LOG_TOKEN LogToken
            # );
            SetupSetThreadLogToken = setupapi.SetupSetThreadLogToken
            SetupSetThreadLogToken.restype = VOID
            # WINSETUPAPI
            # VOID
            # WINAPI
            # SetupWriteTextLog(
            # _In_ SP_LOG_TOKEN LogToken,
            # _In_ DWORD Category,
            # _In_ DWORD Flags,
            # _In_ PCSTR MessageStr,
            # ...
            # );
            SetupWriteTextLog = setupapi.SetupWriteTextLog
            SetupWriteTextLog.restype = VOID
            # WINSETUPAPI
            # VOID
            # WINAPI
            # SetupWriteTextLogError(
            # _In_ SP_LOG_TOKEN LogToken,
            # _In_ DWORD Category,
            # _In_ DWORD LogFlags,
            # _In_ DWORD Error,
            # _In_ PCSTR MessageStr,
            # ...
            # );
            SetupWriteTextLogError = setupapi.SetupWriteTextLogError
            SetupWriteTextLogError.restype = VOID
            # WINSETUPAPI
            # VOID
            # WINAPI
            # SetupWriteTextLogInfLine(
            # _In_ SP_LOG_TOKEN LogToken,
            # _In_ DWORD Flags,
            # _In_ HINF InfHandle,
            # _In_ PINFCONTEXT Context
            # );
            SetupWriteTextLogInfLine = setupapi.SetupWriteTextLogInfLine
            SetupWriteTextLogInfLine.restype = VOID
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        # Backup Information API's
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetBackupInformationA(
        # _In_ HSPFILEQ QueueHandle,
        # _Inout_ PSP_BACKUP_QUEUE_PARAMS_A BackupParams
        # );
        SetupGetBackupInformationA = setupapi.SetupGetBackupInformationA
        SetupGetBackupInformationA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupGetBackupInformationW(
        # _In_ HSPFILEQ QueueHandle,
        # _Inout_ PSP_BACKUP_QUEUE_PARAMS_W BackupParams
        # );
        SetupGetBackupInformationW = setupapi.SetupGetBackupInformationW
        SetupGetBackupInformationW.restype = BOOL
        if defined(UNICODE):
            SetupGetBackupInformation = SetupGetBackupInformationW
        else:
            SetupGetBackupInformation = SetupGetBackupInformationA
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupPrepareQueueForRestoreA(
            # _In_ HSPFILEQ QueueHandle,
            # _In_ PCSTR BackupPath,
            # _In_ DWORD RestoreFlags
            # );
            SetupPrepareQueueForRestoreA = setupapi.SetupPrepareQueueForRestoreA
            SetupPrepareQueueForRestoreA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupPrepareQueueForRestoreW(
            # _In_ HSPFILEQ QueueHandle,
            # _In_ PCWSTR BackupPath,
            # _In_ DWORD RestoreFlags
            # );
            SetupPrepareQueueForRestoreW = setupapi.SetupPrepareQueueForRestoreW
            SetupPrepareQueueForRestoreW.restype = BOOL
            if defined(UNICODE):
                SetupPrepareQueueForRestore = SetupPrepareQueueForRestoreW
            else:
                SetupPrepareQueueForRestore = SetupPrepareQueueForRestoreA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # Control forcing of Non - Interactive Mode

            # Overriden if SetupAPI is run in non - interactive window session
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupSetNonInteractiveMode(
            # _In_ BOOL NonInteractiveFlag
            # );
            SetupSetNonInteractiveMode = setupapi.SetupSetNonInteractiveMode
            SetupSetNonInteractiveMode.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupGetNonInteractiveMode(
            # VOID
            # );
            SetupGetNonInteractiveMode = setupapi.SetupGetNonInteractiveMode
            SetupGetNonInteractiveMode.restype = BOOL
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Device Installer APIs
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiCreateDeviceInfoList(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ HWND hwndParent
        # );
        SetupDiCreateDeviceInfoList = setupapi.SetupDiCreateDeviceInfoList
        SetupDiCreateDeviceInfoList.restype = HDEVINFO
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiCreateDeviceInfoListExA(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ HWND hwndParent,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiCreateDeviceInfoListExA = setupapi.SetupDiCreateDeviceInfoListExA
        SetupDiCreateDeviceInfoListExA.restype = HDEVINFO
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiCreateDeviceInfoListExW(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ HWND hwndParent,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiCreateDeviceInfoListExW = setupapi.SetupDiCreateDeviceInfoListExW
        SetupDiCreateDeviceInfoListExW.restype = HDEVINFO
        if defined(UNICODE):
            SetupDiCreateDeviceInfoListEx = SetupDiCreateDeviceInfoListExW
        else:
            SetupDiCreateDeviceInfoListEx = SetupDiCreateDeviceInfoListExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInfoListClass(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Out_ LPGUID ClassGuid
        # );
        SetupDiGetDeviceInfoListClass = setupapi.SetupDiGetDeviceInfoListClass
        SetupDiGetDeviceInfoListClass.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInfoListDetailA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Out_ PSP_DEVINFO_LIST_DETAIL_DATA_A DeviceInfoSetDetailData
        # );
        SetupDiGetDeviceInfoListDetailA = setupapi.SetupDiGetDeviceInfoListDetailA
        SetupDiGetDeviceInfoListDetailA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInfoListDetailW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Out_ PSP_DEVINFO_LIST_DETAIL_DATA_W DeviceInfoSetDetailData
        # );
        SetupDiGetDeviceInfoListDetailW = setupapi.SetupDiGetDeviceInfoListDetailW
        SetupDiGetDeviceInfoListDetailW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDeviceInfoListDetail = SetupDiGetDeviceInfoListDetailW
        else:
            SetupDiGetDeviceInfoListDetail = SetupDiGetDeviceInfoListDetailA
        # END IF

        # Flags for SetupDiCreateDeviceInfo
        DICD_GENERATE_ID = 0x00000001
        DICD_INHERIT_CLASSDRVS = 0x00000002
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCreateDeviceInfoA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCSTR DeviceName,
        # _In_ CONST GUID *ClassGuid,
        # _In_opt_ PCSTR DeviceDescription,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD CreationFlags,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiCreateDeviceInfoA = setupapi.SetupDiCreateDeviceInfoA
        SetupDiCreateDeviceInfoA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCreateDeviceInfoW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCWSTR DeviceName,
        # _In_ CONST GUID *ClassGuid,
        # _In_opt_ PCWSTR DeviceDescription,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD CreationFlags,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiCreateDeviceInfoW = setupapi.SetupDiCreateDeviceInfoW
        SetupDiCreateDeviceInfoW.restype = BOOL

        if defined(UNICODE):
            SetupDiCreateDeviceInfo = SetupDiCreateDeviceInfoW
        else:
            SetupDiCreateDeviceInfo = SetupDiCreateDeviceInfoA
        # END IF

        # Flags for SetupDiOpenDeviceInfo
        DIOD_INHERIT_CLASSDRVS = 0x00000002
        DIOD_CANCEL_REMOVE = 0x00000004
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiOpenDeviceInfoA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCSTR DeviceInstanceId,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD OpenFlags,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiOpenDeviceInfoA = setupapi.SetupDiOpenDeviceInfoA
        SetupDiOpenDeviceInfoA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiOpenDeviceInfoW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCWSTR DeviceInstanceId,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD OpenFlags,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiOpenDeviceInfoW = setupapi.SetupDiOpenDeviceInfoW
        SetupDiOpenDeviceInfoW.restype = BOOL

        if defined(UNICODE):
            SetupDiOpenDeviceInfo = SetupDiOpenDeviceInfoW
        else:
            SetupDiOpenDeviceInfo = SetupDiOpenDeviceInfoA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInstanceIdA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_writes_opt_(DeviceInstanceIdSize) PSTR DeviceInstanceId,
        # _In_ DWORD DeviceInstanceIdSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDeviceInstanceIdA = setupapi.SetupDiGetDeviceInstanceIdA
        SetupDiGetDeviceInstanceIdA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInstanceIdW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_writes_opt_(DeviceInstanceIdSize) PWSTR DeviceInstanceId,
        # _In_ DWORD DeviceInstanceIdSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDeviceInstanceIdW = setupapi.SetupDiGetDeviceInstanceIdW
        SetupDiGetDeviceInstanceIdW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDeviceInstanceId = SetupDiGetDeviceInstanceIdW
        else:
            SetupDiGetDeviceInstanceId = SetupDiGetDeviceInstanceIdA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDeleteDeviceInfo(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiDeleteDeviceInfo = setupapi.SetupDiDeleteDeviceInfo
        SetupDiDeleteDeviceInfo.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiEnumDeviceInfo(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ DWORD MemberIndex,
        # _Out_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiEnumDeviceInfo = setupapi.SetupDiEnumDeviceInfo
        SetupDiEnumDeviceInfo.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDestroyDeviceInfoList(
        # _In_ HDEVINFO DeviceInfoSet
        # );
        SetupDiDestroyDeviceInfoList = setupapi.SetupDiDestroyDeviceInfoList
        SetupDiDestroyDeviceInfoList.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiEnumDeviceInterfaces(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ CONST GUID *InterfaceClassGuid,
        # _In_ DWORD MemberIndex,
        # _Out_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiEnumDeviceInterfaces = setupapi.SetupDiEnumDeviceInterfaces
        SetupDiEnumDeviceInterfaces.restype = BOOL

        # Backward compatibility - - do not use
        SetupDiEnumInterfaceDevice = SetupDiEnumDeviceInterfaces
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCreateDeviceInterfaceA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ CONST GUID *InterfaceClassGuid,
        # _In_opt_ PCSTR ReferenceString,
        # _In_ DWORD CreationFlags,
        # _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiCreateDeviceInterfaceA = setupapi.SetupDiCreateDeviceInterfaceA
        SetupDiCreateDeviceInterfaceA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCreateDeviceInterfaceW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ CONST GUID *InterfaceClassGuid,
        # _In_opt_ PCWSTR ReferenceString,
        # _In_ DWORD CreationFlags,
        # _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiCreateDeviceInterfaceW = setupapi.SetupDiCreateDeviceInterfaceW
        SetupDiCreateDeviceInterfaceW.restype = BOOL

        if defined(UNICODE):
            SetupDiCreateDeviceInterface = SetupDiCreateDeviceInterfaceW
        else:
            SetupDiCreateDeviceInterface = SetupDiCreateDeviceInterfaceA
        # END IF

        # Backward compatibility - - do not use.
        SetupDiCreateInterfaceDeviceW = SetupDiCreateDeviceInterfaceW
        SetupDiCreateInterfaceDeviceA = SetupDiCreateDeviceInterfaceA

        if defined(UNICODE):
            SetupDiCreateInterfaceDevice = SetupDiCreateDeviceInterfaceW
        else:
            SetupDiCreateInterfaceDevice = SetupDiCreateDeviceInterfaceA
        # END IF

        # Flags for SetupDiOpenDeviceInterface
        DIODI_NO_ADD = 0x00000001
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiOpenDeviceInterfaceA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCSTR DevicePath,
        # _In_ DWORD OpenFlags,
        # _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiOpenDeviceInterfaceA = setupapi.SetupDiOpenDeviceInterfaceA
        SetupDiOpenDeviceInterfaceA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiOpenDeviceInterfaceW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PCWSTR DevicePath,
        # _In_ DWORD OpenFlags,
        # _Out_opt_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiOpenDeviceInterfaceW = setupapi.SetupDiOpenDeviceInterfaceW
        SetupDiOpenDeviceInterfaceW.restype = BOOL

        if defined(UNICODE):
            SetupDiOpenDeviceInterface = SetupDiOpenDeviceInterfaceW
        else:
            SetupDiOpenDeviceInterface = SetupDiOpenDeviceInterfaceA
        # END IF

        # Backward compatibility - - do not use
        SetupDiOpenInterfaceDeviceW = SetupDiOpenDeviceInterfaceW
        SetupDiOpenInterfaceDeviceA = SetupDiOpenDeviceInterfaceA

        if defined(UNICODE):
            SetupDiOpenInterfaceDevice = SetupDiOpenDeviceInterfaceW
        else:
            SetupDiOpenInterfaceDevice = SetupDiOpenDeviceInterfaceA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInterfaceAlias(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _In_ CONST GUID *AliasInterfaceClassGuid,
        # _Out_ PSP_DEVICE_INTERFACE_DATA AliasDeviceInterfaceData
        # );
        SetupDiGetDeviceInterfaceAlias = setupapi.SetupDiGetDeviceInterfaceAlias
        SetupDiGetDeviceInterfaceAlias.restype = BOOL

        # Backward compatibility - - do not use.
        SetupDiGetInterfaceDeviceAlias = SetupDiGetDeviceInterfaceAlias
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDeleteDeviceInterfaceData(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiDeleteDeviceInterfaceData = setupapi.SetupDiDeleteDeviceInterfaceData
        SetupDiDeleteDeviceInterfaceData.restype = BOOL

        # Backward compatibility - - do not use.
        SetupDiDeleteInterfaceDeviceData = SetupDiDeleteDeviceInterfaceData
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiRemoveDeviceInterface(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData
        # );
        SetupDiRemoveDeviceInterface = setupapi.SetupDiRemoveDeviceInterface
        SetupDiRemoveDeviceInterface.restype = BOOL

        # Backward compatibility - - do not use.
        SetupDiRemoveInterfaceDevice = SetupDiRemoveDeviceInterface

        # _Success_(return != FALSE)
        # _At_((LPSTR)DeviceInterfaceDetailData->DevicePath, _Post_z_)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInterfaceDetailA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Inout_updates_bytes_to_opt_(DeviceInterfaceDetailDataSize, *RequiredSize) PSP_DEVICE_INTERFACE_DETAIL_DATA_A DeviceInterfaceDetailData,
        # _In_ DWORD DeviceInterfaceDetailDataSize,
        # _Out_opt_ _Out_range_(>=, sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA_A)) PDWORD RequiredSize,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiGetDeviceInterfaceDetailA = setupapi.SetupDiGetDeviceInterfaceDetailA
        SetupDiGetDeviceInterfaceDetailA.restype = BOOL

        # _Success_(return != FALSE)
        # _At_((LPWSTR)DeviceInterfaceDetailData->DevicePath, _Post_z_)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInterfaceDetailW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Out_writes_bytes_to_opt_(DeviceInterfaceDetailDataSize, *RequiredSize) PSP_DEVICE_INTERFACE_DETAIL_DATA_W DeviceInterfaceDetailData,
        # _In_ DWORD DeviceInterfaceDetailDataSize,
        # _Out_opt_ _Out_range_(>=, sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA_W)) PDWORD RequiredSize,
        # _Out_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiGetDeviceInterfaceDetailW = setupapi.SetupDiGetDeviceInterfaceDetailW
        SetupDiGetDeviceInterfaceDetailW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDeviceInterfaceDetail = SetupDiGetDeviceInterfaceDetailW
        else:
            SetupDiGetDeviceInterfaceDetail = SetupDiGetDeviceInterfaceDetailA
        # END IF

        # Backward compatibility - - do not use.
        SetupDiGetInterfaceDeviceDetailW = SetupDiGetDeviceInterfaceDetailW
        SetupDiGetInterfaceDeviceDetailA = SetupDiGetDeviceInterfaceDetailA

        if defined(UNICODE):
            SetupDiGetInterfaceDeviceDetail = SetupDiGetDeviceInterfaceDetailW
        else:
            SetupDiGetInterfaceDeviceDetail = SetupDiGetDeviceInterfaceDetailA
        # END IF

        # Default install handler for DIF_INSTALLINTERFACES.
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallDeviceInterfaces(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiInstallDeviceInterfaces = setupapi.SetupDiInstallDeviceInterfaces
        SetupDiInstallDeviceInterfaces.restype = BOOL

        # Backward compatibility - - do not use.
        SetupDiInstallInterfaceDevices = SetupDiInstallDeviceInterfaces

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetDeviceInterfaceDefault(
            # _In_ HDEVINFO DeviceInfoSet,
            # _Inout_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
            # _In_ DWORD Flags,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiSetDeviceInterfaceDefault = setupapi.SetupDiSetDeviceInterfaceDefault
            SetupDiSetDeviceInterfaceDefault.restype = BOOL
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        # Default install handler for DIF_REGISTERDEVICE

        # Flags for SetupDiRegisterDeviceInfo
        SPRDI_FIND_DUPS = 0x00000001
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiRegisterDeviceInfo(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Flags,
        # _In_opt_ PSP_DETSIG_CMPPROC CompareProc,
        # _In_opt_ PVOID CompareContext,
        # _Out_opt_ PSP_DEVINFO_DATA DupDeviceInfoData
        # );
        SetupDiRegisterDeviceInfo = setupapi.SetupDiRegisterDeviceInfo
        SetupDiRegisterDeviceInfo.restype = BOOL

        # Ordinal values distinguishing between class drivers and

        # device drivers.

        # (Passed in 'DriverType' parameter of driver information list APIs)
        SPDIT_NODRIVER = 0x00000000
        SPDIT_CLASSDRIVER = 0x00000001
        SPDIT_COMPATDRIVER = 0x00000002
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiBuildDriverInfoList(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD DriverType
        # );
        SetupDiBuildDriverInfoList = setupapi.SetupDiBuildDriverInfoList
        SetupDiBuildDriverInfoList.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCancelDriverInfoSearch(
        # _In_ HDEVINFO DeviceInfoSet
        # );
        SetupDiCancelDriverInfoSearch = setupapi.SetupDiCancelDriverInfoSearch
        SetupDiCancelDriverInfoSearch.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiEnumDriverInfoA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD DriverType,
        # _In_ DWORD MemberIndex,
        # _Out_ PSP_DRVINFO_DATA_A DriverInfoData
        # );
        SetupDiEnumDriverInfoA = setupapi.SetupDiEnumDriverInfoA
        SetupDiEnumDriverInfoA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiEnumDriverInfoW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD DriverType,
        # _In_ DWORD MemberIndex,
        # _Out_ PSP_DRVINFO_DATA_W DriverInfoData
        # );
        SetupDiEnumDriverInfoW = setupapi.SetupDiEnumDriverInfoW
        SetupDiEnumDriverInfoW.restype = BOOL

        if defined(UNICODE):
            SetupDiEnumDriverInfo = SetupDiEnumDriverInfoW
        else:
            SetupDiEnumDriverInfo = SetupDiEnumDriverInfoA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetSelectedDriverA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_ PSP_DRVINFO_DATA_A DriverInfoData
        # );
        SetupDiGetSelectedDriverA = setupapi.SetupDiGetSelectedDriverA
        SetupDiGetSelectedDriverA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetSelectedDriverW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_ PSP_DRVINFO_DATA_W DriverInfoData
        # );
        SetupDiGetSelectedDriverW = setupapi.SetupDiGetSelectedDriverW
        SetupDiGetSelectedDriverW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetSelectedDriver = SetupDiGetSelectedDriverW
        else:
            SetupDiGetSelectedDriver = SetupDiGetSelectedDriverA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetSelectedDriverA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Inout_opt_ PSP_DRVINFO_DATA_A DriverInfoData
        # );
        SetupDiSetSelectedDriverA = setupapi.SetupDiSetSelectedDriverA
        SetupDiSetSelectedDriverA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetSelectedDriverW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Inout_opt_ PSP_DRVINFO_DATA_W DriverInfoData
        # );
        SetupDiSetSelectedDriverW = setupapi.SetupDiSetSelectedDriverW
        SetupDiSetSelectedDriverW.restype = BOOL

        if defined(UNICODE):
            SetupDiSetSelectedDriver = SetupDiSetSelectedDriverW
        else:
            SetupDiSetSelectedDriver = SetupDiSetSelectedDriverA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDriverInfoDetailA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_A DriverInfoData,
        # _Inout_updates_bytes_opt_(DriverInfoDetailDataSize) PSP_DRVINFO_DETAIL_DATA_A DriverInfoDetailData,
        # _In_ DWORD DriverInfoDetailDataSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDriverInfoDetailA = setupapi.SetupDiGetDriverInfoDetailA
        SetupDiGetDriverInfoDetailA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDriverInfoDetailW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_W DriverInfoData,
        # _Inout_updates_bytes_opt_(DriverInfoDetailDataSize) PSP_DRVINFO_DETAIL_DATA_W DriverInfoDetailData,
        # _In_ DWORD DriverInfoDetailDataSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDriverInfoDetailW = setupapi.SetupDiGetDriverInfoDetailW
        SetupDiGetDriverInfoDetailW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDriverInfoDetail = SetupDiGetDriverInfoDetailW
        else:
            SetupDiGetDriverInfoDetail = SetupDiGetDriverInfoDetailA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDestroyDriverInfoList(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD DriverType
        # );
        SetupDiDestroyDriverInfoList = setupapi.SetupDiDestroyDriverInfoList
        SetupDiDestroyDriverInfoList.restype = BOOL

        # Flags controlling what is included in the device information set
        # built

        # by SetupDiGetClassDevs
        DIGCF_DEFAULT = 0x00000001
        DIGCF_PRESENT = 0x00000002
        DIGCF_ALLCLASSES = 0x00000004
        DIGCF_PROFILE = 0x00000008
        DIGCF_DEVICEINTERFACE = 0x00000010

        # Backward compatibility - - do not use.
        DIGCF_INTERFACEDEVICE = DIGCF_DEVICEINTERFACE
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiGetClassDevsA(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ PCSTR Enumerator,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD Flags
        # );
        SetupDiGetClassDevsA = setupapi.SetupDiGetClassDevsA
        SetupDiGetClassDevsA.restype = HDEVINFO
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiGetClassDevsW(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ PCWSTR Enumerator,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD Flags
        # );
        SetupDiGetClassDevsW = setupapi.SetupDiGetClassDevsW
        SetupDiGetClassDevsW.restype = HDEVINFO

        if defined(UNICODE):
            SetupDiGetClassDevs = SetupDiGetClassDevsW
        else:
            SetupDiGetClassDevs = SetupDiGetClassDevsA
        # END IF
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiGetClassDevsExA(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ PCSTR Enumerator,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD Flags,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassDevsExA = setupapi.SetupDiGetClassDevsExA
        SetupDiGetClassDevsExA.restype = HDEVINFO
        # _Check_return_
        # WINSETUPAPI
        # HDEVINFO
        # WINAPI
        # SetupDiGetClassDevsExW(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_opt_ PCWSTR Enumerator,
        # _In_opt_ HWND hwndParent,
        # _In_ DWORD Flags,
        # _In_opt_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassDevsExW = setupapi.SetupDiGetClassDevsExW
        SetupDiGetClassDevsExW.restype = HDEVINFO

        if defined(UNICODE):
            SetupDiGetClassDevsEx = SetupDiGetClassDevsExW
        else:
            SetupDiGetClassDevsEx = SetupDiGetClassDevsExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetINFClassA(
        # _In_ PCSTR InfName,
        # _Out_ LPGUID ClassGuid,
        # _Out_writes_(ClassNameSize) PSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetINFClassA = setupapi.SetupDiGetINFClassA
        SetupDiGetINFClassA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetINFClassW(
        # _In_ PCWSTR InfName,
        # _Out_ LPGUID ClassGuid,
        # _Out_writes_(ClassNameSize) PWSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetINFClassW = setupapi.SetupDiGetINFClassW
        SetupDiGetINFClassW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetINFClass = SetupDiGetINFClassW
        else:
            SetupDiGetINFClass = SetupDiGetINFClassA
        # END IF

        # Flags controlling exclusion from the class information list built

        # by SetupDiBuildClassInfoList(Ex)
        DIBCI_NOINSTALLCLASS = 0x00000001
        DIBCI_NODISPLAYCLASS = 0x00000002

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiBuildClassInfoList(
        # _In_ DWORD Flags,
        # _Out_writes_to_opt_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize
        # );
        SetupDiBuildClassInfoList = setupapi.SetupDiBuildClassInfoList
        SetupDiBuildClassInfoList.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiBuildClassInfoListExA(
        # _In_ DWORD Flags,
        # _Out_writes_to_opt_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiBuildClassInfoListExA = setupapi.SetupDiBuildClassInfoListExA
        SetupDiBuildClassInfoListExA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiBuildClassInfoListExW(
        # _In_ DWORD Flags,
        # _Out_writes_to_opt_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiBuildClassInfoListExW = setupapi.SetupDiBuildClassInfoListExW
        SetupDiBuildClassInfoListExW.restype = BOOL

        if defined(UNICODE):
            SetupDiBuildClassInfoListEx = SetupDiBuildClassInfoListExW
        else:
            SetupDiBuildClassInfoListEx = SetupDiBuildClassInfoListExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDescriptionA(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassDescriptionSize) PSTR ClassDescription,
        # _In_ DWORD ClassDescriptionSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetClassDescriptionA = setupapi.SetupDiGetClassDescriptionA
        SetupDiGetClassDescriptionA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDescriptionW(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassDescriptionSize) PWSTR ClassDescription,
        # _In_ DWORD ClassDescriptionSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetClassDescriptionW = setupapi.SetupDiGetClassDescriptionW
        SetupDiGetClassDescriptionW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetClassDescription = SetupDiGetClassDescriptionW
        else:
            SetupDiGetClassDescription = SetupDiGetClassDescriptionA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDescriptionExA(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassDescriptionSize) PSTR ClassDescription,
        # _In_ DWORD ClassDescriptionSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassDescriptionExA = setupapi.SetupDiGetClassDescriptionExA
        SetupDiGetClassDescriptionExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDescriptionExW(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassDescriptionSize) PWSTR ClassDescription,
        # _In_ DWORD ClassDescriptionSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassDescriptionExW = setupapi.SetupDiGetClassDescriptionExW
        SetupDiGetClassDescriptionExW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetClassDescriptionEx = SetupDiGetClassDescriptionExW
        else:
            SetupDiGetClassDescriptionEx = SetupDiGetClassDescriptionExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiCallClassInstaller(
        # _In_ DI_FUNCTION InstallFunction,
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiCallClassInstaller = setupapi.SetupDiCallClassInstaller
        SetupDiCallClassInstaller.restype = BOOL

        # Default install handler for DIF_SELECTDEVICE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSelectDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiSelectDevice = setupapi.SetupDiSelectDevice
        SetupDiSelectDevice.restype = BOOL

        # Default install handler for DIF_SELECTBESTCOMPATDRV
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSelectBestCompatDrv(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiSelectBestCompatDrv = setupapi.SetupDiSelectBestCompatDrv
        SetupDiSelectBestCompatDrv.restype = BOOL

        # Default install handler for DIF_INSTALLDEVICE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiInstallDevice = setupapi.SetupDiInstallDevice
        SetupDiInstallDevice.restype = BOOL

        # Default install handler for DIF_INSTALLDEVICEFILES
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallDriverFiles(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiInstallDriverFiles = setupapi.SetupDiInstallDriverFiles
        SetupDiInstallDriverFiles.restype = BOOL

        # Default install handler for DIF_REGISTER_COINSTALLERS
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiRegisterCoDeviceInstallers(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiRegisterCoDeviceInstallers = setupapi.SetupDiRegisterCoDeviceInstallers
        SetupDiRegisterCoDeviceInstallers.restype = BOOL

        # Default install handler for DIF_REMOVE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiRemoveDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiRemoveDevice = setupapi.SetupDiRemoveDevice
        SetupDiRemoveDevice.restype = BOOL

        # Default install handler for DIF_UNREMOVE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiUnremoveDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiUnremoveDevice = setupapi.SetupDiUnremoveDevice
        SetupDiUnremoveDevice.restype = BOOL

        if _SETUPAPI_VER >= _WIN32_WINNT_WS03:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiRestartDevices(
            # _In_ HDEVINFO DeviceInfoSet,
            # _Inout_ PSP_DEVINFO_DATA DeviceInfoData
            # );
            SetupDiRestartDevices = setupapi.SetupDiRestartDevices
            SetupDiRestartDevices.restype = BOOL
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WS03

        # Default install handler for DIF_PROPERTYCHANGE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiChangeState(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiChangeState = setupapi.SetupDiChangeState
        SetupDiChangeState.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallClassA(
        # _In_opt_ HWND hwndParent,
        # _In_ PCSTR InfFileName,
        # _In_ DWORD Flags,
        # _In_opt_ HSPFILEQ FileQueue
        # );
        SetupDiInstallClassA = setupapi.SetupDiInstallClassA
        SetupDiInstallClassA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallClassW(
        # _In_opt_ HWND hwndParent,
        # _In_ PCWSTR InfFileName,
        # _In_ DWORD Flags,
        # _In_opt_ HSPFILEQ FileQueue
        # );
        SetupDiInstallClassW = setupapi.SetupDiInstallClassW
        SetupDiInstallClassW.restype = BOOL
        if defined(UNICODE):
            SetupDiInstallClass = SetupDiInstallClassW
        else:
            SetupDiInstallClass = SetupDiInstallClassA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallClassExA(
        # _In_opt_ HWND hwndParent,
        # _In_opt_ PCSTR InfFileName,
        # _In_ DWORD Flags,
        # _In_opt_ HSPFILEQ FileQueue,
        # _In_opt_ CONST GUID *InterfaceClassGuid,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupDiInstallClassExA = setupapi.SetupDiInstallClassExA
        SetupDiInstallClassExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiInstallClassExW(
        # _In_opt_ HWND hwndParent,
        # _In_opt_ PCWSTR InfFileName,
        # _In_ DWORD Flags,
        # _In_opt_ HSPFILEQ FileQueue,
        # _In_opt_ CONST GUID *InterfaceClassGuid,
        # _Reserved_ PVOID Reserved1,
        # _Reserved_ PVOID Reserved2
        # );
        SetupDiInstallClassExW = setupapi.SetupDiInstallClassExW
        SetupDiInstallClassExW.restype = BOOL

        if defined(UNICODE):
            SetupDiInstallClassEx = SetupDiInstallClassExW
        else:
            SetupDiInstallClassEx = SetupDiInstallClassExA
        # END IF
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiOpenClassRegKey(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_ REGSAM samDesired
        # );
        SetupDiOpenClassRegKey = setupapi.SetupDiOpenClassRegKey
        SetupDiOpenClassRegKey.restype = HKEY

        # Flags for SetupDiOpenClassRegKeyEx
        DIOCR_INSTALLER = 0x00000001
        DIOCR_INTERFACE = 0x00000002
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiOpenClassRegKeyExA(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_ REGSAM samDesired,
        # _In_ DWORD Flags,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiOpenClassRegKeyExA = setupapi.SetupDiOpenClassRegKeyExA
        SetupDiOpenClassRegKeyExA.restype = HKEY
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiOpenClassRegKeyExW(
        # _In_opt_ CONST GUID *ClassGuid,
        # _In_ REGSAM samDesired,
        # _In_ DWORD Flags,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiOpenClassRegKeyExW = setupapi.SetupDiOpenClassRegKeyExW
        SetupDiOpenClassRegKeyExW.restype = HKEY

        if defined(UNICODE):
            SetupDiOpenClassRegKeyEx = SetupDiOpenClassRegKeyExW
        else:
            SetupDiOpenClassRegKeyEx = SetupDiOpenClassRegKeyExA
        # END IF
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiCreateDeviceInterfaceRegKeyA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Reserved_ DWORD Reserved,
        # _In_ REGSAM samDesired,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCSTR InfSectionName
        # );
        SetupDiCreateDeviceInterfaceRegKeyA = setupapi.SetupDiCreateDeviceInterfaceRegKeyA
        SetupDiCreateDeviceInterfaceRegKeyA.restype = HKEY
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiCreateDeviceInterfaceRegKeyW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Reserved_ DWORD Reserved,
        # _In_ REGSAM samDesired,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCWSTR InfSectionName
        # );
        SetupDiCreateDeviceInterfaceRegKeyW = setupapi.SetupDiCreateDeviceInterfaceRegKeyW
        SetupDiCreateDeviceInterfaceRegKeyW.restype = HKEY

        if defined(UNICODE):
            SetupDiCreateDeviceInterfaceRegKey = (
                SetupDiCreateDeviceInterfaceRegKeyW
            )
        else:
            SetupDiCreateDeviceInterfaceRegKey = (
                SetupDiCreateDeviceInterfaceRegKeyA
            )
        # END IF

        # Backward compatibility - - do not use.
        SetupDiCreateInterfaceDeviceRegKeyW = (
            SetupDiCreateDeviceInterfaceRegKeyW
        )
        SetupDiCreateInterfaceDeviceRegKeyA = (
            SetupDiCreateDeviceInterfaceRegKeyA
        )

        if defined(UNICODE):
            SetupDiCreateInterfaceDeviceRegKey = (
                SetupDiCreateDeviceInterfaceRegKeyW
            )
        else:
            SetupDiCreateInterfaceDeviceRegKey = (
                SetupDiCreateDeviceInterfaceRegKeyA
            )
        # END IF
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiOpenDeviceInterfaceRegKey(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Reserved_ DWORD Reserved,
        # _In_ REGSAM samDesired
        # );
        SetupDiOpenDeviceInterfaceRegKey = setupapi.SetupDiOpenDeviceInterfaceRegKey
        SetupDiOpenDeviceInterfaceRegKey.restype = HKEY

        # Backward compatibility - - do not use.
        SetupDiOpenInterfaceDeviceRegKey = SetupDiOpenDeviceInterfaceRegKey
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDeleteDeviceInterfaceRegKey(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
        # _Reserved_ DWORD Reserved
        # );
        SetupDiDeleteDeviceInterfaceRegKey = setupapi.SetupDiDeleteDeviceInterfaceRegKey
        SetupDiDeleteDeviceInterfaceRegKey.restype = BOOL

        # Backward compatibility - - do not use.
        SetupDiDeleteInterfaceDeviceRegKey = SetupDiDeleteDeviceInterfaceRegKey

        # KeyType values for SetupDiCreateDevRegKey, SetupDiOpenDevRegKey, and

        # SetupDiDeleteDevRegKey.
        DIREG_DEV = 0x00000001
        DIREG_DRV = 0x00000002
        DIREG_BOTH = 0x00000004
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiCreateDevRegKeyA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Scope,
        # _In_ DWORD HwProfile,
        # _In_ DWORD KeyType,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCSTR InfSectionName
        # );
        SetupDiCreateDevRegKeyA = setupapi.SetupDiCreateDevRegKeyA
        SetupDiCreateDevRegKeyA.restype = HKEY
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiCreateDevRegKeyW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Scope,
        # _In_ DWORD HwProfile,
        # _In_ DWORD KeyType,
        # _In_opt_ HINF InfHandle,
        # _In_opt_ PCWSTR InfSectionName
        # );
        SetupDiCreateDevRegKeyW = setupapi.SetupDiCreateDevRegKeyW
        SetupDiCreateDevRegKeyW.restype = HKEY

        if defined(UNICODE):
            SetupDiCreateDevRegKey = SetupDiCreateDevRegKeyW
        else:
            SetupDiCreateDevRegKey = SetupDiCreateDevRegKeyA
        # END IF
        # _Check_return_
        # WINSETUPAPI
        # HKEY
        # WINAPI
        # SetupDiOpenDevRegKey(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Scope,
        # _In_ DWORD HwProfile,
        # _In_ DWORD KeyType,
        # _In_ REGSAM samDesired
        # );
        SetupDiOpenDevRegKey = setupapi.SetupDiOpenDevRegKey
        SetupDiOpenDevRegKey.restype = HKEY
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDeleteDevRegKey(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Scope,
        # _In_ DWORD HwProfile,
        # _In_ DWORD KeyType
        # );
        SetupDiDeleteDevRegKey = setupapi.SetupDiDeleteDevRegKey
        SetupDiDeleteDevRegKey.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileList(
        # _Out_writes_to_(HwProfileListSize, *RequiredSize) PDWORD HwProfileList,
        # _In_ DWORD HwProfileListSize,
        # _Out_ PDWORD RequiredSize,
        # _Out_opt_ PDWORD CurrentlyActiveIndex
        # );
        SetupDiGetHwProfileList = setupapi.SetupDiGetHwProfileList
        SetupDiGetHwProfileList.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileListExA(
        # _Out_writes_to_(HwProfileListSize, *RequiredSize) PDWORD HwProfileList,
        # _In_ DWORD HwProfileListSize,
        # _Out_ PDWORD RequiredSize,
        # _Out_opt_ PDWORD CurrentlyActiveIndex,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetHwProfileListExA = setupapi.SetupDiGetHwProfileListExA
        SetupDiGetHwProfileListExA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileListExW(
        # _Out_writes_to_(HwProfileListSize, *RequiredSize) PDWORD HwProfileList,
        # _In_ DWORD HwProfileListSize,
        # _Out_ PDWORD RequiredSize,
        # _Out_opt_ PDWORD CurrentlyActiveIndex,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetHwProfileListExW = setupapi.SetupDiGetHwProfileListExW
        SetupDiGetHwProfileListExW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetHwProfileListEx = SetupDiGetHwProfileListExW
        else:
            SetupDiGetHwProfileListEx = SetupDiGetHwProfileListExA
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetDevicePropertyKeys(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVINFO_DATA DeviceInfoData,
            # _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
            # _In_         DWORD            PropertyKeyCount,
            # _Out_opt_    PDWORD           RequiredPropertyKeyCount,
            # _In_         DWORD            Flags
            # );
            SetupDiGetDevicePropertyKeys = setupapi.SetupDiGetDevicePropertyKeys
            SetupDiGetDevicePropertyKeys.restype = BOOL

            # _Success_(return != FALSE)
            # _When_(*PropertyType == DEVPROP_TYPE_STRING, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_INDIRECT, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST, _At_((PZZWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetDevicePropertyW(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVINFO_DATA DeviceInfoData,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _Out_        DEVPROPTYPE     *PropertyType,
            # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _Out_opt_    PDWORD           RequiredSize,
            # _In_         DWORD            Flags
            # );
            SetupDiGetDevicePropertyW = setupapi.SetupDiGetDevicePropertyW
            SetupDiGetDevicePropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetDeviceProperty = SetupDiGetDevicePropertyW
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetDevicePropertyW(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVINFO_DATA DeviceInfoData,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _In_         DEVPROPTYPE      PropertyType,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _In_         DWORD            Flags
            # );
            SetupDiSetDevicePropertyW = setupapi.SetupDiSetDevicePropertyW
            SetupDiSetDevicePropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiSetDeviceProperty = SetupDiSetDevicePropertyW
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetDeviceInterfacePropertyKeys(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
            # _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
            # _In_         DWORD            PropertyKeyCount,
            # _Out_opt_    PDWORD           RequiredPropertyKeyCount,
            # _In_         DWORD            Flags
            # );
            SetupDiGetDeviceInterfacePropertyKeys = setupapi.SetupDiGetDeviceInterfacePropertyKeys
            SetupDiGetDeviceInterfacePropertyKeys.restype = BOOL

            # _Success_(return != FALSE)
            # _When_(*PropertyType == DEVPROP_TYPE_STRING, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_INDIRECT, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST, _At_((PZZWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetDeviceInterfacePropertyW(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _Out_        DEVPROPTYPE     *PropertyType,
            # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _Out_opt_    PDWORD           RequiredSize,
            # _In_         DWORD            Flags
            # );
            SetupDiGetDeviceInterfacePropertyW = setupapi.SetupDiGetDeviceInterfacePropertyW
            SetupDiGetDeviceInterfacePropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetDeviceInterfaceProperty = (
                    SetupDiGetDeviceInterfacePropertyW
                )
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetDeviceInterfacePropertyW(
            # _In_         HDEVINFO         DeviceInfoSet,
            # _In_         PSP_DEVICE_INTERFACE_DATA DeviceInterfaceData,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _In_         DEVPROPTYPE      PropertyType,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _In_         DWORD            Flags
            # );
            SetupDiSetDeviceInterfacePropertyW = setupapi.SetupDiSetDeviceInterfacePropertyW
            SetupDiSetDeviceInterfacePropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiSetDeviceInterfaceProperty = (
                    SetupDiSetDeviceInterfacePropertyW
                )
            # END IF

            # Flags for SetupDiGetClassPropertyKeys, SetupDiGetClassProperty,
            # and

            # SetupDiSetClassProperty.
            DICLASSPROP_INSTALLER = 0x00000001
            DICLASSPROP_INTERFACE = 0x00000002
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassPropertyKeys(
            # _In_   CONST GUID            *ClassGuid,
            # _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
            # _In_         DWORD            PropertyKeyCount,
            # _Out_opt_    PDWORD           RequiredPropertyKeyCount,
            # _In_         DWORD            Flags
            # );
            SetupDiGetClassPropertyKeys = setupapi.SetupDiGetClassPropertyKeys
            SetupDiGetClassPropertyKeys.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassPropertyKeysExW(
            # _In_   CONST GUID            *ClassGuid,
            # _Out_writes_opt_(PropertyKeyCount) DEVPROPKEY *PropertyKeyArray,
            # _In_         DWORD            PropertyKeyCount,
            # _Out_opt_    PDWORD           RequiredPropertyKeyCount,
            # _In_         DWORD            Flags,
            # _In_opt_     PCWSTR           MachineName,
            # _Reserved_   PVOID            Reserved
            # );
            SetupDiGetClassPropertyKeysExW = setupapi.SetupDiGetClassPropertyKeysExW
            SetupDiGetClassPropertyKeysExW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetClassPropertyKeysEx = SetupDiGetClassPropertyKeysExW
            # END IF

            # _Success_(return != FALSE)
            # _When_(*PropertyType == DEVPROP_TYPE_STRING, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_INDIRECT, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST, _At_((PZZWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassPropertyW(
            # _In_   CONST GUID            *ClassGuid,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _Out_        DEVPROPTYPE     *PropertyType,
            # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _Out_opt_    PDWORD           RequiredSize,
            # _In_         DWORD            Flags
            # );
            SetupDiGetClassPropertyW = setupapi.SetupDiGetClassPropertyW
            SetupDiGetClassPropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetClassProperty = SetupDiGetClassPropertyW
            # END IF

            # _Success_(return != FALSE)
            # _When_(*PropertyType == DEVPROP_TYPE_STRING, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_INDIRECT, _At_((PWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # _When_(*PropertyType == DEVPROP_TYPE_STRING_LIST, _At_((PZZWSTR) PropertyBuffer, _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize)))
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassPropertyExW(
            # _In_   CONST GUID            *ClassGuid,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _Out_        DEVPROPTYPE     *PropertyType,
            # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _Out_opt_    PDWORD           RequiredSize,
            # _In_         DWORD            Flags,
            # _In_opt_     PCWSTR           MachineName,
            # _Reserved_   PVOID            Reserved
            # );

            SetupDiGetClassPropertyExW = setupapi.SetupDiGetClassPropertyExW
            SetupDiGetClassPropertyExW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetClassPropertyEx = SetupDiGetClassPropertyExW
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetClassPropertyW(
            # _In_   CONST GUID            *ClassGuid,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _In_         DEVPROPTYPE      PropertyType,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _In_         DWORD            Flags
            # );
            SetupDiSetClassPropertyW = setupapi.SetupDiSetClassPropertyW
            SetupDiSetClassPropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiSetClassProperty = SetupDiSetClassPropertyW
            # END IF
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetClassPropertyExW(
            # _In_   CONST GUID            *ClassGuid,
            # _In_   CONST DEVPROPKEY      *PropertyKey,
            # _In_         DEVPROPTYPE      PropertyType,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST PBYTE PropertyBuffer,
            # _In_         DWORD            PropertyBufferSize,
            # _In_         DWORD            Flags,
            # _In_opt_     PCWSTR           MachineName,
            # _Reserved_   PVOID            Reserved
            # );
            SetupDiSetClassPropertyExW = setupapi.SetupDiSetClassPropertyExW
            SetupDiSetClassPropertyExW.restype = BOOL

            if defined(UNICODE):
                SetupDiSetClassPropertyEx = SetupDiSetClassPropertyExW
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        # Device registry property codes
        # (Codes marked as read - only (R) may only be used for
        # SetupDiGetDeviceRegistryProperty)
        # These values should cover the same set of registry properties
        # as defined by the CM_DRP codes in cfgmgr32.h.
        # Note that SPDRP codes are zero based while CM_DRP codes are one
        # basednot
        SPDRP_DEVICEDESC = 0x00000000
        SPDRP_HARDWAREID = 0x00000001
        SPDRP_COMPATIBLEIDS = 0x00000002
        SPDRP_UNUSED0 = 0x00000003
        SPDRP_SERVICE = 0x00000004
        SPDRP_UNUSED1 = 0x00000005
        SPDRP_UNUSED2 = 0x00000006
        SPDRP_CLASS = 0x00000007
        SPDRP_CLASSGUID = 0x00000008
        SPDRP_DRIVER = 0x00000009
        SPDRP_CONFIGFLAGS = 0x0000000A
        SPDRP_MFG = 0x0000000B
        SPDRP_FRIENDLYNAME = 0x0000000C
        SPDRP_LOCATION_INFORMATION = 0x0000000D
        SPDRP_PHYSICAL_DEVICE_OBJECT_NAME = 0x0000000E
        SPDRP_CAPABILITIES = 0x0000000F
        SPDRP_UI_NUMBER = 0x00000010
        SPDRP_UPPERFILTERS = 0x00000011
        SPDRP_LOWERFILTERS = 0x00000012
        SPDRP_BUSTYPEGUID = 0x00000013
        SPDRP_LEGACYBUSTYPE = 0x00000014
        SPDRP_BUSNUMBER = 0x00000015
        SPDRP_ENUMERATOR_NAME = 0x00000016
        SPDRP_SECURITY = 0x00000017
        SPDRP_SECURITY_SDS = 0x00000018
        SPDRP_DEVTYPE = 0x00000019
        SPDRP_EXCLUSIVE = 0x0000001A
        SPDRP_CHARACTERISTICS = 0x0000001B
        SPDRP_ADDRESS = 0x0000001C
        SPDRP_UI_NUMBER_DESC_FORMAT = 0x0000001D
        SPDRP_DEVICE_POWER_DATA = 0x0000001E
        SPDRP_REMOVAL_POLICY = 0x0000001F
        SPDRP_REMOVAL_POLICY_HW_DEFAULT = 0x00000020
        SPDRP_REMOVAL_POLICY_OVERRIDE = 0x00000021
        SPDRP_INSTALL_STATE = 0x00000022
        SPDRP_LOCATION_PATHS = 0x00000023
        SPDRP_BASE_CONTAINERID = 0x00000024
        SPDRP_MAXIMUM_PROPERTY = 0x00000025

        # Class registry property codes
        # (Codes marked as read - only (R) may only be used for
        # SetupDiGetClassRegistryProperty)
        # These values should cover the same set of registry properties
        # as defined by the CM_CRP codes in cfgmgr32.h.
        # they should also have a 1:1 correspondence with Device registers,
        # where applicable

        # but no overlap otherwise
        SPCRP_UPPERFILTERS = 0x00000011
        SPCRP_LOWERFILTERS = 0x00000012
        SPCRP_SECURITY = 0x00000017
        SPCRP_SECURITY_SDS = 0x00000018
        SPCRP_DEVTYPE = 0x00000019
        SPCRP_EXCLUSIVE = 0x0000001A
        SPCRP_CHARACTERISTICS = 0x0000001B
        SPCRP_MAXIMUM_PROPERTY = 0x0000001C

        # _Success_(return != FALSE)
        # _When_((*PropertyRegDataType == REG_SZ), _At_((PSTR) PropertyBuffer, _Post_valid_))
        # _When_((*PropertyRegDataType == REG_MULTI_SZ), _At_((PZZSTR) PropertyBuffer, _Post_valid_))
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceRegistryPropertyA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Property,
        # _Out_opt_ PDWORD PropertyRegDataType,
        # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
        # _In_ DWORD PropertyBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDeviceRegistryPropertyA = setupapi.SetupDiGetDeviceRegistryPropertyA
        SetupDiGetDeviceRegistryPropertyA.restype = BOOL

        # _Success_(return != FALSE)
        # _When_((*PropertyRegDataType == REG_SZ), _At_((PWSTR) PropertyBuffer, _Post_valid_))
        # _When_((*PropertyRegDataType == REG_MULTI_SZ), _At_((PZZWSTR) PropertyBuffer, _Post_valid_))
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceRegistryPropertyW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Property,
        # _Out_opt_ PDWORD PropertyRegDataType,
        # _Out_writes_bytes_to_opt_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
        # _In_ DWORD PropertyBufferSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetDeviceRegistryPropertyW = setupapi.SetupDiGetDeviceRegistryPropertyW
        SetupDiGetDeviceRegistryPropertyW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDeviceRegistryProperty = (
                SetupDiGetDeviceRegistryPropertyW
            )
        else:
            SetupDiGetDeviceRegistryProperty = (
                SetupDiGetDeviceRegistryPropertyA
            )
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # _Success_(return != FALSE)
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassRegistryPropertyA(
            # _In_ CONST GUID *ClassGuid,
            # _In_ DWORD Property,
            # _Out_opt_ PDWORD PropertyRegDataType,
            # _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _In_opt_ PCSTR MachineName,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetClassRegistryPropertyA = setupapi.SetupDiGetClassRegistryPropertyA
            SetupDiGetClassRegistryPropertyA.restype = BOOL

            # _Success_(return != FALSE)
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetClassRegistryPropertyW(
            # _In_ CONST GUID *ClassGuid,
            # _In_ DWORD Property,
            # _Out_opt_ PDWORD PropertyRegDataType,
            # _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _In_opt_ PCWSTR MachineName,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetClassRegistryPropertyW = setupapi.SetupDiGetClassRegistryPropertyW
            SetupDiGetClassRegistryPropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetClassRegistryProperty = (
                    SetupDiGetClassRegistryPropertyW
                )
            else:
                SetupDiGetClassRegistryProperty = (
                    SetupDiGetClassRegistryPropertyA
                )
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDeviceRegistryPropertyA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Property,
        # _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
        # _In_ DWORD PropertyBufferSize
        # );
        SetupDiSetDeviceRegistryPropertyA = setupapi.SetupDiSetDeviceRegistryPropertyA
        SetupDiSetDeviceRegistryPropertyA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDeviceRegistryPropertyW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ DWORD Property,
        # _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
        # _In_ DWORD PropertyBufferSize
        # );
        SetupDiSetDeviceRegistryPropertyW = setupapi.SetupDiSetDeviceRegistryPropertyW
        SetupDiSetDeviceRegistryPropertyW.restype = BOOL

        if defined(UNICODE):
            SetupDiSetDeviceRegistryProperty = (
                SetupDiSetDeviceRegistryPropertyW
            )
        else:
            SetupDiSetDeviceRegistryProperty = (
                SetupDiSetDeviceRegistryPropertyA
            )
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetClassRegistryPropertyA(
            # _In_ CONST GUID *ClassGuid,
            # _In_ DWORD Property,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _In_opt_ PCSTR MachineName,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiSetClassRegistryPropertyA = setupapi.SetupDiSetClassRegistryPropertyA
            SetupDiSetClassRegistryPropertyA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiSetClassRegistryPropertyW(
            # _In_ CONST GUID *ClassGuid,
            # _In_ DWORD Property,
            # _In_reads_bytes_opt_(PropertyBufferSize) CONST BYTE *PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _In_opt_ PCWSTR MachineName,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiSetClassRegistryPropertyW = setupapi.SetupDiSetClassRegistryPropertyW
            SetupDiSetClassRegistryPropertyW.restype = BOOL
            if defined(UNICODE):
                SetupDiSetClassRegistryProperty = (
                    SetupDiSetClassRegistryPropertyW
                )
            else:
                SetupDiSetClassRegistryProperty = (
                    SetupDiSetClassRegistryPropertyA
                )
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_ PSP_DEVINSTALL_PARAMS_A DeviceInstallParams
        # );
        SetupDiGetDeviceInstallParamsA = setupapi.SetupDiGetDeviceInstallParamsA
        SetupDiGetDeviceInstallParamsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDeviceInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_ PSP_DEVINSTALL_PARAMS_W DeviceInstallParams
        # );
        SetupDiGetDeviceInstallParamsW = setupapi.SetupDiGetDeviceInstallParamsW
        SetupDiGetDeviceInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDeviceInstallParams = SetupDiGetDeviceInstallParamsW
        else:
            SetupDiGetDeviceInstallParams = SetupDiGetDeviceInstallParamsA
        # END IF

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_writes_bytes_to_opt_(ClassInstallParamsSize, *RequiredSize) PSP_CLASSINSTALL_HEADER ClassInstallParams,
        # _In_ DWORD ClassInstallParamsSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetClassInstallParamsA = setupapi.SetupDiGetClassInstallParamsA
        SetupDiGetClassInstallParamsA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _Out_writes_bytes_to_opt_(ClassInstallParamsSize, *RequiredSize) PSP_CLASSINSTALL_HEADER ClassInstallParams,
        # _In_ DWORD ClassInstallParamsSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetClassInstallParamsW = setupapi.SetupDiGetClassInstallParamsW
        SetupDiGetClassInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetClassInstallParams = SetupDiGetClassInstallParamsW
        else:
            SetupDiGetClassInstallParams = SetupDiGetClassInstallParamsA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDeviceInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DEVINSTALL_PARAMS_A DeviceInstallParams
        # );
        SetupDiSetDeviceInstallParamsA = setupapi.SetupDiSetDeviceInstallParamsA
        SetupDiSetDeviceInstallParamsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDeviceInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DEVINSTALL_PARAMS_W DeviceInstallParams
        # );
        SetupDiSetDeviceInstallParamsW = setupapi.SetupDiSetDeviceInstallParamsW
        SetupDiSetDeviceInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiSetDeviceInstallParams = SetupDiSetDeviceInstallParamsW
        else:
            SetupDiSetDeviceInstallParams = SetupDiSetDeviceInstallParamsA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetClassInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_reads_bytes_opt_(ClassInstallParamsSize) PSP_CLASSINSTALL_HEADER ClassInstallParams,
        # _In_ DWORD ClassInstallParamsSize
        # );
        SetupDiSetClassInstallParamsA = setupapi.SetupDiSetClassInstallParamsA
        SetupDiSetClassInstallParamsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetClassInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_reads_bytes_opt_(ClassInstallParamsSize) PSP_CLASSINSTALL_HEADER ClassInstallParams,
        # _In_ DWORD ClassInstallParamsSize
        # );
        SetupDiSetClassInstallParamsW = setupapi.SetupDiSetClassInstallParamsW
        SetupDiSetClassInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiSetClassInstallParams = SetupDiSetClassInstallParamsW
        else:
            SetupDiSetClassInstallParams = SetupDiSetClassInstallParamsA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDriverInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_A DriverInfoData,
        # _Out_ PSP_DRVINSTALL_PARAMS DriverInstallParams
        # );
        SetupDiGetDriverInstallParamsA = setupapi.SetupDiGetDriverInstallParamsA
        SetupDiGetDriverInstallParamsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetDriverInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_W DriverInfoData,
        # _Out_ PSP_DRVINSTALL_PARAMS DriverInstallParams
        # );
        SetupDiGetDriverInstallParamsW = setupapi.SetupDiGetDriverInstallParamsW
        SetupDiGetDriverInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetDriverInstallParams = SetupDiGetDriverInstallParamsW
        else:
            SetupDiGetDriverInstallParams = SetupDiGetDriverInstallParamsA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDriverInstallParamsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_A DriverInfoData,
        # _In_ PSP_DRVINSTALL_PARAMS DriverInstallParams
        # );
        SetupDiSetDriverInstallParamsA = setupapi.SetupDiSetDriverInstallParamsA
        SetupDiSetDriverInstallParamsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetDriverInstallParamsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_DRVINFO_DATA_W DriverInfoData,
        # _In_ PSP_DRVINSTALL_PARAMS DriverInstallParams
        # );
        SetupDiSetDriverInstallParamsW = setupapi.SetupDiSetDriverInstallParamsW
        SetupDiSetDriverInstallParamsW.restype = BOOL

        if defined(UNICODE):
            SetupDiSetDriverInstallParams = SetupDiSetDriverInstallParamsW
        else:
            SetupDiSetDriverInstallParams = SetupDiSetDriverInstallParamsA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiLoadClassIcon(
        # _In_ CONST GUID *ClassGuid,
        # _Out_opt_ HICON *LargeIcon,
        # _Out_opt_ PINT MiniIconIndex
        # );
        SetupDiLoadClassIcon = setupapi.SetupDiLoadClassIcon
        SetupDiLoadClassIcon.restype = BOOL

        if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiLoadDeviceIcon(
            # _In_ HDEVINFO DeviceInfoSet,
            # _In_ PSP_DEVINFO_DATA DeviceInfoData,
            # _In_ UINT cxIcon,
            # _In_ UINT cyIcon,
            # _In_ DWORD Flags,
            # _Out_ HICON *hIcon
            # );
            SetupDiLoadDeviceIcon = setupapi.SetupDiLoadDeviceIcon
            SetupDiLoadDeviceIcon.restype = BOOL
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

        # Flags controlling the drawing of mini - icons
        DMI_MASK = 0x00000001
        DMI_BKCOLOR = 0x00000002
        DMI_USERECT = 0x00000004
        # WINSETUPAPI
        # INT
        # WINAPI
        # SetupDiDrawMiniIcon(
        # _In_ HDC hdc,
        # _In_ RECT rc,
        # _In_ INT MiniIconIndex,
        # _In_ DWORD Flags
        # );
        SetupDiDrawMiniIcon = setupapi.SetupDiDrawMiniIcon
        SetupDiDrawMiniIcon.restype = INT
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassBitmapIndex(
        # _In_opt_ CONST GUID *ClassGuid,
        # _Out_ PINT MiniIconIndex
        # );
        SetupDiGetClassBitmapIndex = setupapi.SetupDiGetClassBitmapIndex
        SetupDiGetClassBitmapIndex.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassImageList(
        # _Out_ PSP_CLASSIMAGELIST_DATA ClassImageListData
        # );
        SetupDiGetClassImageList = setupapi.SetupDiGetClassImageList
        SetupDiGetClassImageList.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassImageListExA(
        # _Out_ PSP_CLASSIMAGELIST_DATA ClassImageListData,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassImageListExA = setupapi.SetupDiGetClassImageListExA
        SetupDiGetClassImageListExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassImageListExW(
        # _Out_ PSP_CLASSIMAGELIST_DATA ClassImageListData,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetClassImageListExW = setupapi.SetupDiGetClassImageListExW
        SetupDiGetClassImageListExW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetClassImageListEx = SetupDiGetClassImageListExW
        else:
            SetupDiGetClassImageListEx = SetupDiGetClassImageListExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassImageIndex(
        # _In_ PSP_CLASSIMAGELIST_DATA ClassImageListData,
        # _In_ CONST GUID *ClassGuid,
        # _Out_ PINT ImageIndex
        # );
        SetupDiGetClassImageIndex = setupapi.SetupDiGetClassImageIndex
        SetupDiGetClassImageIndex.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiDestroyClassImageList(
        # _In_ PSP_CLASSIMAGELIST_DATA ClassImageListData
        # );
        SetupDiDestroyClassImageList = setupapi.SetupDiDestroyClassImageList
        SetupDiDestroyClassImageList.restype = BOOL

        # PropertySheetType values for the SetupDiGetClassDevPropertySheets API
        DIGCDP_FLAG_BASIC = 0x00000001
        DIGCDP_FLAG_ADVANCED = 0x00000002

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            DIGCDP_FLAG_REMOTE_BASIC = 0x00000003
            DIGCDP_FLAG_REMOTE_ADVANCED = 0x00000004
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDevPropertySheetsA(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ LPPROPSHEETHEADERA PropertySheetHeader,
        # _In_ DWORD PropertySheetHeaderPageListSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_ DWORD PropertySheetType
        # );
        SetupDiGetClassDevPropertySheetsA = setupapi.SetupDiGetClassDevPropertySheetsA
        SetupDiGetClassDevPropertySheetsA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetClassDevPropertySheetsW(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ LPPROPSHEETHEADERW PropertySheetHeader,
        # _In_ DWORD PropertySheetHeaderPageListSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_ DWORD PropertySheetType
        # );
        SetupDiGetClassDevPropertySheetsW = setupapi.SetupDiGetClassDevPropertySheetsW
        SetupDiGetClassDevPropertySheetsW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetClassDevPropertySheets = (
                SetupDiGetClassDevPropertySheetsW
            )
        else:
            SetupDiGetClassDevPropertySheets = (
                SetupDiGetClassDevPropertySheetsA
            )
        # END IF

        # Define ICON IDs publicly exposed from setupapi.
        IDI_RESOURCEFIRST = 159
        IDI_RESOURCE = 159
        IDI_RESOURCELAST = 161
        IDI_RESOURCEOVERLAYFIRST = 161
        IDI_RESOURCEOVERLAYLAST = 161
        IDI_CONFLICT = 161
        IDI_CLASSICON_OVERLAYFIRST = 500
        IDI_CLASSICON_OVERLAYLAST = 502
        IDI_PROBLEM_OVL = 500
        IDI_DISABLED_OVL = 501
        IDI_FORCED_OVL = 502
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiAskForOEMDisk(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiAskForOEMDisk = setupapi.SetupDiAskForOEMDisk
        SetupDiAskForOEMDisk.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSelectOEMDrv(
        # _In_opt_ HWND hwndParent,
        # _In_ HDEVINFO DeviceInfoSet,
        # _Inout_opt_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiSelectOEMDrv = setupapi.SetupDiSelectOEMDrv
        SetupDiSelectOEMDrv.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassNameFromGuidA(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassNameSize) PSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiClassNameFromGuidA = setupapi.SetupDiClassNameFromGuidA
        SetupDiClassNameFromGuidA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassNameFromGuidW(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassNameSize) PWSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiClassNameFromGuidW = setupapi.SetupDiClassNameFromGuidW
        SetupDiClassNameFromGuidW.restype = BOOL

        if defined(UNICODE):
            SetupDiClassNameFromGuid = SetupDiClassNameFromGuidW
        else:
            SetupDiClassNameFromGuid = SetupDiClassNameFromGuidA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassNameFromGuidExA(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassNameSize) PSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiClassNameFromGuidExA = setupapi.SetupDiClassNameFromGuidExA
        SetupDiClassNameFromGuidExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassNameFromGuidExW(
        # _In_ CONST GUID *ClassGuid,
        # _Out_writes_(ClassNameSize) PWSTR ClassName,
        # _In_ DWORD ClassNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiClassNameFromGuidExW = setupapi.SetupDiClassNameFromGuidExW
        SetupDiClassNameFromGuidExW.restype = BOOL

        if defined(UNICODE):
            SetupDiClassNameFromGuidEx = SetupDiClassNameFromGuidExW
        else:
            SetupDiClassNameFromGuidEx = SetupDiClassNameFromGuidExA
        # END IF

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassGuidsFromNameA(
        # _In_ PCSTR ClassName,
        # _Out_writes_to_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize
        # );
        SetupDiClassGuidsFromNameA = setupapi.SetupDiClassGuidsFromNameA
        SetupDiClassGuidsFromNameA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassGuidsFromNameW(
        # _In_ PCWSTR ClassName,
        # _Out_writes_to_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize
        # );
        SetupDiClassGuidsFromNameW = setupapi.SetupDiClassGuidsFromNameW
        SetupDiClassGuidsFromNameW.restype = BOOL

        if defined(UNICODE):
            SetupDiClassGuidsFromName = SetupDiClassGuidsFromNameW
        else:
            SetupDiClassGuidsFromName = SetupDiClassGuidsFromNameA
        # END IF

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassGuidsFromNameExA(
        # _In_ PCSTR ClassName,
        # _Out_writes_to_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiClassGuidsFromNameExA = setupapi.SetupDiClassGuidsFromNameExA
        SetupDiClassGuidsFromNameExA.restype = BOOL

        # _Success_(return != FALSE)
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiClassGuidsFromNameExW(
        # _In_ PCWSTR ClassName,
        # _Out_writes_to_(ClassGuidListSize, *RequiredSize) LPGUID ClassGuidList,
        # _In_ DWORD ClassGuidListSize,
        # _Out_ PDWORD RequiredSize,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiClassGuidsFromNameExW = setupapi.SetupDiClassGuidsFromNameExW
        SetupDiClassGuidsFromNameExW.restype = BOOL

        if defined(UNICODE):
            SetupDiClassGuidsFromNameEx = SetupDiClassGuidsFromNameExW
        else:
            SetupDiClassGuidsFromNameEx = SetupDiClassGuidsFromNameExA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileFriendlyNameA(
        # _In_ DWORD HwProfile,
        # _Out_writes_(FriendlyNameSize) PSTR FriendlyName,
        # _In_ DWORD FriendlyNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetHwProfileFriendlyNameA = setupapi.SetupDiGetHwProfileFriendlyNameA
        SetupDiGetHwProfileFriendlyNameA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileFriendlyNameW(
        # _In_ DWORD HwProfile,
        # _Out_writes_(FriendlyNameSize) PWSTR FriendlyName,
        # _In_ DWORD FriendlyNameSize,
        # _Out_opt_ PDWORD RequiredSize
        # );
        SetupDiGetHwProfileFriendlyNameW = setupapi.SetupDiGetHwProfileFriendlyNameW
        SetupDiGetHwProfileFriendlyNameW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetHwProfileFriendlyName = SetupDiGetHwProfileFriendlyNameW
        else:
            SetupDiGetHwProfileFriendlyName = SetupDiGetHwProfileFriendlyNameA
        # END IF
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileFriendlyNameExA(
        # _In_ DWORD HwProfile,
        # _Out_writes_(FriendlyNameSize) PSTR FriendlyName,
        # _In_ DWORD FriendlyNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetHwProfileFriendlyNameExA = setupapi.SetupDiGetHwProfileFriendlyNameExA
        SetupDiGetHwProfileFriendlyNameExA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetHwProfileFriendlyNameExW(
        # _In_ DWORD HwProfile,
        # _Out_writes_(FriendlyNameSize) PWSTR FriendlyName,
        # _In_ DWORD FriendlyNameSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _In_opt_ PCWSTR MachineName,
        # _Reserved_ PVOID Reserved
        # );
        SetupDiGetHwProfileFriendlyNameExW = setupapi.SetupDiGetHwProfileFriendlyNameExW
        SetupDiGetHwProfileFriendlyNameExW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetHwProfileFriendlyNameEx = (
                SetupDiGetHwProfileFriendlyNameExW
            )
        else:
            SetupDiGetHwProfileFriendlyNameEx = (
                SetupDiGetHwProfileFriendlyNameExA
            )
        # END IF

        # PageType values for SetupDiGetWizardPage API
        SPWPT_SELECTDEVICE = 0x00000001

        # Flags for SetupDiGetWizardPage API
        SPWP_USE_DEVINFO_DATA = 0x00000001
        # WINSETUPAPI
        # HPROPSHEETPAGE
        # WINAPI
        # SetupDiGetWizardPage(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_opt_ PSP_DEVINFO_DATA DeviceInfoData,
        # _In_ PSP_INSTALLWIZARD_DATA InstallWizardData,
        # _In_ DWORD PageType,
        # _In_ DWORD Flags
        # );
        SetupDiGetWizardPage = setupapi.SetupDiGetWizardPage
        SetupDiGetWizardPage.restype = HPROPSHEETPAGE
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetSelectedDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _Out_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiGetSelectedDevice = setupapi.SetupDiGetSelectedDevice
        SetupDiGetSelectedDevice.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiSetSelectedDevice(
        # _In_ HDEVINFO DeviceInfoSet,
        # _In_ PSP_DEVINFO_DATA DeviceInfoData
        # );
        SetupDiSetSelectedDevice = setupapi.SetupDiSetSelectedDevice
        SetupDiSetSelectedDevice.restype = BOOL

        if _SETUPAPI_VER >= _WIN32_WINNT_WS03:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetActualModelsSectionA(
            # _In_ PINFCONTEXT Context,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _Out_writes_opt_(InfSectionWithExtSize) PSTR InfSectionWithExt,
            # _In_ DWORD InfSectionWithExtSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetActualModelsSectionA = setupapi.SetupDiGetActualModelsSectionA
            SetupDiGetActualModelsSectionA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetActualModelsSectionW(
            # _In_ PINFCONTEXT Context,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _Out_writes_opt_(InfSectionWithExtSize) PWSTR InfSectionWithExt,
            # _In_ DWORD InfSectionWithExtSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetActualModelsSectionW = setupapi.SetupDiGetActualModelsSectionW
            SetupDiGetActualModelsSectionW.restype = BOOL
            if defined(UNICODE):
                SetupDiGetActualModelsSection = SetupDiGetActualModelsSectionW
            else:
                SetupDiGetActualModelsSection = SetupDiGetActualModelsSectionA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WS03
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetActualSectionToInstallA(
        # _In_ HINF InfHandle,
        # _In_ PCSTR InfSectionName,
        # _Out_writes_opt_(InfSectionWithExtSize) PSTR InfSectionWithExt,
        # _In_ DWORD InfSectionWithExtSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _Out_opt_ PSTR *Extension
        # );
        SetupDiGetActualSectionToInstallA = setupapi.SetupDiGetActualSectionToInstallA
        SetupDiGetActualSectionToInstallA.restype = BOOL
        # WINSETUPAPI
        # BOOL
        # WINAPI
        # SetupDiGetActualSectionToInstallW(
        # _In_ HINF InfHandle,
        # _In_ PCWSTR InfSectionName,
        # _Out_writes_opt_(InfSectionWithExtSize) PWSTR InfSectionWithExt,
        # _In_ DWORD InfSectionWithExtSize,
        # _Out_opt_ PDWORD RequiredSize,
        # _Out_opt_ PWSTR *Extension
        # );
        SetupDiGetActualSectionToInstallW = setupapi.SetupDiGetActualSectionToInstallW
        SetupDiGetActualSectionToInstallW.restype = BOOL

        if defined(UNICODE):
            SetupDiGetActualSectionToInstall = (
                SetupDiGetActualSectionToInstallW
            )
        else:
            SetupDiGetActualSectionToInstall = (
                SetupDiGetActualSectionToInstallA
            )
        # END IF

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetActualSectionToInstallExA(
            # _In_ HINF InfHandle,
            # _In_ PCSTR InfSectionName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _Out_writes_opt_(InfSectionWithExtSize) PSTR InfSectionWithExt,
            # _In_ DWORD InfSectionWithExtSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _Out_opt_ PSTR *Extension,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetActualSectionToInstallExA = setupapi.SetupDiGetActualSectionToInstallExA
            SetupDiGetActualSectionToInstallExA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetActualSectionToInstallExW(
            # _In_ HINF InfHandle,
            # _In_ PCWSTR InfSectionName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AlternatePlatformInfo,
            # _Out_writes_opt_(InfSectionWithExtSize) PWSTR InfSectionWithExt,
            # _In_ DWORD InfSectionWithExtSize,
            # _Out_opt_ PDWORD RequiredSize,
            # _Out_opt_ PWSTR *Extension,
            # _Reserved_ PVOID Reserved
            # );
            SetupDiGetActualSectionToInstallExW = setupapi.SetupDiGetActualSectionToInstallExW
            SetupDiGetActualSectionToInstallExW.restype = BOOL
            if defined(UNICODE):
                SetupDiGetActualSectionToInstallEx = (
                    SetupDiGetActualSectionToInstallExW
                )
            else:
                SetupDiGetActualSectionToInstallEx = (
                    SetupDiGetActualSectionToInstallExA
                )
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # SetupEnumInfSections is for low - level parsing of an INF

            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupEnumInfSectionsA (
            # _In_ HINF InfHandle,
            # _In_ UINT Index,
            # _Out_writes_opt_(Size) PSTR Buffer,
            # _In_ UINT Size,
            # _Out_opt_ UINT *SizeNeeded
            # );
            SetupEnumInfSectionsA = setupapi.SetupEnumInfSectionsA
            SetupEnumInfSectionsA.restype = BOOL

            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupEnumInfSectionsW (
            # _In_ HINF InfHandle,
            # _In_ UINT Index,
            # _Out_writes_opt_(Size) PWSTR Buffer,
            # _In_ UINT Size,
            # _Out_opt_ UINT *SizeNeeded
            # );
            SetupEnumInfSectionsW = setupapi.SetupEnumInfSectionsW
            SetupEnumInfSectionsW.restype = BOOL

            if defined(UNICODE):
                SetupEnumInfSections = SetupEnumInfSectionsW
            else:
                SetupEnumInfSections = SetupEnumInfSectionsA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:
            class _SP_INF_SIGNER_INFO_V1_A(ctypes.Structure):
                pass


            _SP_INF_SIGNER_INFO_V1_A._fields_ = [
                ('cbSize', DWORD),
                ('CatalogFile', CHAR * MAX_PATH),
                ('DigitalSigner', CHAR * MAX_PATH),
                ('DigitalSignerVersion', CHAR * MAX_PATH),
            ]
            SP_INF_SIGNER_INFO_V1_A = _SP_INF_SIGNER_INFO_V1_A
            PSP_INF_SIGNER_INFO_V1_A = POINTER(_SP_INF_SIGNER_INFO_V1_A)
            class _SP_INF_SIGNER_INFO_V1_W(ctypes.Structure):
                pass


            _SP_INF_SIGNER_INFO_V1_W._fields_ = [
                ('cbSize', DWORD),
                ('CatalogFile', WCHAR * MAX_PATH),
                ('DigitalSigner', WCHAR * MAX_PATH),
                ('DigitalSignerVersion', WCHAR * MAX_PATH),
            ]
            SP_INF_SIGNER_INFO_V1_W = _SP_INF_SIGNER_INFO_V1_W
            PSP_INF_SIGNER_INFO_V1_W = POINTER(_SP_INF_SIGNER_INFO_V1_W)
            if defined(UNICODE):
                SP_INF_SIGNER_INFO_V1 = SP_INF_SIGNER_INFO_V1_W
                PSP_INF_SIGNER_INFO_V1 = PSP_INF_SIGNER_INFO_V1_W
            else:
                SP_INF_SIGNER_INFO_V1 = SP_INF_SIGNER_INFO_V1_A
                PSP_INF_SIGNER_INFO_V1 = PSP_INF_SIGNER_INFO_V1_A
            # END IF
            if _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN:
                class _SP_INF_SIGNER_INFO_V2_A(ctypes.Structure):
                    pass


                _SP_INF_SIGNER_INFO_V2_A._fields_ = [
                    ('cbSize', DWORD),
                    ('CatalogFile', CHAR * MAX_PATH),
                    ('DigitalSigner', CHAR * MAX_PATH),
                    ('DigitalSignerVersion', CHAR * MAX_PATH),
                    ('SignerScore', DWORD),
                ]
                SP_INF_SIGNER_INFO_V2_A = _SP_INF_SIGNER_INFO_V2_A
                PSP_INF_SIGNER_INFO_V2_A = POINTER(_SP_INF_SIGNER_INFO_V2_A)
                class _SP_INF_SIGNER_INFO_V2_W(ctypes.Structure):
                    pass


                _SP_INF_SIGNER_INFO_V2_W._fields_ = [
                    ('cbSize', DWORD),
                    ('CatalogFile', WCHAR * MAX_PATH),
                    ('DigitalSigner', WCHAR * MAX_PATH),
                    ('DigitalSignerVersion', WCHAR * MAX_PATH),
                    ('SignerScore', DWORD),
                ]
                SP_INF_SIGNER_INFO_V2_W = _SP_INF_SIGNER_INFO_V2_W
                PSP_INF_SIGNER_INFO_V2_W = POINTER(_SP_INF_SIGNER_INFO_V2_W)
                if defined(UNICODE):
                    SP_INF_SIGNER_INFO_V2 = SP_INF_SIGNER_INFO_V2_W
                    PSP_INF_SIGNER_INFO_V2 = PSP_INF_SIGNER_INFO_V2_W
                else:
                    SP_INF_SIGNER_INFO_V2 = SP_INF_SIGNER_INFO_V2_A
                    PSP_INF_SIGNER_INFO_V2 = PSP_INF_SIGNER_INFO_V2_A
                # END IF

                # Driver signer scores
                # (high order bit of the signing byte means UINT)
                SIGNERSCORE_UNKNOWN = 0xFF000000
                SIGNERSCORE_W9X_SUSPECT = 0xC0000000
                SIGNERSCORE_UNSIGNED = 0x80000000
                SIGNERSCORE_AUTHENTICODE = 0x0F000000
                SIGNERSCORE_WHQL = 0x0D000005
                SIGNERSCORE_UNCLASSIFIED = 0x0D000004
                SIGNERSCORE_INBOX = 0x0D000003
                SIGNERSCORE_LOGO_STANDARD = 0x0D000002
                SIGNERSCORE_LOGO_PREMIUM = 0x0D000001
                SIGNERSCORE_MASK = 0xFF000000
                SIGNERSCORE_SIGNED_MASK = 0xF0000000
            # END IF   _SETUPAPI_VER >= _WIN32_WINNT_LONGHORN

            if USE_SP_INF_SIGNER_INFO_V1 or (_SETUPAPI_VER < _WIN32_WINNT_LONGHORN): #  use version 1 signer info structure
                SP_INF_SIGNER_INFO_A = SP_INF_SIGNER_INFO_V1_A
                PSP_INF_SIGNER_INFO_A = PSP_INF_SIGNER_INFO_V1_A
                SP_INF_SIGNER_INFO_W = SP_INF_SIGNER_INFO_V1_W
                PSP_INF_SIGNER_INFO_W = PSP_INF_SIGNER_INFO_V1_W
                SP_INF_SIGNER_INFO = SP_INF_SIGNER_INFO_V1
                PSP_INF_SIGNER_INFO = PSP_INF_SIGNER_INFO_V1
            else: #  use version 2 signer info structure
                SP_INF_SIGNER_INFO_A = SP_INF_SIGNER_INFO_V2_A
                PSP_INF_SIGNER_INFO_A = PSP_INF_SIGNER_INFO_V2_A
                SP_INF_SIGNER_INFO_W = SP_INF_SIGNER_INFO_V2_W
                PSP_INF_SIGNER_INFO_W = PSP_INF_SIGNER_INFO_V2_W
                SP_INF_SIGNER_INFO = SP_INF_SIGNER_INFO_V2
                PSP_INF_SIGNER_INFO = PSP_INF_SIGNER_INFO_V2
            # END IF   use current version of signer info structure
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupVerifyInfFileA(
            # _In_ PCSTR InfName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AltPlatformInfo,
            # _Inout_ PSP_INF_SIGNER_INFO_A InfSignerInfo
            # );
            SetupVerifyInfFileA = setupapi.SetupVerifyInfFileA
            SetupVerifyInfFileA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupVerifyInfFileW(
            # _In_ PCWSTR InfName,
            # _In_opt_ PSP_ALTPLATFORM_INFO AltPlatformInfo,
            # _Inout_ PSP_INF_SIGNER_INFO_W InfSignerInfo
            # );
            SetupVerifyInfFileW = setupapi.SetupVerifyInfFileW
            SetupVerifyInfFileW.restype = BOOL
            if defined(UNICODE):
                SetupVerifyInfFile = SetupVerifyInfFileW
            else:
                SetupVerifyInfFile = SetupVerifyInfFileA
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WINXP:

            # Flags for use by SetupDiGetCustomDeviceProperty
            DICUSTOMDEVPROP_MERGE_MULTISZ = 0x00000001

            # _Success_(return != FALSE)
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetCustomDevicePropertyA(
            # _In_ HDEVINFO DeviceInfoSet,
            # _In_ PSP_DEVINFO_DATA DeviceInfoData,
            # _In_ PCSTR CustomPropertyName,
            # _In_ DWORD Flags,
            # _Out_opt_ PDWORD PropertyRegDataType,
            # _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupDiGetCustomDevicePropertyA = setupapi.SetupDiGetCustomDevicePropertyA
            SetupDiGetCustomDevicePropertyA.restype = BOOL

            # _Success_(return != FALSE)
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupDiGetCustomDevicePropertyW(
            # _In_ HDEVINFO DeviceInfoSet,
            # _In_ PSP_DEVINFO_DATA DeviceInfoData,
            # _In_ PCWSTR CustomPropertyName,
            # _In_ DWORD Flags,
            # _Out_opt_ PDWORD PropertyRegDataType,
            # _Out_writes_bytes_to_(PropertyBufferSize, *RequiredSize) PBYTE PropertyBuffer,
            # _In_ DWORD PropertyBufferSize,
            # _Out_opt_ PDWORD RequiredSize
            # );
            SetupDiGetCustomDevicePropertyW = setupapi.SetupDiGetCustomDevicePropertyW
            SetupDiGetCustomDevicePropertyW.restype = BOOL

            if defined(UNICODE):
                SetupDiGetCustomDeviceProperty = (
                    SetupDiGetCustomDevicePropertyW
                )
            else:
                SetupDiGetCustomDeviceProperty = (
                    SetupDiGetCustomDevicePropertyA
                )
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WINXP

        if _SETUPAPI_VER >= _WIN32_WINNT_WS03:

            # To configure WMI security for downlevel platforms where the
            # [DDInstall.WMI]

            # section isn't natively supported by setupapi, a redistributable
            # co - installer

            # is supplied in the DDK for use on those platforms.

            # Flags for use by SetupConfigureWmiFromInfSection
            SCWMI_CLOBBER_SECURITY = 0x00000001
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupConfigureWmiFromInfSectionA(
            # _In_ HINF InfHandle,
            # _In_ PCSTR SectionName,
            # _In_ DWORD Flags
            # );
            SetupConfigureWmiFromInfSectionA = setupapi.SetupConfigureWmiFromInfSectionA
            SetupConfigureWmiFromInfSectionA.restype = BOOL
            # WINSETUPAPI
            # BOOL
            # WINAPI
            # SetupConfigureWmiFromInfSectionW(
            # _In_ HINF InfHandle,
            # _In_ PCWSTR SectionName,
            # _In_ DWORD Flags
            # );
            SetupConfigureWmiFromInfSectionW = setupapi.SetupConfigureWmiFromInfSectionW
            SetupConfigureWmiFromInfSectionW.restype = BOOL

            if defined(UNICODE):
                SetupConfigureWmiFromInfSection = (
                    SetupConfigureWmiFromInfSectionW
                )
            else:
                SetupConfigureWmiFromInfSection = (
                    SetupConfigureWmiFromInfSectionA
                )
            # END IF
        # END IF   _SETUPAPI_VER >= _WIN32_WINNT_WS03

        if defined(__cplusplus):
            pass
        # END IF
        if defined (_MSC_VER) and (_MSC_VER >= 1200):
            pass
        # END IF
    # END IF  WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
# END IF   _INC_SETUPAPI
